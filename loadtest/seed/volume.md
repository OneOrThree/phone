# 시드 볼륨 설계 — post-V6 스키마 기준 (진실 원천: `back/src/main/resources/db/migration/V1~V6`)

> PRD §8-2(작성 시점 post-V2)와 부하테스트-아키텍처.md(v2 설계) 볼륨표의 **최신 교정본**.
> 이후 V7+ 마이그레이션이 추가되면 이 표부터 갱신한다. `SEED_VERSION=seed-v1`.

## 볼륨표

| 테이블 | 건수(SCALE=1) | 생성 경로 | 핵심 제약·규칙 (post-V6) |
|---|---|---|---|
...
| groups | 5만 | CSV | V5로 미션 컬럼 없음·V3로 code 없음. 한글 name(Phase 3 표적)·status WAITING/ACTIVE/ENDED·host=멤버 공식 i=1 |
| group_join_codes | 0 | — | V3 신설 — 부하 표적 아님, 시드 생략 |
| group_members | ~100만 (그룹×20) | SQL | UNIQUE(user,group)·role OWNER(i=1)/MEMBER·status INACTIVE/CHALLENGE/FOCUS·is_left |

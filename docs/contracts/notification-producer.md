...
| 중복 기기 토큰 0건 | `users.device_token` 에는 UNIQUE 가 없어 로그아웃 없이 계정을 갈아탄 기기의 토큰이 두 계정에 남는다. 그대로 내보내면 두 `device` 레코드가 같은 `device:<토큰 해시>` 키를 갖는다 — 같은 배치면 `DUPLICATE_RECORD_KEY` 로 요청 전체가 실패하고, 배치가 갈리면 한 소유자가 조용히 덮여 manifest 검증이 깨진다. **검출만 하고 통과시키면 검출한 의미가 없다** |

다섯을 통과했는지는 문서의 `report.finalEligible` 에 **계산해서 적어 둔다** — 읽는 쪽마다 다섯 조건을
다시 조합하면 한 곳이 하나를 빠뜨려도 드러나지 않는다. 파일 이름으로는 탐색용과 최종본이 구분되지
않으므로, CLI 도 둘 중 무엇을 만들었는지 로그로 분명히 남긴다(경고 쪽에는 다섯 값을 전부 적는다 —
하나를 빼면 그것이 원인일 때 운영자가 볼 곳이 없다).

중복 목록은 `device` 자원과 **같은 범위**에서 센다(봇 제외). 봇의 토큰은 export 에 실리지 않아
충돌을 만들 수 없는데 그것까지 세면 열릴 수 없는 게이트가 된다.

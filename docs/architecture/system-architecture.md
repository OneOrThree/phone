...
flowchart TB
  ...
  RDS[("RDS PostgreSQL<br/>gromo(링크 원장 포함) · gromo_notification<br/>(dev: db 컨테이너에 2 database)")]
  FCM["FCM"]
  DD["Datadog us5"]

  APPc --> CF --> NX
  APPLE -->|"/.well-known/skadnetwork/…"| CF
  NX --> BIZ
  NX --> NS
  BIZ --> DATA
...
| `/internal/admin/**` | notification | 서비스 토큰(콘솔 전용). ~~Vercel egress IP 는 가변이라 IP 허용 목록 없음~~ → A23: 콘솔은 Business API 안(같은 compose) — 알림 화면 연결은 L16 대로 나중 |
| `/health` | business-api (각 서비스 헬스는 compose 내부) | 없음 |
| `/l/**` · `/link/**` · `/.well-known/**` · `/console/**` | ~~**nginx 가 아니라 DNS** — `link.oneorthree.world` → Vercel~~ → **A23(2026-09-13): business-api** — nginx 라우팅 한 번으로 전환한다(링크 HLD §7 4단계). data-api 로 가는 링크 경로는 없다(공인 노출 0) | `/l/referrer` 만 AT(게스트 포함, 링크 정책 L23) · 콘솔은 팀원별 비밀번호 슬롯 + 서명 쿠키(L16) · 나머지 공개 경로는 무인증 + IP HMAC 레이트리밋(링크 LLD §4.3) · 인증 예외는 (메서드, 경로) 정확 목록(링크 HLD §5) |
| ~~**`/l/match` · `/l/referrer` (이관 호환)**~~ → A23: 이관 호환 경로 없음 — 위 행에 합쳐진다 | **A23(2026-09-13): 아래 원문은 폐기.** 원본 IP 는 현행 규칙(nginx 가 Cloudflare 대역 피어일 때만 `CF-Connecting-IP` 를 믿어 `X-Real-IP` 로 넘긴다, 링크 HLD §5)이고 `X-Link-Client-IP`·`X-Link-Proxy-Secret`·`LINK_PROXY_SECRET`·Vercel 프록시는 쓰지 않는다(링크 HLD §2). 원문: ~~nginx → **Vercel 프록시** — 배포된 앱이 `${API_URL}/l/match`(= `api.oneorthree.world`)로 부르고 있어(`deferredInvite.ts`) 이 경로를 지우면 설치 매치가 실패하고 3시간 어트리뷰션 창을 잃는다. **원본 IP 를 반드시 보존한다** — 매치는 클릭 때 저장한 `ip_hash`(SHA-256(ip+salt))와 대조하는데, 그냥 프록시하면 링크 서버엔 EC2 주소가 보여 **정상 설치도 `matched:false`** 가 된다. nginx 가 `X-Forwarded-For` 를 **검증된 원본 IP 로 재작성하고**(클라이언트가 위조한 값 무시) — **`$remote_addr` 로 그냥 덮으면 안 된다**: 운영 경로가 **앱 → Cloudflare → nginx** 라 그 값은 **Cloudflare 엣지 IP** 이고, 그러면 Vercel 랜딩 때 저장한 방문자 IP 해시와 달라 **정상 설치도 `matched:false`** 가 된다. 반대로 요청의 `CF-Connecting-IP` 를 **그대로 믿으면 오리진을 직접 때린 호출자가 위조**할 수 있다. 그래서 **`set_real_ip_from` 에 Cloudflare CIDR 만 등록하고(+ 오리진 방화벽으로 CF 외 접근 차단) 그때만 `CF-Connecting-IP` 를 신뢰해 XFF 를 재작성**한다. Vercel은 수신 XFF를 플랫폼에서 덮어쓰므로 nginx는 같은 검증된 IP를 **`X-Link-Client-IP`에도 실어야 한다**. `X-Link-Proxy-Secret`에 프록시 전용 공유 시크릿을 함께 실으며, 링크 서버는 **그 시크릿이 유효할 때만 `X-Link-Client-IP`의 단일 IP를 신뢰**한다. **시크릿이 없는 직접 요청(방문자 → Vercel)은 소켓 주소가 아니라 Vercel 이 보장하는 클라이언트 IP 헤더**를 쓴다 — 서버리스 함수의 소켓 상대는 방문자가 아니라 플랫폼 프록시이고 실행 방식에 따라 소켓 정보가 아예 없어서, 소켓을 쓰면 클릭에 방문자가 아닌 IP 가 저장돼 나중에 nginx 가 전달한 실제 IP 와 해시가 어긋난다~~ | ~~무인증(현행과 동일) + 프록시 시크릿~~ → 위 행 |
...
- 백업·파라미터·모니터링은 인스턴스 단위 그대로. ~~Neon 은 링크 서버 소유(링크 장부).~~ → A23: 링크 원장은 `gromo` database 안(Data API) — 별도 DB 없음.
...
| `SVC_TOKEN_BIZ_TO_DATA` · **`SVC_TOKEN_NOTI_TO_DATA`**(리컨실 — 서비스 전용 호출) · **`SVC_TOKEN_BIZ_TO_NOTI`** · **`SVC_TOKEN_DATA_TO_NOTI`** · ~~`SVC_TOKEN_CONSOLE_TO_NOTI`~~(→ A23: A11 ⑥ 의 `link콘솔→notification` 폐기 — 콘솔은 Business API 안, 알림 화면은 L16 대로 나중) · ~~**`SVC_TOKEN_BIZ_TO_LINK`** · **`SVC_TOKEN_DATA_TO_LINK`**~~(→ A23: 링크 서버 없음 — #745 잔여는 링크 구현 PR 이 걷어낸다, 링크 LLD §9) — **위성 호출 토큰은 caller 별로 쪼갠다**(~~총 7종~~ → 4종). 같은 비밀값을 Business 와 Data 에 함께 주면 수신 측이 **어느 프로세스가 불렀는지 구분할 수 없어** Business 용 경로(기기 토큰·설정~~·링크 발급~~)와 Data relay 용 경로(~~withdraw·revoke·joined~~ → A23: 알림 대상 재전달)의 허용목록이 **합쳐지고**, 한쪽 자격이 유출되면 다른 쪽 전용 명령까지 열린다. 토큰마다 audience 와 메서드·경로를 **독립적으로** 제한한다(A11) | 발신·수신 양쪽 |
...
| 콘솔 비밀번호(해시) | ~~Vercel env (링크 레포)~~ → A23: business-api env — 팀원별 `CONSOLE_SLOT_<NAME>_HASH`(bcrypt) + `CONSOLE_COOKIE_KEY`(링크 정책 L16 · LLD §6.1) |
| `LINK_IP_SALT` ~~· SKAN 키~~ | ~~링크 서버 (Vercel env)~~ → A23: **data-api 만** — IP 해시는 data-api 가 만들고 솔트는 한 곳에만 둔다(링크 정책 L03). **기존 운영 값을 그대로 쓴다(새로 생성 금지)**. SKAN 수신은 business-api 가 Apple 공개키로 서명을 검증하는 것이라 별도 비밀 키가 없다(링크 LLD §4.2) |
...
**`LINK_IP_SALT` 는 ~~전환~~ 불변식이다.** 현 `invite_link_clicks.ip_hash` 는 운영 `LINK_IP_SALT`(`application-prod.yml`)로 이미 계산돼 있어서, ~~Vercel 에~~(→ A23: data-api 가 그대로 갖는다, 링크 정책 L03) **새 salt 를 생성하면 ~~백필한~~ 기존 클릭의 해시와 설치 시 ~~새 서버가~~ 계산한 해시가 전부 어긋나** IP·OS 가 같은 정상 설치도 3시간 매치 창 내내 `matched:false` 가 된다. 그래서 ⓐ ~~이관 시 **기존 값을 그대로 옮기고**~~ → 값은 현행 그대로 쓰고 ⓑ 미매치 클릭이 남아 있는 동안(= 최소 매치 창 3시간)은 회전하지 않으며 ⓒ 나중에 회전한다면 **구·신 salt 를 모두 계산해 조회하는 기간**을 두고 그 기간이 끝난 뒤 구 salt 를 폐기한다.
...
레포 (A17 · 1695 개정)
  ...
  (OneOrThree/mmp-custom — A23 로 폐기: 링크는 services/business-api·data-api 안, 별도 레포 없음)
...
| ~~link~~ | — | ~~Vercel 로그·Analytics (Datadog 밖, 링크 장부 대가 ④)~~ → A23: business-api·data-api 의 APM·로그에 포함(별도 행 없음) |
...
- 서비스 간 = compose 내부 DNS(`http://data-api:8081`). 사설망이라 TLS 없음(같은 호스트). ~~Vercel → notification 만 인터넷 경유(HTTPS + 토큰).~~ → A23: 인터넷을 경유하는 서비스 간 호출은 없다(콘솔은 Business API 안).
- ~~link 서버 → 코어 호출 없음(단방향) → 코어에 링크용 인바운드 없음.~~ → A23(2026-09-13): 링크 서버 없음 — 링크 인바운드는 nginx → business-api(`/l/**` · `/link/**` · `/.well-known/**` · `/console/**`)뿐이고 data-api 는 `/internal/*` 만 받는다(공인 노출 0).
- ~~앱 → link 는 `link.oneorthree.world` 직접(Cloudflare 밖, Vercel). **단 이관 기간 동안** 기존 배포본이 부르는 `api.oneorthree.world/l/match`·`/l/referrer` 를 nginx 가 Vercel 로 프록시한다 — 제거 조건: 새 호스트를 쓰는 앱 버전이 최소 지원 버전이 되고, 구 경로 호출이 7일 연속 0 일 때(Datadog 로 확인).~~ → A23: 앱의 링크 호출(`/l/match` · `/l/referrer` · `/l/resolve`)도 Cloudflare → nginx → business-api 다(`link.oneorthree.world` 가 prod nginx 로 들어오는지는 정본이 **미문서화 확인 항목**으로 둔다 — 링크 LLD §11 확인 목록 4). 별도 프록시·제거 조건이 없다(링크 HLD §1·§7).
...
3. ~~Vercel Pro vs Cloudflare(링크 장부 미결) — 알림 콘솔 편집자 수가 변수.~~ → A23 로 소멸(Vercel 없음).
4. ~~**GROMO-1660 재정의** — 티켓 본문 범위 4("클릭 적재를 MQ 비동기 경로로")가 링크 v3(Vercel·Neon 직접 적재, 단방향)와 어긋난다.~~ → **A23(2026-09-13)으로 닫힘**: 1660 의 분리 범위 자체를 폐기했다(`mmp-custom` 저장소 폐기). 링크는 business-api(공개 표면·콘솔)·data-api(원장·트랜잭션·집계)이고, 클릭 적재는 MQ 가 아니라 Business → Data `/internal/links/{slug}/visits` 의 같은 DB 트랜잭션이다(링크 HLD §3.1). (배포 정의 소유자는 A17 로 확정 — `oneorthree/server` 안, 레포 간 dispatch 없음)
5. **A18(보류)** — Kafka 발행 실패 정책(유실 수용 / 발행 실패 테이블 재발행 / HTTP 자동 폴백).

## 9. 08-12 AS-IS 대비 변경 요약

- 배포 단위 1(`app`) → **컨테이너 prod 6 / dev 7**(JVM 서비스 3 + nginx · kafka · datadog-agent, dev 는 + Postgres `db`) ~~+ 외부 1(link/Vercel)~~(→ A23). 레포 1 → ~~4~~ 3 (A17: server · app · ~~link~~ · docs — link 는 A23 으로 서버에 흡수).
- DB 1 database → 같은 인스턴스 2 database ~~+ Neon~~(→ A23: 링크 원장은 `gromo` 안).
...
GROMO-1659의 Data export는 settings·device·delivery·user·participation 다섯 자원의 건수·체크섬을 같은 RR 스냅샷에서 생성한다. 실제 Noti bootJar의 정규화·키 유도 함수와 Data export 직렬화를 CI에서 함께 실행한다(A22 ㋳·㋶). ~~Link 이관에는 필드별 표시 version과 LEGACY 귀속을 포함하며,~~(→ A23: 링크 이관 없음) 서비스별 시크릿·이미지 digest·nginx 적용 준비는 `docs/prd/fishcat/server-separation/deployment.md`를 따른다(A22 ㋷·㋸).

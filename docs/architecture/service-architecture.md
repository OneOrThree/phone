...
| **코어 이력 의존 판정 잡** — 비활성 복귀(`InactiveReturnNotificationService`, `users.last_active_at` D+3/7/14) · 리그 리인게이지먼트(`LeagueReengagementNotificationService`, 주간 집중 통계·스트릭·진행 중 세션) | **Data API 에 남긴다** — 판정만 코어에서 하고 알림 서버엔 **발송 명령**만 보낸다. **그 명령은 동기 호출로 끝내지 않고 판정 결과를 결정적 키(`userId` + 단계 + 판정일)와 함께 outbox 에 기록해 `noti_delivered_at` relay 가 재전달한다** — `InactiveReturnNotificationService` 는 `last_active_at` 의 **정확히 그 하루만** 조회하고 주석에도 「그날 배치가 못 돌면 그 창의 유저는 빠진다」고 적혀 있어, 알림 서버가 잠깐 죽어 HTTP 재시도를 넘기면 그 단계 알림은 **영구히 복구되지 않는다** | 그대로 |

...
| A19 | **Target-2 공유 저장소(Redis) 규칙** — 단방향 규칙은 서비스 호출만 다루므로 공유 저장소에 같은 철학을 명시한다: ① 키는 소유자 네임스페이스로 나눈다(`league:*`·`presence:*` = Data API, `noti:*` = 알림 서버, `auth:rt:*` = Business API) ② **쓰기는 소유자만**, 다른 서비스는 읽기만(Business API 의 `ZREVRANK` 등) ③ Redis 는 진실원천이 아니라 사본 — 소유자가 DB/로그에서 재구축할 수 있어야 한다. | PR #731 리뷰 지적: A10 은 Postgres 가 DB 간 조인을 막아 경계가 물리적으로 강제되는데 Redis 공유엔 대응 규칙이 없었다 | 09-10 |

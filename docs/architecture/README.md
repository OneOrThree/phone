...
**앱은 Business API 하나만 본다. 데이터는 Data API 만 만진다. 알림은 자기 데이터만 갖고, 코어를 부르지 않는다.** ~~알림과 링크는~~ → A23(2026-09-13): 링크·MMP 는 위성이 아니다 — 공개 표면·콘솔은 Business API, 원장·트랜잭션·집계는 Data API.
...
- 위성(알림~~ · 링크~~ → A23: 링크는 별도 위성이 아니라 Business·Data 안)은 코어를 부르지 않는다. 코어가 위성에 밀어준다(이벤트 · 발급 시 스냅샷 동봉). 예외는 알림 → Data API **조회 3종**(리컨실 · ack 수렴 · 발송 적격)뿐이다.
...
6. **밖에서 닿아야 하나** — 외부(앱·~~Vercel 콘솔~~ → 팀원 브라우저의 `/console/**`(A23)·Apple SKAN 포스트백·웹훅)가 부르는 경로가 있으면 `system-architecture.md` §2.1 공인 노출면 표에 행을 추가하고 nginx 라우팅·인증 방식을 적는다. 없으면 "노출 0"을 명시한다(data-api 처럼).
...
- 초기 user·participation 투영까지 다섯 자원을 대조하고 같은 원본 재적재·탈퇴 fence를 검사한다(A22 ㋶). ~~Link 이관은 LEGACY 귀속과 필드별 표시 version을 복원한다(A22 ㋷·㋸).~~ → A23: 링크 이관 없음.
...
~~Link/MMP의 실제 저장소는 [OneOrThree/mmp-custom](https://github.com/OneOrThree/mmp-custom)이다(A17).~~ → A23(2026-09-13): 그 저장소는 폐기됐다 — 링크·MMP 는 `server/business-api`(공개 표면·콘솔)·`server/data-api`(원장) 안이다([`docs/prd/fishcat/link-attribution/`](../prd/fishcat/link-attribution/README.md)). 알림은 현재 `server/notification/`에 유지하며 최종 JVM 레포 경로 전환은 1695와 맞춘다. 기존 파일 미리보기 캐시의 Target-1 예외와 ACL은 A22 ㋺를 따른다.
...
- ~~운영 Business env 생성 시 `LINK_PROXY_SECRET` 누락·빈 값 차단을 검사한다(A22 ㋯).~~ → A23: `LINK_PROXY_SECRET` 불필요(링크 HLD §2) — #745 잔여 설정은 링크 구현 PR 이 걷어낸다(링크 LLD §9).

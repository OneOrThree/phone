...
판정 기준: 새 14종 화면 계약([policy.md B15](../../prd/bff-screens/policy.md))과 도메인 LLD 로 같은 기능이 제공되면 **대체 있음**, 2.0 설계 어디에도 없으면 **2.0 에 없음**, 제품 결정이 필요하면 **판단 불가**. 근거는 실제 코드 경로(`@RequestMapping`)와 도메인 LLD 의 `파일:줄` 인용이며 주석은 근거로 쓰지 않았다.
...
| `group/GroupController.java` | `/groups*` | 17 | 대체 있음 | [island-membership LLD §1](../../prd/island-membership/low-level-design.md) — `GroupService.createGroup`(:12)↔`POST /islands`, `joinGroup`(:14)↔`POST /islands/{islandId}/memberships`, `getGroupOverview`(:21)↔`GET /islands/{islandId}`(PublicIslandSummary); [island-management LLD §1](../../prd/island-management/low-level-design.md) — `GroupMemberService.transferOwner`(:12)↔`POST .../host-transfer`, `kickMember`(:13)↔`DELETE .../members/{userId}`; [island-board LLD](../../prd/island-board/low-level-design.md) — notices 5종↔공지 CRUD | `POST /groups/{groupId}/code`(초대코드 발급)는 이미 legacy에서 `@Deprecated generateUniqueCode()`(island-membership LLD:15)라 대체가 아니라 폐기 확인. `GET /groups/search`↔`GET /islands?q=`([policy B19](../../prd/bff-screens/policy.md)). 설정(`/groups/{groupId}/settings`)의 필드별 1:1은 미검증 — 도메인 PR에서 재확인 필요 |
| `invitelink/InviteLinkController.java` | `/groups/*/invite-link`,`/invite-links/claim` | 2 | 대체 있음 | [link-attribution LLD](../../prd/link-attribution/low-level-design.md)(A23) · [island-membership LLD §3.10-3.11](../../prd/island-membership/low-level-design.md) `POST /invitations/resolve`·`POST /islands/{islandId}/invitations` — `InviteLinkService.issue`(island-membership:22) 재사용 근거 명시 | — |
| `item/EquipmentController.java` | `/equipment*` | 3 | 대체 있음 | [island-appearance LLD §1.2](../../prd/island-appearance/low-level-design.md) `PATCH /me/appearance`(:28) — clothes/decor/hull/position 필드로 equip·unequip 통합 | `GET /equipment`(장착 조회)는 `GET /me/inventory` 응답의 `equipped` 필드(§1.1, :16)로 흡수 |
| `item/InventoryController.java` | `/inventory*` | 2 | 대체 있음 | [island-appearance LLD §1.1](../../prd/island-appearance/low-level-design.md) `GET /me/inventory`(:13) | `POST /inventory/grant`(관리자 지급)는 새 4계약에 없음 — 퀘스트 클레임·상점 구매 등 개별 도메인 TX가 보유품을 직접 만드는 구조로 흡수된 것으로 보이나 명시 확인은 안 됨 |
...
| --- | --- | --- |
| `focus/service/FocusService.java` | `FocusController` | [focus-rest-session HLD](../../prd/focus-rest-session/high-level-design.md):11 (`start`의 users FOR UPDATE),:21 |
| `friend/service/FriendService.java` | `FriendController`,`PinController` | [account LLD](../../prd/account/low-level-design.md):740,:1091 — 탈퇴 시 `FriendService.detachWithdrawnUser`로 친구 관계 soft delete |
| `group/service/GroupMemberService.java` | `GroupController`,`GroupBetController` 등 | [island-management LLD](../../prd/island-management/low-level-design.md):12-13(`transferOwner`·`kickMember`) · [account LLD](../../prd/account/low-level-design.md):582,:1080(탈퇴 `detachWithdrawnUser`) · [link-attribution LLD](../../prd/link-attribution/low-level-design.md):689(호출부 보존) |
| `stats/service/StatsService.java` | `StatsController` | [account LLD](../../prd/account/low-level-design.md):1107(`anonymizeWithdrawnUser`) · [island-records LLD](../../prd/island-records/low-level-design.md):264 |
| `screentime/service/ScreenTimeService.java` | `ScreenTimeController` | [account LLD](../../prd/account/low-level-design.md):1108(`anonymizeWithdrawnUser`) · [island-records LLD](../../prd/island-records/low-level-design.md):235,:265(`saveScreenTimeTx` 재사용) |
| `user/service/UserService.java` | `UserController` | [account LLD](../../prd/account/low-level-design.md):86(`changeNickname`),:92(`setupProfile`/`updateProfile`),:1077,:529(`erasePersonalData`) · [link-attribution LLD](../../prd/link-attribution/low-level-design.md):369 |
| `ClientIpResolver` | `AnalyticsController` 등 IP 로깅 사용처 | [link-attribution HLD](../../prd/link-attribution/high-level-design.md):161,[LLD](../../prd/link-attribution/low-level-design.md):544 — 같은 규칙을 business-api 로 그대로 옮김 |
| `invitelink/service/InviteLinkService.java` | `InviteLinkController`,`GroupController`(POST code) | [island-membership LLD](../../prd/island-membership/low-level-design.md):22(`issue` 재사용) |
| `invitelink/service/InviteLinkMatchService.java` | `InviteLinkController` | [account LLD](../../prd/account/low-level-design.md):1105(claim 무잠금 findBySlug 로직) |
| `notification/` 의 서비스 전부(`FriendNotificationService` 등) | `NotificationBatchController`,`GroupNotificationBatchController`,`NotificationTestController` | [account LLD](../../prd/account/low-level-design.md):540(`NotificationOutboxProducer`),:846(`FriendNotificationService`) — 탈퇴 TX가 모든 알림 writer 와 직렬화해야 해서 전부 살아있어야 함 |
...
| `group/service/ChallengeResultAckService.java` | `ChallengeResultAckController` | [island-quests LLD](../../prd/island-quests/low-level-design.md):209 · [policy.md Q09](../../prd/island-quests/policy.md):17(`claimDisplay`는 결과 모달 선점이며 지급 수단이 아님이라는 의미로 재사용) · [account LLD](../../prd/account/low-level-design.md):870 |

추가로 코드 확인 중 발견한, 지시된 최소 목록 밖의 재사용:

| 클래스 | 근거 |
| --- | --- |
| `group/service/GroupService.java` | [island-membership LLD](../../prd/island-membership/low-level-design.md):12,14,21(`createGroup`·`joinGroup`·`getGroupOverview` 로직 선례로 재사용) |
| `currency/service/InGameCurrencyService.java` | [island-shop LLD §"기존 재화 계측 의무"](../../prd/island-shop/low-level-design.md)(`spendCurrency` 재사용, 서버 MP 계측 의무 이행 주체) |
| `group/service/GroupAnnouncementService.java`,`group/service/GroupQueryService.java` | [api-platform policy.md](../../prd/api-platform/policy.md):81(신규 섬 조회·관리·공지 어댑터가 이 두 서비스의 403 사유를 그대로 반환) |

...
작업 머신의 임시 파일에 의존하지 않도록 원본 로그 55개와 집계용 JUnit 결과 7세트를 이 저장소에 보관했다.
스택 검증의 로그·이미지 로그 경로와 원격 manifest 증거의 로그 경로는 evidence 디렉터리 기준 상대 경로다.
로그는 gzip 압축 전후 해시를 기록했으며, JUnit 결과는 `system-out`·`system-err`만 제외한 변환본임을 표시했다.
[증거 재확인 안내](evidence/README.md)의 명령으로 다른 checkout에서도 해시와 테스트 집계를 확인할 수 있다.

...
신고·사용자 차단의 확정 기준은 [신고센터 정책](../character-report/policy.md)의 RP-일반신고·RP-표시차단을 따른다. 차단자가 보는 상대 공지·댓글은 숨기며 사용자 작성 공지는 방장 작성이어도 예외가 아니다. 차단만으로 상대의 작성·수정 요청을 거절하지 않는다. 기존 작성·수정 권한과 별도로 적용할 정책이며 구현 완료를 의미하지 않는다.

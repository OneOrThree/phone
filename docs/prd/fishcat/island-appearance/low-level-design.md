...
A03은 GROMO-1909에서 **모든 선체 공통**으로 확정됐다 — 선체별 호환표를 만들지 않으므로 hull 변경 시 decor 자동 해제 분기 자체가 필요 없다. A02는 여전히 미결이고 최초 활성화 전에 정한다. 어느 쪽이든 이후 기존 ID의 의미를 몰래 바꾸지 않는다.
...
|하위 선체 재착용·decor 호환·해제 위치|하위 재착용은 **A02 결정 후** 검증. decor 호환은 A03이 「모든 선체 공통」으로 확정돼 호환 거절·자동 해제 분기가 없다는 것을 검증한다. A04는 일반 PATCH 생략/유지 회귀 검사|

이번문서작업은빌드/DB테스트를실행하지않았다.

**구현 활성화 보류는 해소됐다(GROMO-1909).** 개인 기존 자산 승계(A05 = 새 모델 분리)와 공동 권한(A01 = 방장만)이 값으로 확정됐다. 남은 제약은 하나다 — **A02 가 정해질 때까지 `PATCH /me/appearance` 의 hull 재착용 분기를 활성화하지 않는다.** 나머지 계약은 착수 가능하다.

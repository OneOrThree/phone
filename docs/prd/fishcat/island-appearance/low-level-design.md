...
배열은 항상 존재한다. `clothes`/`decor`는 실제 보유 목록이므로 보유품이 없으면 `[]`다. **`hulls`는 배 종류 폐지(2026-09-16 B23·B25, GROMO-1851) 뒤 항상 `["raft"]`** — 유료 선체 상품이 없다. 위 원본 예시의 `sailboat` 는 폐지 전 형태다. raft를 위해 owned product/grant/order를 만들지 않고 inventoryVersion도 기본 표현만으로 증가시키지 않는다. raft 표시는 유료품 무상 지급이나 기존 자산 승계를 뜻하지 않는다.

목록에 표시된다는 사실은 현재 적용을 허가한다는 뜻이 아니다. ~~상위 선체 구매 후 raft/하위 선체 재착용은 기존 A02 미결 gate를 그대로 따른다.~~ A02 는 배 종류 폐지로 대상이 소멸했다. 이 설명은 원본 표시 예시와 빈 목록 문구의 모순을 닫은 것이며 새로운 지급 정책을 승인한 것이 아니다.
...
|hull|현재 값 유지|422|`raft` 만 허용 — 배 종류 폐지(B23·B25, GROMO-1851)로 유료 선체·재착용 규칙(A02)·호환 조건(A03) 모두 대상 소멸. `raft` 외 값은 미등록 상품 규칙(404 PRODUCT_NOT_FOUND)을 따른다|
...
A03은 GROMO-1909에서 **모든 선체 공통**으로 확정됐다 — 선체별 호환표를 만들지 않으므로 hull 변경 시 decor 자동 해제 분기 자체가 필요 없다. A02·A03 은 이어진 배 종류 폐지(2026-09-16 B23·B25, GROMO-1851)로 대상이 소멸했다. 어느 쪽이든 이후 기존 ID의 의미를 몰래 바꾸지 않는다.
...
|시나리오|기대증거|
|---|---|
|옷/소품 미보유(유료 선체는 존재하지 않음)|hulls=[raft], clothes/decor=[]; raft 소유/grant/order 생성 없음|
...
|hull 값·decor 호환·해제 위치|hull 은 `raft` 만 통과하고 그 외는 미등록 상품 규칙으로 거절되는지 검증(배 종류 폐지 — A02 대상 소멸). decor 호환은 A03이 「모든 선체 공통」으로 확정돼 호환 거절·자동 해제 분기가 없다는 것을 검증한다. A04는 일반 PATCH 생략/유지 회귀 검사|

이번문서작업은빌드/DB테스트를실행하지않았다.

**구현 활성화 보류는 전부 해소됐다.** 개인 기존 자산 승계(A05 = 새 모델 분리)와 공동 권한(A01 = 방장만)은 GROMO-1909 로, 남아 있던 A02(hull 재착용 분기)는 배 종류 폐지(2026-09-16 B23·B25, GROMO-1851)로 대상이 소멸했다. 네 계약 모두 착수 가능하다.

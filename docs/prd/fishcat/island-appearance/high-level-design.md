...
시설 완공으로 건물 집합과 기본 테마가 추가될 때도 시설 TX가 appearance.version을 올리고 전체 외양 outbox를
함께 저장한다. 시설 일반 버전만 올려 외양 변경을 숨기지 않는다. 철거 기능을 신설하는 계약은 아니다.

구매→테마적용→음원재생을 한 요청으로 묶지 않는다. 한 단계 실패가 이미 완료된 구매를 암묵 취소하지 않는다.
공동적용권한은1761 권한행렬과 같은 정책provider에서 얻는다. GROMO-1909 가 그 행을 **방장만**으로
확정했고(권한 이름 `SHARED_APPEARANCE`), 판정 주체는 여전히 Data 다. 다른 TBD 권한 행을 이 결정으로
함께 채우지 않는다 — 공동 구매 권한은 그대로 미정이다.
...
회원탈퇴·강퇴·방장이양, 시설 완공, 향후 소유회수와 같은 동시writer는 공통잠금규율에 합류해야 한다.
판매 catalog비활성/가격 변경은 기존 소유·착용을 변경하는 writer가 아니다. 보유 자산의 종류·대상·호환은
productId별 불변 정의로 복원하므로 퇴역한 기존 착용품 때문에 무관한 외양 PATCH가 실패하지 않는다.

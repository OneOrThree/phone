...
GROMO-1782 · 2026-09-12 · GROMO-1909 · 2026-09-17 · 상태: **설계 확정 — 미결 없음**.
공동 테마 적용 권한(A01)·소품 호환(A03)·기존 자산 승계(A05)가 값으로 확정됐고, 마지막 미결이던 A02(하위 선체 재착용)는
**배 종류 폐지(2026-09-16 B23·B25, GROMO-1851)로 대상이 소멸**했다 — 선체는 기본 뗏목 `raft` 하나다. 구현 착수가 가능하다. [정책의 결정표](policy.md) 참조.
...
구 슬롯의 이름 변경으로 취급하지 않는다. GROMO-1909 가 **승계하지 않음(새 모델 분리)**으로 확정했다(A05).
그래도 **원래 아이템/캐릭터 장비를 삭제·변환하지 않는다** — 「승계하지 않는다」는 「지운다」가 아니다.
레거시 표면의 폐기는 [레거시 v1 폐기 계획](../../../engineering/legacy-v1-retirement/README.md)이 받는다.
...
네 계약/소유 매핑/배치/원자외양 변경/이벤트 설계를 문서화한다. 공동 적용 권한·소품 호환·기존 자산 승계는
값으로 확정됐고(GROMO-1909), 하위선체 재착용(A02)은 배 종류 폐지로 대상이 소멸해 미결이 없다. 문서 확정이 서버구현 완료를 뜻하지는 않는다.

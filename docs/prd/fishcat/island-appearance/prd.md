...
GROMO-1782 · 2026-09-12 · GROMO-1909 · 2026-09-17 · 상태: **설계 확정 — 단 A02(하위 선체 재착용)만 미결**.
공동 테마 적용 권한(A01)·소품 호환(A03)·기존 자산 승계(A05)가 값으로 확정됐다. 구현 착수가 가능하고,
`PATCH /me/appearance` 의 **hull 재착용 분기만** A02 결정까지 비활성으로 둔다. [정책의 결정표](policy.md) 참조.
...
네 계약/소유 매핑/계보/배치/원자외양 변경/이벤트 설계를 문서화한다. 공동 적용 권한·소품 호환·기존 자산 승계는
값으로 확정됐고(GROMO-1909), 남은 미결은 하위선체 재착용(A02) 하나다. 문서 확정이 서버구현 완료를 뜻하지는 않는다.

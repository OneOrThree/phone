...
|A03|선체별 소품 호환|**모든 선체 공통**|호환표 자체가 불필요해진다. 「미승인 호환표」의 소유 문서가 레포 어디에도 지정돼 있지 않았고, 아이템 마스터 데이터도 레포에 없어(마이그레이션 INSERT 0건·시드 없음·하드코딩 없음) 표를 만들 재료가 없다|
...
|---|---|---|---|
|A02|상위선체 구매 뒤 하위선체 재착용|보유한 선체 자유선택 또는 상위만|목업 예시를 최종정책으로 확정하지 않음. **`PATCH /me/appearance` 의 hull 재착용 분기를 활성화하지 않는다**|
...
「공동 상품 구매」 행)은 여전히 TBD 이고, A01 의 결정을 그 행으로 옮기지 않는다** — 두 권한은 별개라는
위 문장이 그대로 유효하다.

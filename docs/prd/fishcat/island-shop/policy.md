...
|S02|~~공동 구매 권한: 방장만/주민 허용/설정 가능 중 어느 정책인지~~ → **확정(GROMO-2000)**: 공동 구매(상점 상품·축음기 음원)는 **활성 주민 누구나**다 — 기획 정본 「주민 누구나 섬 물고기로 상점 상품·축음기 음원을 구매할 수 있다」. 권한 이름 `SHARED_PURCHASE` 유지|~~2026-09-18 D2 의 섬 설정 토글~~ 폐기 — 토글 기본값·설정 API 잔여도 함께 소멸. [권한 행렬](../island-management/permissions.md)·건설 C13 과 같은 값이되, **건설만 방장으로 갈렸다**|잔여 없음. 게스트 계정 제한(GROMO-1992)은 다른 축이다|

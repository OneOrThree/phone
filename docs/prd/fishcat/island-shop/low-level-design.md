...
**구매 내역은 섬 귀속이다(2026-09-19 재영님 결정 SH-BG18).** 모든 주문은 한 섬에 속한다. `scope=personal`은
경로 섬 안에서 본인이 요청한 주문만, `scope=shared`는 경로 섬의 공동 주문만 조회한다. 두 cursor 모두 요청자+해당 섬에
묶인다. 여러 섬의 개인 구매를 합친 전체 이력은 제공하지 않는다(BG18 해소). ~~`scope=personal`은 섬에서 무엇을 샀든
본인 개인 주문을 조회~~ — 이 종전 문장과 충돌해 위로 바꿨다.

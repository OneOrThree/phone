...
| contract (HLD §7 5단계) | 그다음 번호 | 기존 ACTIVE 초대 링크 **전부** `REVOKED` 보정(재가입 이력 증명 불가, 아래) → unique 교체 → rename 2개 → V52 링크 테이블 5개 DROP. 같은 이미지에서 폐기·재발급 코드를 켠다 | **불가 — roll-forward 전용.** 장애는 앞으로 고치는 핫픽스로 대응하고, 최후 수단은 contract 직전 RDS 스냅샷 복원이다(그 뒤 쓰기는 잃는다) |
...
**contract ① 이 기존 초대 링크를 전부 폐기하는 이유.** 현재 상태(발급자 `is_left = false`·그룹 활성)만 보는 보정은 **이탈 전에 공유된 slug** 를 남긴다 — 발급자가 링크를 발급한 뒤 이탈했다가 contract 전에 재가입하면 `GroupMember.rejoin()` 이 기존 행을 되살려 `is_left = false` 로 돌리고, expand 기간의 발급은 같은 활성 행을 재사용하므로 그 slug 가 contract 뒤에도 비공개 그룹 가입 자격으로 남는다. 이 순서를 증명할 근거가 없다: `group_members` 에 재가입 시각 컬럼이 없고(`created_at` 은 최초 가입, `updated_at` 은 모든 갱신에 바뀜), V52 의 `membership_epoch` 는 전이마다 오르지만 **링크에 발급 당시 epoch 가 기록돼 있지 않고** V52 이전의 전이는 남아 있지 않다. 그래서 이력을 증명할 수 없는 기존 INVITE 링크를 모두 `REVOKED`(`BACKFILL_UNPROVABLE`)로 확정한다. **영향**: contract 시점에 이미 공유된 초대 링크는 모두 만료된다 — 랜딩·매치·비공개 가입은 만료로, claim 은 no-op 으로 끝나고, 수신자는 [그룹 획득 HLD §2](../../gromo/group/features/01-acquisition/high-level-design.md) 의 기존 「새 링크 재요청 안내」를 본다. 발급자가 다시 발급하면 새 slug 가 나온다. CAMPAIGN 행은 contract 전에 생기지 않으므로(HLD §7 6단계 플래그) 대상이 없다.
...
- 기존 행은 전부 `type=INVITE`·`status=ACTIVE` 가 된다. `REVOKED` 보정은 contract ① 에서 **기존 ACTIVE 초대 링크 전부**에 한다(§1.1 — 재가입 이력을 증명할 수 없다). expand 동안엔 V21 전체 unique 가 멱등 발급의 최후 방어선으로 그대로 남는다.
- `revoke_reason`: `MEMBER_LEFT` · `MEMBER_KICKED` · `ACCOUNT_WITHDRAWN` · `GROUP_CLOSED`(INVITE, 그룹 획득 문서의 전이) · `CONSOLE`(CAMPAIGN) · `BACKFILL_UNPROVABLE`(contract ① 보정).
- 폐기 트리거·잠금 순서·재발급은 [그룹 획득 LLD §2.1](../../gromo/group/features/01-acquisition/low-level-design.md) 이 정본이다. 이 마이그레이션은 그 규칙을 담을 자리만 만들고, 그 코드는 contract 이미지에서 켠다(§1.1).

### 1.4 `link_clicks` (expand 동안 물리 이름 `invite_link_clicks`)

expand 에서 컬럼 셋과 인덱스 여섯(기간 집계 셋 · 랜딩 클릭 저장 상한 하나 · claim 사용자 하나 · 미claim 선점 하나)을 더하고 contract 에서 rename 한다. 기존 컬럼·인덱스(`idx_invite_clicks_match` · `idx_invite_clicks_link` · `idx_invite_clicks_device`)는 그대로다.
...
ALTER TABLE public.invite_link_clicks
    ...
    ADD COLUMN match_rejected      boolean NOT NULL DEFAULT false;  -- 후보를 소진했지만 활성 판정에 실패해 귀속하지 않은 매치(§2.3). 설치 집계에서 뺀다(§3.2)
...
-- slug claim 의 미claim 클릭 선점(§2.5). idx_invite_clicks_link_matched 는 이미 claim 된 matched 행까지 담아, 오래 운영된 링크에서
-- 후보가 없을 때 유저 행 배타 락을 쥔 채 링크의 matched 이력 전체를 거른다
CREATE INDEX idx_invite_clicks_unclaimed ON public.invite_link_clicks (link_id, matched_at DESC) WHERE matched AND claimed_at IS NULL;
-- 캠페인 slug claim 의 기기 조건(matched_device_id = :deviceId)은 기존 idx_invite_clicks_device (matched_device_id, matched_at) WHERE matched 가
-- 기기로 먼저 좁혀(기기당 매치 몇 건) 충분하므로 별도 인덱스를 두지 않는다
```
...
CREATE TABLE public.install_referrers (
    ...
    -- ck_ir_click(click_id 가 있으면 link_id 필수)은 두지 않는다: META·GOOGLE referrer 도 같은 설치의 fingerprint 클릭을 click_id 로 가리킨다(§2.4-3)
...
**잠금 — 폐기·보관과 직렬화한다.** 활성 판정 전에 **(INVITE) groupId 순 `groups` · 발급자 `group_members` → `links`**, **(CAMPAIGN) `campaigns` → `links`** 를 **공유 락(`FOR SHARE`)** 으로 잡고, 같은 트랜잭션에서 활성 판정·뷰 구성·클릭 INSERT 를 한다. 잠금 없이 판정하면 판정 직후 캠페인 보관·링크 폐기·발급자 이탈이 커밋되고 방문이 뒤이어 클릭 행과 `state=ACTIVE` 뷰를 남겨, 폐기된 링크의 클릭이 늘고 방문자에게 옛 목적지·초대 정보가 보인다. 쓰기 경로(폐기·보관·발급자 이탈)는 같은 행을 배타 락으로 잡으므로 방문과 순서가 맞춰지고, 방문끼리는 공유 락이라 서로 막지 않는다. `recordClick=false` 와 기록하지 않는 방문(아래 봇·유료 캠페인 iOS)도 같은 공유 락 아래에서 뷰를 만든다 — 폐기 직후 옛 목적지를 내보내지 않는다. 방문은 user 행을 잠그지 않고 매치(§2.3)·claim(§2.5)·referrer 저장(§2.4)과 같은 (group·membership | campaign) → link 방향만 쥐므로 교착이 없다.
...
**같은 링크·IP 해시·OS 의 클릭은 매치 여부와 무관하게 매치 창(`LINK_MATCH_WINDOW_HOURS`) 안에서 20건까지만 새로 만든다.** 21번째 방문부터는 새 행 없이 그중 가장 최근 클릭의 id 를 `clickId` 로 돌려준다(조회는 `idx_invite_clicks_recent`, §1.4). 미매치 클릭만 세면 방문과 공개 `/l/match` 를 번갈아 부르며 매번 다른 `deviceId` 로 후보를 소진해 건수를 20 아래로 되돌리고 계속 INSERT 할 수 있다 — 전체 수는 매치로 줄지 않는다. `User-Agent` 는 호출자가 정상 브라우저 값으로 지어낼 수 있어 봇 판별만으로는 저장량을 막지 못한다 — 현행 `record` 에는 봇 판별만 있고 상한이 없다. 1건으로 합치지 않는 이유는 같은 와이파이(같은 공인 IP)에서 같은 OS 로 여러 사람이 같은 링크를 누를 때 각자 설치를 매치할 후보가 필요해서다. 건수 확인과 INSERT 는 **같은 키의 advisory 락 아래 직렬화**한다(`pg_advisory_xact_lock(hashtext(link_id::text || ip_hash || os))`) — 잠금 없이 세면 병렬 방문이 같은 20 미만 값을 읽고 모두 INSERT 해 상한(정책 L25·PRD 수용 조건)을 넘긴다. 이 락은 §2.2 의 공유 락과 다른 축이라 폐기·claim 경로와 순서가 얽히지 않는다. business-api 의 IP 한도(§4.3)가 앞에서 한 번 더 막는다.

### 2.3 fingerprint 매치

현행 `InviteLinkMatchService.match` 를 쓰되(`ip_hash + os + LINK_MATCH_WINDOW_HOURS`, `PESSIMISTIC_WRITE` + `SKIP LOCKED`, 기존 매치 재반환) **재반환 조건만 설치 단위로 좁힌다**(이 절 끝). **활성 조건을 후보 조회에 넣지 않는다.** 현행처럼 창 안의 최신 미매치 클릭을 후보로 고정하고, 소진한 뒤 §2.1 활성 판정을 하고, 실패면 그 클릭에 **`match_rejected = true`** 를 적고 `matched:false` 로 끝낸다(소진은 재매치 방어라 그대로 두고, 귀속하지 않았다는 사실만 남긴다 — 집계는 §3.2 가 이 행을 뺀다) — 더 오래된 다른 후보로 내려가지 않는다. 활성 조건으로 먼저 거르면 같은 IP·OS 에서 더 최근에 폐기된 클릭 뒤의 과거 클릭이 이 기기에 엉뚱하게 귀속된다. 기존 매치 재반환도 같은 활성 판정을 거친다. 현행 재반환은 매치 창 안의 매치를 기기로만 찾는다(`findFirstByMatchedDeviceIdAndMatchedAtAfterOrderByMatchedAtDesc(deviceId, cutoff)`). 그래서 창 밖 재설치는 새 후보를 매치하지만, **창 안에서 재설치**하면 Auto Backup 이 복원한 같은 `deviceId` 로 과거 매치가 재반환돼 새 링크의 설치가 기록되지 않는다(referrer 읽기에 실패해 fingerprint 로 내려온 경우). 그러므로 매치 때 `matched_install_id` 에 요청의 `installId` 를 기록하고, **재반환은 `matched_device_id = deviceId` · `matched_install_id = installId` · 매치 창 안일 때만** 한다. 요청에 `installId` 가 없으면(구 앱) 현행 규칙(기기·창)을 그대로 쓴다. 응답에 `type` 과, CAMPAIGN 이면 `destination` 을 싣는다.

**잠금 순서 — 폐기·보관과 직렬화한다.** 현행은 클릭 행만 잠근 채 링크·캠페인·그룹의 활성 상태를 잠금 없이 읽는다. 그래서 매치가 ACTIVE 를 읽은 직후 콘솔의 캠페인 보관·링크 폐기나 발급자 이탈이 커밋되면, 매치가 그 뒤에 커밋해 폐기된 링크의 목적지를 `matched=true` 로 돌려주고 `installs.fingerprint` 에 설치를 남긴다. 그러므로 매치는 다음 순서로 한다.

1. 후보 클릭을 **잠금 없이** 조회해 대상 링크를 찾는다(현행 후보 조건 그대로 — 창 안 최신 미매치 클릭).
2. **(INVITE) groupId 순 group · 발급자 membership → link → click**, **(CAMPAIGN) campaign → link → click** 순으로 잠근다. 클릭은 `PESSIMISTIC_WRITE` + `SKIP LOCKED` 로 다시 잡는다.
3. 잠긴 뒤 **그 클릭이 여전히 미소진인지**와 §2.1 활성 조건을 재검증한다.
   - 다른 요청이 그 클릭을 먼저 잡았거나(`SKIP LOCKED` 로 빈 결과) 이미 소진됐으면 **다음 후보로 한 번만** 다시 시도하고, 그래도 없으면 `matched:false` 다.
   - 활성 조건 실패는 기존 규칙대로 그 클릭을 소진하고 `matched:false` 로 끝낸다 — 더 오래된 후보로 내려가지 않는다.
4. 기존 매치 재반환도 같은 순서(링크 쪽 행 → 클릭)로 잠근 뒤 활성을 본다.

`/l/match` 는 인증이 없어 user 행을 잠그지 않는다. 그래서 매치가 쥐는 잠금은 group·membership 또는 campaign → link → click 한 방향뿐이고, 이는 claim(§2.5: user → group·membership 또는 campaign → link → 대상 행)·referrer 저장(§2.4-5)·콘솔의 보관·폐기·발급(campaign → link)·발급자 이탈(그룹 획득 LLD §2.1 순서)의 뒤쪽 부분과 같은 방향이다. 서로 반대 순서로 기다리는 쌍이 없어 교착이 생기지 않는다. expand 단계의 INVITE 는 현행 활성 판정(`findActiveGroup`)만 쓰지만 잠금 순서는 같다.
...
3. `click_id` 를 아래 순서로 정한다(source 가 LINK·META·GOOGLE 일 때 — ORGANIC·UNKNOWN 은 `click_id=NULL`). 한 설치가 fingerprint 와 referrer 양쪽에 잡히지 않게 하려는 규칙이다.
   1. (LINK 만) 전달된 `clickId` 가 그 링크의 **미매치** 클릭이면 그 클릭을 `PESSIMISTIC_WRITE` 로 잠가 `matched=true`·`matched_at`·`matched_device_id`·`matched_install_id`·`app_instance_id` 를 채우고 `click_id` 로 둔다. 같은 클릭이 fingerprint 로 다른 기기에 다시 매치되지 않는다.
   2. 아니면(전달값 없음 · 이미 매치됨 · 다른 링크의 클릭) **LINK 는 같은 링크에서, META·GOOGLE 은 링크와 무관하게** 이 `deviceId` 로 매치된 클릭 중 **아래 둘 중 하나에 해당하는 것**이 있으면, 그중 `matched_at` 이 `COALESCE(install_begin_at, received_at)` 에 가장 가까운 클릭을 `click_id` 로 둔다. 이번 설치에서 레이스로 `/l/match` 가 먼저 성공한 경우다.
      ...
      **META·GOOGLE 은 첫 조건(`matched_install_id = installId`)만** 쓴다 — 링크가 없어 구 앱 NULL 매치가 같은 설치인지 링크로 좁힐 수 없다. 첫 실행에 referrer 읽기가 실패해 fingerprint 매치·slug claim 이 먼저 끝나고 다음 실행에 같은 설치의 META·GOOGLE referrer 가 오면, 이 규칙이 그 클릭을 `click_id` 로 가리켜 설치는 referrer 쪽(Meta 캠페인·GOOGLE 채널)에 한 번만 잡히고 fingerprint 쪽(그 클릭의 캠페인 링크)에서는 빠진다 — 결정적 신호(광고 referrer)가 확률 매치보다 우선이다(정책 L12). `link_id` 가 없는 행도 `click_id` 를 가질 수 있어 `ck_ir_click` 을 두지 않는다(§1.5). 클릭 행은 갱신하지 않으므로(가리키기만 한다) 클릭 잠금이 없다.
...
5. **저장은 access token 이 있는 요청만 받는다**(게스트 포함, §4). 트랜잭션 첫 조회로 `SELECT … FROM users WHERE id = :reporterUserId AND is_deleted = false FOR UPDATE` 를 잡는다 — 구현은 기존 `UserQueryService.getCallerForUpdate`(`UserRepository.findActiveByIdForUpdate`: `u.isDeleted = false` + `PESSIMISTIC_WRITE`)를 쓴다. 소프트 삭제 컬럼은 `users.is_deleted` 다(`V2__align_common_columns.sql`).
   ...
   - 같은 `install_key` 행이 있으면 1번대로 그 행을 돌려준다. 재전송은 상한에 세지 않는다. **응답은 최초 저장과 같은 잠금(user → campaign → link) 아래 현재 활성 상태로 다시 만든다** — 저장 뒤 폐기·보관됐으면 재시도 응답도 `matched:false` 이고 `destination`·`groupId` 를 싣지 않는다(저장된 행은 그대로 둔다). 그러지 않으면 앱이 응답을 못 받아 재시도할 때 폐기된 링크의 목적지가 되살아난다.
...
- **INVITE** — [그룹 획득 LLD §2.1](../../gromo/group/features/01-acquisition/low-level-design.md) 의 규칙 「초대 write 경로는 slug를 잠금 없이 한 번 읽어 대상 ID를 찾은 뒤 **관련 user UUID 오름차순 → groupId 오름차순의 group·membership → active link ID** 순으로 잠그고 모든 조건을 재검증한다」를 그대로 따른다 — 위 1·2단계가 그 순서다. 그 아래에서 §2.1 활성 조건(링크 `status` · 그룹 활성 · 발급자 활성 멤버)과 셀프 초대를 재검증한 뒤 UPDATE 한다. 폐기와 claim 중 먼저 잠근 쪽이 커밋되고, 다른 쪽은 커밋된 상태를 재검증한다 — 이탈이 먼저면 claim 이 no-op, claim 이 먼저면 그 claim 은 폐기 전 감사 이력으로 남는다(그룹 획득 HLD §2 와 같다).
...
| `GET /internal/links/{slug}` | `/l/resolve` | — | 200 `LinkView` · 404. **뷰 구성은 §2.2 방문과 같은 공유 락 아래**((INVITE) `groups` · 발급자 `group_members` → `links`, (CAMPAIGN) `campaigns` → `links`) — 활성 판정까지 같은 트랜잭션이라 판정 직후 커밋된 폐기·보관·발급자 이탈의 옛 목적지·초대 정보를 돌려주지 않는다. 클릭은 기록하지 않는다 |
...
| `GET /internal/campaigns?status=` | 콘솔 | — | 200 `{linkIssueEnabled, campaigns: [CampaignSummary]}` — `linkIssueEnabled` 는 data-api 의 `CAMPAIGN_LINK_ISSUE_ENABLED` 현재 값(§6.1). DB 를 읽지 않는 콘솔이 409 를 받아 보기 전에 발급 폼·안내를 고를 수 있어야 한다 |
...
| `POST /internal/campaigns/{id}/links` | 콘솔 | `{destination}` — `campaigns` 행 배타 락 아래 ACTIVE 확인 → link INSERT(§2.5) | 201 `{slug, url, playStoreUrl, destination}`(`playStoreUrl` 은 `slug` 만 심은 콘솔용이고 캠페인 `platform` 이 `IOS` 이거나 `channel` 이 `GOOGLE` 이면 `null`, §4.4) · 409 `CAMPAIGN_ARCHIVED` · 409 `CAMPAIGN_LINK_ISSUE_DISABLED`(`CAMPAIGN_LINK_ISSUE_ENABLED=false`, §6.1) · 422 `DESTINATION_NOT_ALLOWED` |
...
| `installs.fingerprint` | `link_clicks.matched_at` 이 기간 안 이고 **`match_rejected = false`**(폐기 링크 후보를 소진만 한 행은 귀속이 아니다, §2.3) 이고 그 클릭을 가리키는 `install_referrers.click_id` 가 없음 — **이때 「가리킴」은 `installs.referrer`·`channels.*` 로 **실제 집계되는 행**만 센다(source LINK·META·**GOOGLE**, `COALESCE(install_begin_at, received_at) >= REFERRER_COUNT_SINCE`). 집계에서 빠지는 소급 referrer 가 과거 fingerprint 설치를 소급해서 지우면 이미 보고한 수치가 1 → 0 으로 줄기 때문이다. 클릭 `os` 가 캠페인 `platform` 과 맞을 때만(`ALL` 은 둘 다) | 확률 |
...
| `signups.claim` | `claimed_as_new_user = true` 인 `link_clicks` + `install_referrers`(source LINK·META·GOOGLE — ORGANIC·UNKNOWN 은 claim 대상이 아니라 참이 생기지 않는다) 중 `signup_at`(가입 시각)이 기간 안. `install_referrers` 쪽은 설치 집계와 같은 `COALESCE(install_begin_at, received_at) >= REFERRER_COUNT_SINCE` 도 만족해야 한다 — 소급 행의 늦은 claim 이 이미 보고한 가입 기간을 바꾸지 않게 한다. 기존 계정 로그인 claim(`false`)은 뺀다. **`link_clicks` 쪽은 `claimed_as_new_user = true` 이고 `COALESCE(install_begin_at, received_at) >= REFERRER_COUNT_SINCE` 인 `install_referrers`(source LINK·META·GOOGLE) 행의 `click_id` 가 가리키는 클릭을 뺀다**(같은 설치의 가입이 referrer 원장에 이미 있을 때만 — 같은 사용자면 claim 때 참값을 referrer 로 옮겼으므로(§2.5) 빼는 클릭에는 참값이 없다. referrer 가 기존 계정 claim(`false`)이면 클릭 쪽 신규 가입을 그대로 센다). 사용자당 `true` 는 한 행뿐이라(§2.5) 한 사용자가 여러 링크·referrer 를 claim 해도 가입 1건 | 확률 또는 결정 |
...
- referrer 로 귀속된 설치를 fingerprint 에서 빼기 위해 `install_referrers.click_id` 로 조인해 그 클릭을 뺀다(추가 컬럼 없음, 부분 인덱스 `idx_install_referrers_click`). §2.4-3 규칙으로 같은 설치의 LINK 설치는 fingerprint 매치가 먼저 났든 뒤에 오든, META·GOOGLE 설치는 fingerprint 매치가 먼저 났을 때(같은 `installId`) `click_id` 가 그 클릭을 가리키므로 두 칸에 한 번씩 잡히지 않는다. META·GOOGLE 이면 설치는 referrer 쪽(Meta 캠페인·GOOGLE 채널)에만 남고 그 클릭의 캠페인 링크(오가닉) 수치에서는 빠진다. 다른 기기에 매치된 클릭은 별개 설치다.
- `signups.claim` 은 신규 유저만 센다. 기간은 claim 이 늦게 성공해도 가입 날로 잡히도록 `signup_at` 으로 자른다. 광고로 설치하고 기존 계정으로 로그인한 사용자는 설치에는 들어가고 가입에는 들어가지 않는다. 설치 수와 같은 규칙으로 중복을 뺀다 — 첫 실행에 referrer 읽기가 실패해 fingerprint 매치·slug claim 이 먼저 끝나고 다음 실행에 같은 설치의 LINK·META·GOOGLE referrer 가 저장·claim 되면, 그 클릭을 가리키는 referrer 가 신규 가입(`claimed_as_new_user = true`)이면 클릭 쪽 가입은 빼고 referrer 쪽 1건만 센다(anti-join, `idx_install_referrers_click`). referrer 가 기존 계정 claim 이면 빼지 않는다. 이때 가입 참값은 claim 이 클릭에서 referrer 로 옮겨 두었으므로(§2.5 「연결된 두 원장」) anti-join 이 참값을 버리지 않는다. 채널 합계도 같다.
...
- **폐기 링크의 소진 클릭**: §2.3 은 창 안 최신 후보를 고정·소진한 뒤 활성 검증에 실패하면 그 클릭에 `match_rejected = true` 를 적고 `matched:false` 로 끝낸다(정책 L14). 소진은 재매치 방어라 남기지만 귀속이 아니므로, `installs.fingerprint` 는 `match_rejected = false` 인 행만 센다(§1.4 · §3.2).
...
- **Android 스토어 버튼은 신규다**(현 템플릿엔 App Store 버튼만 있다). 두 템플릿 모두 `androidStoreUrl` 을 받아, 설정값 `LINK_ANDROID_PACKAGE` 가 비어 있으면 버튼을 숨긴다. 값이 있으면 Play URL 은 쓰임에 따라 둘이다.
  ...
  - **콘솔 발급 응답의 `playStoreUrl`**(오가닉 게시물에 직접 넣는 용, 방문 전): `referrer=<URL 인코딩된 "slug=<slug>">`. 방문이 없어 `click` 이 없고, 이 URL 로 설치해도 §4.1 1행으로 LINK 에 귀속된다. 캠페인 `platform` 이 `IOS` **이거나 `channel` 이 `GOOGLE` 이면 `null`** 이다 — Google 광고는 최종 URL 의 referrer 를 Google 이 `gclid` 로 덮으므로 slug URL 이 살아남지 않고, 살아남는 경로(팀원이 게시물에 붙이는 등)로 들어오면 채널 단위로만 센다는 정책 L11·콘솔의 `GOOGLE_CHANNEL_ONLY` 표시와 어긋난 캠페인 귀속이 생긴다. 콘솔은 GOOGLE 캠페인 상세에 URL 대신 "Google 광고는 Google Ads 최종 URL 을 쓰고, 설치는 채널 합계로 셉니다" 를 보여준다(§6.3).
...
- **판정 축은 방문 국가 하나**다. Cloudflare 가 붙이는 `CF-IPCountry`(ISO 3166-1 alpha-2, 판정 불가는 `XX`, Tor 는 `T1`)를 쓴다.
- **선행 조건: Cloudflare 의 IP Geolocation 설정을 켜야 한다**(배포 순서 [HLD §7](high-level-design.md#7-배포-순서) 4단계). 이 설정이 꺼져 있으면 헤더 자체가 오리진에 오지 않고, 아래 폴백 규칙 때문에 **오류 없이 전원 기본 변형으로 떨어진다** — 국가 변형을 배포해 놓고도 아무도 그것을 못 보는 상태가 조용히 성립하므로, 설정 활성화를 배포 항목으로 박고 배포 뒤 다른 국가에서 실제로 다른 변형이 오는지 한 번 확인한다.
...
- **두 겹째: business-api 는 원격 피어가 사설망(nginx)일 때만 `CF-IPCountry` 를 믿는다.** §4.3 의 `X-Real-IP` 규칙과 같은 조건이고, `ClientIpResolver` 가 이미 그 판정을 하고 있으므로 같은 자리에서 국가 헤더도 가른다. 한 겹(nginx)만 두면 nginx 를 우회해 business-api 에 직접 닿는 경로가 설정 실수·네트워크 변경으로 생겼을 때 국가를 위조해 원하는 변형을 받아 갈 수 있다 — 지금은 국가가 UI 변형만 정하므로 피해가 작지만, A23 의 신뢰 경계는 원래 두 겹(`set_real_ip_from` = CF CIDR + 오리진 방화벽)이라 대칭을 깨뜨리면 다음에 이 헤더로 무언가를 더 정할 때 구멍이 된다.
...
| 2 | Android **첫 실행**: `InstallReferrerClient` 로 referrer 를 읽어 **로컬에서** 출처를 가린다 — `slug` 가 있으면 **그 `installId` 에서 referrer 를 처음 성공적으로 읽었을 때 한 번만** `POST /l/resolve {slug}` 로 초대 시트·목적지를 열고(새로 설치한 경우만 — 업데이트 설치는 이 행 끝) `/l/match` 생략, 출처 판별은 **§4.1 표와 같은 규칙·같은 순서**로 한다(META 는 `utm_content` JSON 에 `source.data`·`source.nonce` 가 **둘 다** 있을 때만) — META·GOOGLE(`gclid`)이면 `/l/match` 생략, 그 밖(ORGANIC·UNKNOWN)이면 기존 매치(요청에 `installId` 를 싣는다, 6행). **fingerprint 매치의 완료 값 `deferredInviteChecked` 는 존재 플래그가 아니라 처리한 `installId` 로 저장**하고, 저장값이 현재 `installId` 와 다르면 새 설치로 보고 매치를 다시 시도한다(현 `deferredInvite.ts:84` 는 값이 있기만 하면 건너뛴다). **앱의 완료 값은 `installId` 만으로 비교한다** — Play 설치 시작 초는 첫 실행에 referrer 읽기가 일시 실패하면 없다가 다음 실행에 생겨 같은 설치인데 값이 바뀌므로 서버 `install_key` 에만 쓴다. `installId` 는 백업 제외 저장소라 재설치마다 새로 생기므로, 설치 시각이 없을 때의 폴백을 따로 둘 필요가 없다. **업데이트 전 구 앱이 남긴 값(`'1'` 처럼 `installId` 형식이 아닌 값)은 OS 설치 메타데이터로 업데이트와 재설치를 가른다** — `PackageManager` 의 `PackageInfo.firstInstallTime` 과 `lastUpdateTime` 을 비교해, **`firstInstallTime < lastUpdateTime`(기존 설치 위 업데이트)이면 「현재 설치를 이미 처리함」으로 보고 매치 없이 현재 `installId` 로 옮겨 적고**, **`firstInstallTime == lastUpdateTime`(이번 버전을 새로 설치 — 구 앱을 지운 뒤 Auto Backup 이 `'1'` 을 복원한 재설치)이면 새 설치로 보고 매치·referrer 를 시도한다.** 두 값은 OS 가 주는 설치 메타데이터라 Auto Backup 대상이 아니고 기기 식별자도 아니다. 현 앱은 이 값을 읽지 않으므로(`app/legacy/app-dev/src`·`android/app/src` 에 사용처 없음) `installId` 와 같은 네이티브 모듈에서 읽는다. 구분 없이 옮겨 적으면 백업 복원 재설치의 fallback 귀속이 영구 누락되고, 구분 없이 비교하면 업데이트 직후 기존 Android 사용자 전원이 새 `installId` 로 재매치해(과거 매치는 `matched_install_id` 가 NULL 이라 재반환되지 않는다) 같은 공인 IP 의 남의 미소진 클릭을 빼앗는다. iOS 는 앱을 지우면 앱 데이터가 함께 지워지므로 구 값이 남아 있으면 업데이트로 보고 현행 플래그를 유지한다(기기 전체 복원은 한계). **저장 `POST /l/referrer` 는 세션을 확보한 두 지점에서** access token 과 함께 보낸다: ① 로그인·게스트 시작 뒤 `postAuthSave` 안(claim 보다 먼저) ② **콜드스타트 세션 복원이 성공한 직후**(`App.tsx` 복원 effect 에서 `getMyProfile` 이 성공해 토큰이 유효로 확인된 뒤). Auto Backup 이 `STORAGE_KEYS.user`(토큰)와 `onboardingComplete` 까지 복원하면 재설치한 앱은 로그인 화면 없이 홈으로 가서 ① 이 불리지 않기 때문이다(`App.tsx:181-243` — 둘 다 있으면 `postAuthSave` 없이 복원, 매니페스트에 `fullBackupContent`·`dataExtractionRules` 없음). 오프라인이라 프로필 조회가 실패한 복원에서는 보내지 않고 다음 실행에 다시 본다. 두 지점 모두 **현재 `installId` 가 이미 보고한 값과 다를 때만** 보내고, ② 에서도 저장 응답 뒤 claim 을 부른다(복원된 세션은 기존 계정이라 가입 수에는 들어가지 않는다). **claim 재시도는 referrer 재보고와 별개다** — 세션 복원이 성공하면 referrer 를 다시 보내지 않는 경우에도, 로컬에 `claimed:false` 인 대기 attribution(현재 `installId` 의 것)이 있으면 claim 을 부른다. 저장은 성공했는데 바로 뒤 claim 이 503·타임아웃으로 실패하면 완료 값에 이미 현재 `installId` 가 적혀 재보고가 생략되고, `postAuthSave` 도 안 거치므로 다시 부를 지점이 없어져 귀속이 영구 누락된다. 2xx 를 받으면 그때의 `installId` 를 완료 값으로 저장하고, 다음 세션에서 `installId` 가 저장값과 다르면 다시 보낸다(Auto Backup 으로 앱 데이터가 복원돼도 백업되지 않는 `installId` 는 새로 생겨 값이 달라진다). referrer 를 아직 못 읽었으면 완료 값을 저장하지 않고 다음 실행에 다시 읽는다. referrer 완료 값은 구 앱에 없던 값이라 옮겨 적기 규칙이 필요 없다. **400(`installId` 없음)·401·429·5xx·타임아웃이면 완료 값을 저장하지 않는다**(현 `deferredInvite.matchOnce` 와 같은 규칙). **세션 확보 뒤 `/l/referrer` 응답의 `source` 가 `UNKNOWN`·`ORGANIC` 인데 그 `installId` 에서 fingerprint 매치를 아직 하지 않았으면 그때 `/l/match` 를 1회 부른다** — 로컬 판별과 서버 판별이 어긋나도 fallback 을 잃지 않게 한다(매치 창 밖이면 서버가 후보 없음으로 끝낸다). `attributionId` 보관은 **응답 `source` 가 LINK·META·GOOGLE 일 때만** 한다 — ORGANIC·UNKNOWN 이면 보관하지 않고 claim 은 fingerprint 매치의 slug 로만 한다(§2.5·§3.2). **로컬에 보관하는 대기 attribution(`inviteAttribution` 의 slug·groupId, `attributionId`)에는 저장 당시 `installId` 를 함께 기록**하고, claim 전에 현재 `installId` 와 다르면 **보내지 않고 지운다** — 현 `inviteAttribution` 은 AsyncStorage 의 `{slug, groupId, claimed}` 뿐이라(`deferredInvite.ts:119`·`136-146`) 설치 A 에서 매치한 미claim INVITE 가 Auto Backup 으로 설치 B 에 복원되면 B 의 계정이 bare `{slug}` claim 으로 A 의 클릭을 가져간다. `installId` 가 없는 구 형식 값은 구 완료 값과 같은 `firstInstallTime`·`lastUpdateTime` 분기로 가른다(재설치면 폐기, 업데이트면 현재 `installId` 로 승계). **이 기능이 들어간 버전으로 업데이트한 설치**(Android `firstInstallTime < REFERRER_FEATURE_SINCE` — referrer 를 담은 첫 앱 버전의 출시 시각, 앱 상수. `firstInstallTime < lastUpdateTime` 과 완료 값 부재만으로 가르면, 이 기능을 담은 버전을 설치하고 첫 실행 전에 자동 업데이트된 정상 신규 설치도 구 설치로 오인해 이동·claim 을 영구히 건너뛴다)는 Play 가 돌려주는 과거 referrer 로 **`/l/resolve` 화면 이동과 `attributionId` 보관·claim 을 하지 않는다** — `/l/referrer` 저장만 하고, 그 행은 서버 `REFERRER_COUNT_SINCE` 로 집계에서도 빠진다(`REFERRER_COUNT_SINCE` 는 서버 집계만 거르므로 화면 이동은 앱이 막아야 한다). 첫 성공 읽기 때의 1회 이동은 새로 설치한 경우(`firstInstallTime == lastUpdateTime`)에만 한다 | 구 앱은 호출하지 않을 뿐 |
...
- **기능 플래그 `CAMPAIGN_LINK_ISSUE_ENABLED`**(data-api env, 기본 `false`). 꺼져 있으면 `POST /internal/campaigns/{id}/links` 가 409 `CAMPAIGN_LINK_ISSUE_DISABLED` 이고, 콘솔 캠페인 상세는 발급 폼 대신 "앱 업데이트가 배포된 뒤 링크 발급이 열립니다" 를 보여준다. **플래그는 data-api 한 곳에만 두고**, 콘솔은 `GET /internal/campaigns` 응답의 `linkIssueEnabled`(§3)로 화면을 고른다 — business-api 에 같은 값을 또 두면 두 곳이 어긋난다. 발급 거절의 정본은 여전히 data-api 의 409 다. 캠페인 생성·수정·보관·통계 화면과 iOS 광고 캠페인(링크 없음)은 플래그와 무관하다. **켜는 조건**: 파서·목적지·`/l/resolve`·Install Referrer 를 담은 앱 버전([HLD §7](high-level-design.md#7-배포-순서) 7단계)이 양 스토어에 출시되고 최소 지원 버전으로 지정된 뒤. 그 전에 캠페인 링크로 설치한 구 앱은 CAMPAIGN 매치 응답의 `type`·`destination` 을 무시하고 `groupId` 가 없어 이동·claim 을 하지 않으면서 완료 플래그(`deferredInviteChecked`)는 남기므로, 업데이트 뒤에도 그 설치의 이동·가입 귀속이 복구되지 않는다(정책 L27).
...
| `/console/campaigns/{id}` | 링크 표(slug · URL · 목적지 · 상태 · 클릭 · 설치 · 가입), Android 는 Play 스토어 URL 복사 칸(`channel` 이 `GOOGLE` 이면 URL 대신 "Google 광고는 Google Ads 최종 URL 을 쓰고, 설치는 채널 합계로 셉니다"), `notes` 문구 | 링크 발급 · 폐기 · 캠페인 수정(이름 · 보관 / Meta campaign_id · SKAN source-identifier 는 비어 있을 때 한 번만 입력, §1.2) |

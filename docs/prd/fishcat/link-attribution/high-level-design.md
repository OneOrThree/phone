...
sequenceDiagram
  ...
  P->>A: 설치
  A->>A: 첫 실행 · InstallReferrerClient 로 읽어 로컬에서 출처 판별(LLD §4.1 과 같은 규칙)
  alt slug 있음 (우리 링크)
...
설치 단위는 `install_key` 라, 같은 기기에서 앱을 지웠다가 다른 광고로 다시 깔면 새 설치로 센다. 앱의 완료 값(referrer 보고와 fingerprint 매치)은 처리한 `installId` 로 저장해, Auto Backup 으로 복원돼도 재설치에서 다시 시도한다. Play 설치 시작 초는 첫 실행 읽기 실패 뒤 생기면 값이 바뀌므로 서버 `install_key` 에만 쓰고, 구 앱이 남긴 값은 Android 설치 메타데이터로 가른다 — `firstInstallTime < lastUpdateTime`(기존 설치 위 업데이트)이면 매치 없이 현재 `installId` 로 옮겨 적고, `firstInstallTime == lastUpdateTime`(구 앱을 지운 뒤 Auto Backup 이 값을 복원한 재설치)이면 새 설치로 보고 매치·referrer 를 다시 시도한다(iOS 는 업데이트로 본다, LLD §5-2 · 정책 L21). 로컬에 보관한 대기 초대·`attributionId` 도 저장 당시 `installId` 에 묶어 현재 설치와 다르면 claim 전에 지우고, **기능 도입 전에 설치된 기기**(`firstInstallTime < REFERRER_FEATURE_SINCE`)는 Play 가 돌려주는 과거 referrer 로 화면을 열거나 claim 하지 않는다(업데이트 여부만으로 가르면 기능 포함 버전을 설치하고 첫 실행 전에 자동 업데이트된 신규 설치까지 막힌다). 서버도 설치를 백업되지 않는 `installId` 로 가른다 — 매치 창 안 재설치가 복원된 `deviceId` 로 과거 매치를 돌려받지 않고, Play 설치 시각이 없는 referrer 도 설치마다 다른 키가 된다. 저장은 세션 확보 뒤라 첫 실행 뒤 로그인·게스트 시작 없이 앱을 떠난 설치는 우리 수치에 들어가지 않는다. Auto Backup 이 로그인 세션까지 복원하면 재설치한 앱은 로그인 화면을 건너뛰므로, 저장은 **콜드스타트 세션 복원이 성공한 직후**에도 보낸다. 대신 토큰 없는 위조 저장이 막힌다(§5).
...
sequenceDiagram
  ...
  B->>B: 크기·동시 검증 상한 · app-id 확인 · 서명 검증 (실패는 저장 없이 200)
...
Android 의 Play 기본값(ORGANIC)·판별 불가(UNKNOWN) referrer 는 귀속이 없는 기록이라 설치·가입 수에 넣지 않는다 — 그 설치는 fingerprint 원장이 센다(두 원장 이중 집계 방지).
...
- **SKAN**: 공개·무인증 경로라 **서명을 통과한 우리 앱 포스트백만 저장한다.** 서명 실패·모르는 버전·다른 앱은 저장 없이 200 으로 끝내고 건수만 센다. 본문 16KB 제한과 인스턴스당 동시 서명 검증 상한(32건, 넘으면 503 + `Retry-After`)만 둔다 — Apple 이 여러 설치 분을 서버 간으로 보내 발신 IP 가 사용자 식별자가 아니므로 IP·엣지 레이트리밋은 두지 않는다(두면 캠페인이 커질 때 정상 포스트백을 잘라 과소 집계한다). 저장 실패만 5xx 로 남겨 경보한다.
...
- **랜딩 클릭 저장량**: `User-Agent` 는 위조할 수 있으므로 봇 판별만으로는 클릭 부풀림을 못 막는다. `GET /l/{slug}` 에 slug 와 무관한 IP 단위 한도를 두고(넘으면 `recordClick=false` 로 같은 랜딩을 보여주고 클릭 미기록 — business-api 는 DB 를 읽지 않아 호출을 건너뛰면 정상 랜딩을 못 그린다), data-api 는 같은 링크·IP 해시·OS 의 클릭을 매치 여부와 무관하게 매치 창 안에서 20건까지만 새로 만든다 — 매치로 줄지 않아 방문과 `/l/match` 를 번갈아 불러도 한도가 되돌아가지 않는다. Cloudflare 엣지 한도도 같이 둔다.
- **캠페인 플랫폼**: 캠페인의 플랫폼과 다른 OS 로 들어온 설치·가입은 그 캠페인 수치에 넣지 않고 채널 합계로 뺀다. iOS 캠페인에는 Play URL 을 발급하지 않고 랜딩에서 Android 스토어 버튼을 숨긴다. Meta·Google 캠페인 링크의 iOS 방문은 클릭을 기록하지 않아 fingerprint 후보가 생기지 않는다(정책 L08).
...
| 4 | Infra: nginx 링크 경로를 business-api 로 전환(reload 1회), Cloudflare 에서 SKAN 경로 봇 챌린지 예외(엣지 레이트리밋 없음) · `/l/*` GET 엣지 레이트리밋 · **Cloudflare IP Geolocation 활성화**(끄면 `CF-IPCountry` 가 오리진에 오지 않아 랜딩 국가 변형이 전원 기본 변형으로 조용히 떨어진다, 정책 L28) · nginx 가 Cloudflare 대역 피어의 `CF-IPCountry` 만 오리진으로 넘기도록 설정 | nginx 원복 reload — 2 단계의 공개 컨트롤러가 같은 DB 를 읽으므로 무손실 |

...
**상태: 구현용 설계 초안. 보상 산식·귀속·강퇴·휴식 채팅 차단·응원 TTL/빈도는 확정됐고, 세션 없는 공유 휴식은 planning-document 개정이 필요한 추가 범위다(GROMO-1958). 기여 배분(FR-D02)은 D6 로 폐기 — [결정 로그](../decision-log.md).** 보상 정책까지 확정됐다는
...
문서 경로는 배치 조정자가 지정한 `docs/prd/fishcat/focus-rest-session/`다. 티켓 본문의 초기 제안 경로
`docs/prd/focus-session/`과 다르며 동일 GROMO-1763 산출물을 가리킨다.

| 읽을 문서 | 내용 |
| --- | --- |
| [PRD](prd.md) | 목적·원본 9계약 + GROMO-1998 추가 계약 범위·완료 조건·사용자 경험 |
...
원본 9계약의 구현 분할은 **1764 REST 6종 + 1765 REST 스냅샷 2종·STOMP emote SEND 1종**이며,
GROMO-1998는 여기에 `rest-auto-close`·`pending-result`·`acknowledge`를 더한다.
SUBSCRIBE 토픽과 내부 제어 계약은 원본 9개와 별개이며 원본 66계약의 분모를 늘리지 않는다.

...
| --- | --- | --- | --- |
| FR-D01 | **산식 확정 — GROMO-1830 코멘트 #11072(2026-09-15), 2026-09-18 재영님 결정 D5 로 정본 승격**: 유효 집중 60초당 1마리 · 휴식 구간 제외 · 하루 상한 주민·섬별 480마리(상한에 닿아도 집중 기록은 쌓이고 물고기만 더 낚지 않음) · 퀘스트 개인 달성 +10마리 · 전원 달성 보너스 대상 주민 수 × 5마리 · 퀘스트 종류별 차등 없음. ~~300초/물고기~~ 는 목업이었다 | **보류 — 보상의 개인 지갑/섬 통장 귀속(비율).** 2026-09-18 재영님 「난배 비율 정해져 있지 않냐」 → 근거 확인 중(2026-09-18): 1829·1830·기획 정본·레포 문서에 분배 비율이 없고, 유일한 서면 규칙(기획 정본 「개인 물고기 지갑은 없다」= 전부 섬)은 D1 이 뒤집었다 — [결정 로그](../decision-log.md) D5. **이 칸은 채우지 않는다** | 산식은 revision 있는 서버 정책 설정으로 관리(60·480·10·5 를 코드 상수로 박지 않음). 귀속 확정 전 fish 지급 경로 비활성. 하루 상한의 「하루」 경계는 D8(UTC 00:00 리셋, 전환 계획 1930 대기)을 따른다. 기존 자산 승계는 D11(이관 안 함)로 닫혔다 |
...
| FR-D03 | 진행/휴식 중 강퇴·섬 종료 시 세션과 미수령 보상은? 관리 설계와 공동 질문 | 서버 시각 강제 종료+개인 확정 보상 보존을 관리 설계가 추천. 공동기여/퀘스트 자격은 별도 결정 | 정책 없는 강퇴가 세션을 CANCELED로 버리거나 임의 정산하지 않도록 해당 교차 기능 출시 차단. **상태(2026-09-18): 설명 대기** — 재영님 「정책이 어떻게 되는데?」, B3(종결하되 개인 보상 보존) 추천 설명을 기다리는 중 |

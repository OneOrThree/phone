...
~~산식 결과 earnedFish=E, 개인 반영=P, 초기건설 기여=C라면 **E=P+C**가 기본 보존식이다.~~ 초기 건설 기여 C 라는 축 자체가 없어졌다(D6). 산식은 1830 확정값(60초당 1마리·휴식 제외·주민·섬별 하루 480 상한, [정책](policy.md) FR-D01)이며 earnedFish 의 **개인 지갑/섬 통장 귀속은 FR-D01 보류 칸**을 따른다 — 아래 응답 예시의 `allocation{personalFishAdded,constructionFishAdded}` 는 폐기된 모델의 형태라 귀속 결정 뒤 개정한다.

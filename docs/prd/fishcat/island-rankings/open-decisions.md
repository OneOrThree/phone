...
> ⚠️ **이 문서는 2026-09-18 작성이고 근거가 낡았다 — 사실 확인에 쓰지 마라(2026-09-21 GROMO-1997 확인).**
> 이후 1759·1764·1769·1773 이 머지되면서 실측으로 확인된 오류가 여섯이다:
> ① 집중 시작 게이트는 「하드 false」가 아니라 **설정 플래그**다 ② `current_island_id` 는 「전부 NULL」이 아니라
> **writer 가 있다**(1759) ③ `business.cursor.enabled` 는 0건이 아니라 **존재한다**(`SignedCursorCodec`)
> ④ 내부 허용목록은 비어 있지 않고 **83줄 존재**한다 ⑤ 다음 migration 은 V59 가 아니라 **V85** 다
> ⑥ RK-D05(귀속)는 답변 대기가 아니라 **확정**됐다(2026-09-19 RC-D01).
> **RK-D01~D03·D05~D06 은 2026-09-21 에 결정됐다** — 결정 정본은 [policy.md](policy.md) 이고, 남은 미결도
> 거기(RK-D04·D10·D11)에 있다. 주민 랭킹 관련 선택지는 엔드포인트 폐기(B23·B15)로 전부 무의미하다.

...
현재 개발 중인 Catus 2.0의 제품·API·전환 문서다. 앱 구현 기준은 `app/app-dev/`이며 기존 GROMO 1.x와의 호환 근거는 필요할 때 [`../gromo/`](../gromo/) 문서를 참조한다.
...
`account`, `link-attribution`, `server-separation`처럼 기존 API와 데이터 승계를 함께 다루는 문서도 Catus 출시를 위한 계약이므로 이 폴더에서 관리한다.

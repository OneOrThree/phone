...
| RC-D02 | 측정 대상 범위·OS별 정확도·대표1대/sum/max, 대표기기 변경/소유 바인딩 | **보류 — 2026-09-18 재영님 결정 D7.** 저장 축(RC-P07·08, 기기별 분리)만 확정. 복수 기기의 표시·집계 병합은 출시 후 결정하며 가장 나중에 다룬다. 대표1대는 단순하나 승인 아님. sum 중복/max 누락 위험. FCM 토큰을 deviceId로 간주하지 않음 | 검증된 기기·수집 경계와 병합 정책 전 공개 일집계/신규 수집 활성화 금지 — [결정 로그](../decision-log.md) |

...
| RC-D02 | 측정 대상 범위·OS별 정확도·대표1대/sum/max, 대표기기 변경/소유 바인딩 | **소비자 추가(2026-09-21 GROMO-2001)**: 일일 스크린타임 퀘스트 판정이 이 관측을 읽는다. 「기기별 최신 하나 · 같은 날 복수 기기는 병합 없이 측정 불가」 규칙의 정의는 `screentime/support/ScreenTimeDayPick` 한 곳이고 회관 기록 조회와 판정이 그것을 공유한다 — 규칙이 갈라지면 도서관이 「측정 불가」로 보여 주는 날을 퀘스트가 「달성」으로 정산한다. 판정에서 복수 기기는 «유예 없이» 분모 밖이다(기다린다고 기기 수가 줄지 않는다). 보상 정산 자체는 여전히 `island-quest.settlement-enabled`(기본 false) 뒤에 있다. **부분 결정(2026-09-19 RC-D02-기기)**: 기기 = 서명된 로그인 세션(deviceId=sid), 같은 날 복수 기기는 null·unavailable 로 보류. 병합 자체는 여전히 D7 보류. **보류 — 2026-09-18 재영님 결정 D7.** 저장 축(RC-P07·08, 기기별 분리)만 확정. 복수 기기의 표시·집계 병합은 출시 후 결정하며 가장 나중에 다룬다. 대표1대는 단순하나 승인 아님. sum 중복/max 누락 위험. FCM 토큰을 deviceId로 간주하지 않음 | 검증된 기기·수집 경계와 병합 정책 전 공개 일집계/신규 수집 활성화 금지 — [결정 로그](../decision-log.md) |

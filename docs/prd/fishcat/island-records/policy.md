...
| RC-D02 | 측정 대상 범위·OS별 정확도·대표1대/sum/max, 대표기기 변경/소유 바인딩 | **부분 결정(2026-09-19 RC-D02-기기)**: 기기 = 서명된 로그인 세션(deviceId=sid), 같은 날 복수 기기는 null·unavailable 로 보류. 병합 자체는 여전히 D7 보류. **보류 — 2026-09-18 재영님 결정 D7.** 저장 축(RC-P07·08, 기기별 분리)만 확정. 복수 기기의 표시·집계 병합은 출시 후 결정하며 가장 나중에 다룬다. 대표1대는 단순하나 승인 아님. sum 중복/max 누락 위험. FCM 토큰을 deviceId로 간주하지 않음 | 검증된 기기·수집 경계와 병합 정책 전 공개 일집계/신규 수집 활성화 금지 — [결정 로그](../decision-log.md) |
...
2026-09-19 소유자 결정(N26)으로 RC-D01·D03·D04·D05 는 권장안(권장안 없는 세부는 가장 보수적인 안)으로 확정됐고 RC-D02 는 기기 식별·복수 기기 null 만 정했다 — 전체 표와 반려한 대안은 [결정 로그](../decision-log.md) 2026-09-19 RC-* 행. 이전 문장(아래)은 이력이다. ~~RC-D01/02는 이미 질문 중인 제품 선택이다. RC-D03/04/05는 연관 기획·퀘스트 결정과 함께 확정하며 기본값을 발명하지 않는다.~~ 내부 원본 저장·CAS·DTO·비활성 구현과 정적 검증은 독립 진행할 수 있다. '일단0분' 또는 '일단 보상0' 성공은 미측정/미결 의미를 영구 손상시키므로 금지한다.

...
내부 경로는 B26 규칙(`docs/prd/fishcat/bff-screens/policy.md` B26) — 본인 것만 읽는 조회는
...
| 화면 | 조각 | 내부 GET | 비고 |
| --- | --- | --- | --- |
| `/screens/friends` (프레임 78·79) | `friends` | `GET /internal/users/{userId}/friends?date=` | `FriendService.getFriends` 그대로(§1 행 5). 내부 GET만 `date` 선택 — 생략 시 KST 오늘(우체통 조합용, GROMO-2119) |
...
**실시간 토픽 — 새로 만들지 않는다.** B22(`docs/prd/fishcat/bff-screens/policy.md` B22)는 토픽을 정확히

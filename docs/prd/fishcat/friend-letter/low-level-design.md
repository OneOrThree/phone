...
이 문단은 GROMO-1975에서 구현한 범위다. Fishcat 2.0의 [RP-표시차단](../character-report/policy.md#rp-표시차단--차단자-화면에서-숨김-발송은-허용-2026-09-24-개정)은 발송을 거절하지 않고 `GET /blocks`를 이용해 받은 친구 요청·편지 상세·채팅·댓글·사용자 공지를 차단자 화면에서 숨긴다. 기존 서버의 받은 편지함 필터는 그대로 쓴다. 친구 요청 푸시는 앱 목록 필터로 억제할 수 없어 알림 경로의 차단 확인이 남는다.

...
| P15 | 사건·측정 시점은 UTC instant. 퀘스트 windowStart/windowEnd는 예외로 KST HH:mm 벽시계. 날짜 집계는 KST이며 timezone 입력 5종은 Asia/Seoul만 허용, 누락 시 같은 값으로 정규화 | 임의 사용자 타임존 지원을 이 문서에서 신설하지 않음. 별도 제품 결정 시 기존 저장/집계/조회 정책까지 함께 검토 |
...
신규 가입1760의 `POST /islands/{islandId}/memberships`는 `(403, KICKED_CANNOT_REJOIN)`도 공개 `403 FORBIDDEN`(`retryable=false`, `field=null`)으로 명시 매핑한다. 기준 main [GroupService.joinGroup](https://github.com/OneOrThree/phone/blob/529a396e5f0f88cb78c172110920e1fa6b9388a9/server/data-api/src/main/java/com/oneorthree/phone/group/service/GroupService.java#L320-L325)의 기존 강퇴 이력 거절이며, legacy/compat는 원 코드와403을 유지한다. 강퇴 이력 사용자의 신규 가입·legacy 재가입 실제 HTTP 회귀로 각각 이 매핑과 원 코드 보존을 검증하기 전 신규 가입을 활성화하지 않는다. 기존 재가입 금지를 미등록502로 바꾸거나 공통 FORBIDDEN이라는 이유로 우회하지 않는다.
...
미리보기 호환 매핑은 분리한다. `RATE_LIMITED`·`NOT_FOUND`는 유지, 요청 유효성 `INVALID_REQUEST`는400으로 유지한다. 기존 provider 실패 코드(`FETCH_TIMEOUT` 등)가 **Preview 객체의 실패 상태 데이터**이면 POST200을 유지한다. 신규 경로는 data 안에, 기존 호환 경로는 원래 직접 반환하던 객체/목록 안에 남는다. 이를 HTTP504로 바꾸지 않는다. 예외로 나오는 미리보기400의 세부 코드는 1751에서 원 코드 목록을 그대로 계약 테스트에 고정한다. 기존 `/api/v1/link-previews`는 성공 본문과 `{code,message}` 평면 오류, 기존 `/api/v1/link-previews/{id}/thumbnail` URL을 보존한다. 신규 `/link-previews`는 JSON 봉투·공통 오류·신규 `/link-previews/{id}/thumbnail` URL을 사용한다. 공유 Preview 캐시의 저장 URL을 전역 변경하지 않고 신규 응답 매핑에서만 URL을 바꾼다. PNG 성공은 양쪽 모두 image/png이며, 미리보기 도메인 실패는 application/json으로 명시하여 각 경로의 오류 형식을 따른다. 앱은 성공 이미지와 실패 JSON을 모두 처리하도록 `Accept: image/png, application/json`을 보낸다. 기존 image/png 단독 요청도 PNG 성공 및 명시 JSON 오류 동작을 유지한다.

406은 JSON 오류 표의 예외다. [기존 오류 규약 §4](../../../conventions/error-contract.md#4-상태-매핑-규칙)에 따라 지원할 수 없는 Accept는 **406+빈 본문**으로 반환하며 서버 X-Request-Id는 유지한다. 썸네일에 application/json만 허용하면 PNG 성공 매핑과 협상할 수 없어 서비스 호출 전406이다. 반면 image/png를 허용해 썸네일 핸들러에 진입한 뒤 발생한 도메인 오류는 앞 문단의 명시 JSON 응답이므로 image/png 단독이라는 이유로406으로 바꾸지 않는다. 협상 불가를 봉투 직렬화 때문에500으로 만들지 않는다. 이 빈 응답을 data:null 성공으로 감싸지 않는다.
...
| 날짜/IANA timezone 표현 | 사건·측정 instant는UTC, 퀘스트 창은KST HH:mm, 집계KST. 아래 5개 입력은 누락 시 Asia/Seoul, 그 외 값은400 INVALID_PARAMETER | 퀘스트 생성의 원본 시간대 오류422도400으로 명시 변경. 원본 입력 예시는 HTML에 보존 |
| 300초/물고기·건설비·상품가격·퀘스트10P | **확정값으로 교체(2026-09-18 D4·D5 정본 승격)**: 집중 보상 60초당 1마리·휴식 제외·주민·섬별 하루 480 상한, 퀘스트 개인 달성 +10·전원 보너스 대상 주민 수 × 5(GROMO-1830 #11072); 건설비 7건물 60~6,800마리·공사 시간 1분~4시간(GROMO-1829 #11077). **상품가격(상점 S03)만 목업 유지** | [집중 정책](../focus-rest-session/policy.md) FR-D01 · [건설 정책](../island-construction/policy.md) 「건설 가격·공사 시간」. 원본 방송기100P 는 1829 표(축음기 1,360마리)로 대체. 값은 설정 revision 으로 주입하며 코드 상수 아님 |
...
퀘스트 `windowStart`·`windowEnd`는 UTC instant 규칙의 예외다. [퀘스트 설계 PR750](https://github.com/OneOrThree/phone/pull/750)의 계약대로 focus 창은 KST의 24시간제 `HH:mm` 문자열(00:00~23:59, 시간·분 각 두 자리)로 검증·정규화한다. 날짜·초·UTC offset/Z를 붙이거나 `18:00`을 `09:00`으로 변환하지 않는다. 명령 fingerprint에도 이 벽시계 값과 정규화한 `Asia/Seoul`을 사용한다. 회차 실행 시 승인된 KST 회차 날짜와 결합해 instant를 계산하는 것은 별도 단계다. focus 생성은 두 필드 필수, screen 생성에는 창 필드를 받지 않으며 screen 조회 응답의 창 필드는 도메인 계약대로 null이다. 자정 통과 창 허용 여부와 목표 범위 등 QQ02 미결 정책은 이 형식 규칙으로 확정하지 않는다.
...
미결 제품 정책(2026-09-18 갱신): 상품 가격(상점 S03), ~~집중 보상의 개인/섬 귀속(FR-D01 보류)~~ → 2026-09-19 D5-귀속-개정으로 확정(섬 통장 100% · 개인 0%, 비율은 운영값 — ~~2026-09-18 D5-귀속 50/50~~ 대체)·집중/휴식 접근, 계정 재인증·삭제 보존, 퀘스트 대상주민·랭킹 분모/섬 귀속, 복수기기 측정 병합(RC-D02 보류). **2026-09-18 결정으로 빠진 것**: 기존 코인/보유품 승계(1909 A05·D11 — 승계·이관 안 함), 새 경제 운영값(1829·1830 확정), 건설 완료 전후 기여/잔량(D6 폐기), 공동소비 권한(D2), 초대 승인 생략(D10 — 승인제 유지). 정본은 [결정 로그](../decision-log.md). 이 설계로 정책 승인이나 운영 활성화가 이루어지지 않는다.

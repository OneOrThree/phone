...
- 내부 route에서 홈 섬으로 뒤로가기/홈 이동할 때 먼저 목적지 route를 표시한 뒤 덮개를 같은 duration/easing으로 줄여 홈을 드러낸다. 화톳불은 `rest`↔`focus`, 나머지 다섯 route는 홈 복귀에 연결한다. 복귀를 시작하면 이전 route의 로딩 덮개를 지워 역방향 모션을 가리지 않게 한다.

...
게임 종속 사항은 `MovementRules`(속도·반경·통행 비용·입구·맵 키)와 Business 제어 어댑터에 둔다. 서버 코어는 Java의 순수 도메인 모듈로 두고 Netty 이벤트 루프에서 A*·DB 호출을 실행하지 않는다. 입출력 루프, 제한된 A* executor, 방 상태를 직렬 갱신하는 틱 실행기를 분리한다. 큐를 무제한 늘리거나 연결마다 전용 플랫폼 스레드를 만들지 않는다. Spring을 제어 API·설정에 쓰더라도 20Hz 틱을 트랜잭션·JPA·STOMP 메시지 처리기에 결합하지 않는다.

핵심 엔진은 Fishcat 시설 enum, 집중 세션, 재화 타입을 import하지 않는다. 앱 예측기와 서버 탐색기는 같은 fixture를 사용하되 TypeScript와 Java 구현이다. Java GC pause·할당률·틱 지연을 측정하고, 20Hz 목표를 충족하는지는 부하 테스트로 판단한다.
...
6. 휴리스틱은 월드 직선 거리 × 맵의 최소 비용으로 두어 과대평가를 피한다. 비용 계산은 공통 고정소수점 스케일(초안 10⁶)로 통일하고 edge 비용은 올림, 휴리스틱은 내림한다. TS/Java의 계산·반올림 규칙을 fixture로 고정한다. 우선순위 동률은 `(f,h,nodeIndex)`로 고정한다. 공통 fixture는 경로·비용·도착점과 장애물 비관통을 검사한다.

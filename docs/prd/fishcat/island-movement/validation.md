...
| T03 탐색 | MV-04 | 길/잔디 비용·바다·건물·모서리·교량·다른 연결 영역·같은 셀 | TS/Go 공통 fixture에서 유효 경로·비용·도착점 일치 |
...
| T07 바이너리 | MV-08, MV-09 | 잘린 프레임·과대 count·미지원 버전·예약 필드·uint64 epoch | TS/Go golden bytes 일치, fuzz 시 크래시·과대 할당·상태 변경 없음 |
...
| 1. 전송 POC | Go/quic-go ↔ iOS/Android quiche adapter, stream+datagram | 실기기 암호화 연결·인증서 검증·망 전환·20Hz 측정. 실패하면 대안 비교 후 진행 |
| 2. 맵·코어 | 기존 아트 변환, 서버 A*·고정 틱·안전 도착점 | TS/Go fixture, headless 시뮬레이션, 통행/코너 회귀 |
...
계약 fixture 작업과 전송 POC는 서로 독립적으로 시작할 수 있지만, 네이티브 전송 검증 전에 전체 앱 연결 방식을 확정하지 않는다. 서버 구현은 기존 아키텍처 8단계와 소유 저장소·새 레포 등록을 함께 수행한다.

## 4. 문서 검증 기록

검증 환경: macOS, Google Chrome headless, Mermaid 11.12.0. Mermaid는 `/tmp`에 설치해 앱 의존성을 변경하지 않았다. `.mmd`를 단일 원본으로 두고 `.svg`를 함께 커밋한다.

- 도식 6개: 전체 구조, 배포, 이동, 입장, 맵 변경, 복구·취소.
...
- [quic-go DATAGRAM](https://quic-go.net/docs/quic/datagrams/): 서버 후보 및 처리량 POC 필요.

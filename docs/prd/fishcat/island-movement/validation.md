...
- 학습 도식 12개: 선택 축, 연결/방, 계산·표시 주기, 전송 계층, 손실, Kafka 팬아웃, 대안 A/B/C, 역압, 소유권, 선택 흐름. `.mmd`/`.svg` 총 19쌍.
...
| 2026-09-23 | Plannotator 2차 피드백 수신 | 폴더와 architecture.md의 동일 요청을 한 건으로 합침: 비교 대안을 이해할 기초 지식을 그림 중심의 별도 문서로 설명 |
| 2026-09-23 | 학습 안내·1차 자료 노트 추가 | learning-guide.md 13개 절에 학습용 새 도식 12개와 기존 입장 도식 1개를 배치. learning-sources.md에 RFC·Netty·Kafka·Java 공식 자료와 현재 코드 근거 기록 |
| 2026-09-23 | 학습 자료 검증 | 새 도식 12개 Mermaid 렌더·시각 점검, 전체 SVG 19개 XML 파싱, 상대 링크 113개 정상. 요구사항 13개와 32B/24B 패킷 계약 유지. 기술 선택·성능 실측·배포 상태를 새로 확정하지 않음 |

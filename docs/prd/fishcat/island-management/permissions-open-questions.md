...
건설 실행(1행)과 공동 상품 구매(4행)도 2026-09-21 GROMO-2000·D2-개정으로 닫혔다 — 남은 미결은 퀘스트·공지 두 행과 표에 없는 댓글이다.
...
### 4.2 공동 상품 구매 (`permissions.md:33` · S02 · ~~막는 티켓 GROMO-1781~~ → 해소)

> **확정(2026-09-21, GROMO-2000·D2-개정): 활성 주민 누구나, 게스트 제외** — 아래 선택지·추천은 결정 전 분석 기록이다.
> 당시 「섬 공동 지갑이 서버에 없다」는 전제도 `island_wallets` 신설로 해소됐다.

`island/village_points` 를 쓰는 공동 지갑 차감이다(상점 `policy.md:26`).

**~~근거 없음 — 섬 공동 지갑은 서버에 없다~~ → 해소: `island_wallets` 는 이후 신설됐다.** 당시 기록: 오늘 포인트는 전부 **유저 1:1** 이다:
...
~~**추천: P-1 (방장만).**~~ → **확정된 값은 P-2 방향이다 — 활성 주민 누구나(게스트 제외), 2026-09-21 GROMO-2000·D2-개정.**
기획 정본 「주민 누구나 섬 물고기로 상점 상품·축음기 음원을 구매할 수 있다」가 P-1·P-3·P-4 를 닫았다.
판정 이름 `SHARED_PURCHASE` 는 유지하되 구매 축(`canSpend`: 활성 주민 + 비게스트)과 건설 축(`canBuild`: 방장)으로 갈렸다.

> 주의: 이 행의 값을 「공동 테마 적용」(`SHARED_APPEARANCE`)에서 자동으로 베껴 오지 않는다.
> 외양 `policy.md:61~63` 이 *"두 권한은 별개"* 라고 못박았다. 값이 같아지더라도 **따로 결정한 결과**여야 한다 —
> 실제로도 테마(1909 방장만 → 2000 주민 누구나)와 구매(2000 주민 누구나)는 별개 결정이 같은 값에 도달했다.

### 4.3 건설 실행 (`permissions.md:30` · P-D01 · ~~막는 티켓 GROMO-1767~~ → 해소)

> **확정(2026-09-21, GROMO-2000·D2-개정): 방장만** — 목표 선택(PUT)·건설 실행(POST) 둘 다.
> 아래 선택지·추천은 결정 전 분석 기록이다. 「서버에 건설 구현이 없다」는 전제도 이후 구현으로 해소됐다.
...
→ 이후 구현됐다(`construction/` 도메인, `SharedPurchase.canBuild` 가 방장 게이트).
...
~~**추천: C-1 (방장만), 단 C-3 를 같이 물어봐야 한다.**~~ → **확정된 값은 C-1 이다 — 방장만, 2026-09-21 GROMO-2000·D2-개정.**
기획 정본 「방장이 회관의 다음 건물 선택에서 목표를 정하면」·「방장이 건설하기를 누를 때 … 차감하고 공사를 시작한다」가
C-2·C-3·C-4 를 닫았다 — 목표 선택을 주민에게 여는 C-3 도 채택되지 않았다. 잔여 미결은 가격 P-D03 뿐이다.
...
|---|---|---|---|
| ~~GROMO-1767~~ | 건설 3종 구현 | ~~4.3~~ → **해소(2026-09-21, GROMO-2000): 방장만 확정, 구현됨** | — |
...
| ~~GROMO-1781~~ | 상점 5종 구현 | ~~4.2~~ → **해소(2026-09-21, GROMO-2000): 활성 주민 누구나·게스트 제외 확정, `SHARED_PURCHASE` 제공자 `SharedPurchase.canSpend` 구현됨** | — |
| GROMO-1783 | 외양 4종 구현 | **없음** | 공동 테마 적용은 GROMO-1909 → GROMO-2000(주민 누구나)으로 닫혔다. 남은 블로커는 외양 A02 뿐이라 이 표 소관이 아니다 |

- **4.1(공지)은 결정 비용이 사실상 0** 이다 — 코드가 답을 갖고 있어 먼저 닫으면 1771 의 절반이 바로 풀린다.
- ~~**4.2·4.3·4.4 는 같은 공동 지갑을 쓰는 한 묶음**이라 한 번에 같은 값으로 답하는 것이
  `permissions.md:48`(「공동 소비 5행의 답변은 한 정본」)에 맞는다.~~ → 4.2·4.3 은 GROMO-2000 으로 닫혔고
  실제로 같은 값이 아니게 갈렸다(구매=주민 누구나, 건설=방장만). 남은 것은 4.4(퀘스트)다.
- ~~4.2·4.3 은 권한을 정해도 **섬 공동 지갑이 없어서** 바로 열리지 않는다~~ → `island_wallets` 신설로 해소.
...
| 2 | 「공동 테마 적용 권한을 확정한다 (1782)」 | **이미 확정** — GROMO-1909(PR #782, 머지) 「방장만」 → GROMO-2000 「활성 주민 누구나」 개정, 이름 `SHARED_APPEARANCE`(`permissions.md:34`, 외양 `policy.md`) | 완료 조건 1건이 이미 충족. 이 행을 다시 결정 대상에 올리면 안 된다 |
...
| 4 | 「TBD 를 0건으로 만든다」 | TBD 는 ~~4행 8칸~~ **2행 4칸**(GROMO-2000 으로 건설·구매 해소). 4.3 에서 C-3 를 골랐다면 행이 **늘어났을** 것이다(건설 목표 선택 분리) — 실제 결정은 C-1 이라 행 수 유지 | TBD 0 이 곧 행 수 유지는 아니다 |
| 5 | 막는 티켓 5건 (1767·1771·1773·1781·1783) | **1783 은 이미 안 막혀 있다** — 공동 테마 적용이 확정됐다. 1767·1781 도 GROMO-2000 으로 해소 | 이 표가 실제로 막는 것은 ~~4건~~ **2건**(1771·1773) |

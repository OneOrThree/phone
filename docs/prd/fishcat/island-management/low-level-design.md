...
| 강퇴 + active/paused | 새 섬 세션 연동 규칙 없음 | **확정 — 2026-09-18 재영님 결정(FR-D03 = B1): 강제 종료, 미정산.** 강퇴 TX 가 같은 잠금 아래 진행 세션을 서버 시각으로 원자 종료하고 그 시점까지의 물고기는 정산하지 않는다. ~~원자 종료 + 개인 확정 보상 보존 추천~~ 은 채택되지 않았다. 종결 사유는 `ABANDONED` 와 구분되는 값(예: `MEMBERSHIP_LOST`)으로 남긴다. 구현 GROMO-1924 |

...
| B14 | focusMembers.items[].appearanceVersion은 같은 snapshot의 user appearance.version을 직접 반환 | 승인된 필수 응답 확장. 개인 외양 축 (member.appearance,userId)이며 focus.member/session/island 외양 버전과 비교하지 않음. 병렬 조합에서는 `focus-members` 도메인 GET 한 번의 읽기 안에서 같은 값을 반환한다 |
...
| BG13 | **계정 전환·충돌 해소 계약이 없다** — 기획 `convert`(`POST /v1/me/auth-identity`)·`conflict`(`POST /v1/me/identity-conflicts/{id}/resolve`) | `account` 화면(프레임 80). [A24①](../../architecture/decisions.md)은 「계정 전환 보상」만 적었고 **API 계약은 어느 도메인 LLD 에도 없다**. 게스트→소셜 승격과 이미 그 소셜로 만든 계정이 있을 때의 충돌 처리가 정해져야 한다. 설계 전에는 해당 조각을 만들지 않는다 |
...
| BG17 | **건설 진행 상태 조회 계약이 없다** — 기획 `construction`(`GET /v1/islands/{islandId}/constructions/current`) | [island-construction LLD](../island-construction/low-level-design.md)의 `construction-options` 응답은 `selectedBuildingId`·옵션별 `selectable`·`buildable`·`blockedReason` 뿐이고 **진행 상태(`status`)가 없다** — `status` 는 POST `/constructions` 완료 응답에만 있다. `town-hall` 화면이 진행 중인 건설을 그리려면 도메인 계약에 필드를 더해야 한다 |

...
| BG11 | ~~모닥불 휴식 주민이 섬 홈 장면에 보이는지~~ **home `restMembers` 해소** — 2026-09-19 재영님 결정: 쉬는 멤버도 home 에 싣는다(GROMO-1897, 다른 조각과 같은 필수 조각). 남은 것: 다른 섬 도서관을 어디서 여는지 | `library`의 타 섬 경로. 확인 전 구현하지 않는다 |

...
| B11 |`/screens` 14종 외부 응답 TTL0, Cache-Control:no-store | B15 기준 개수. 내부 불변 snapshot/자산 재사용도 현재 인가 필요 |
...
| B17 | 항해(08·30·62)는 조회가 없다. 도착지 화면(`home`·`visit`·`focus`) 조회를 출발할 때 부른다. 모닥불 휴식(26)은 `home`이 `restMembers`를 함께 준다(BG11) — `home.restMembers`는 `GET /islands/{islandId}/rest-members`를 소비하는 조각이고 rest 전용 BFF 화면이 아니다(B28) | travel·rest 폐지 근거. 휴식은 원래 섬 모닥불(기획 확정 정책) |
...
| B22 | 실시간 구독은 [realtime-events LLD](../realtime-events/low-level-design.md) §3.1의 토픽 7개를 그대로 쓴다: `events`·`focus`·`rest`·`emotes`·`playback`·`messages`·`/user/queue/events` | 토픽이 인가 단위다(emotes는 active·paused 진행 세션, playback은 방송기, messages는 우체통 접근) |
...
| B27 | **`missingFragments` 는 정식 화면 계약이다.** ① **정의:** 화면이 실어야 할 조각인데 그 조각의 도메인 GET 이 **아직 구현되지 않은** 경우, 서버는 그 조각을 부르지 않고 응답 키를 **명시 `null`** 로 둔 뒤 조각 이름을 `data.missingFragments`(문자열 배열, 화면 코드의 조각 순서)에 싣는다. ② **키 유무:** 빠진 조각이 없는 응답에는 키 자체가 **없다** — 빈 배열 `[]` 을 보내지 않는다. 같은 화면이라도 분기에 따라 달라진다(`library` 미완공은 조각을 부를 일이 없어 키가 없고, 완공이면 두 기록 조각이 실린다). ③ **availability 와 다르다:** `…Availability`(B26 — `facility_locked`·`host_only`·`none`)는 서버가 **검증한 비적용**(B03 N)이라 앱이 그 사유 화면(잠김·방장 전용·신청 없음)을 그린다. `missingFragments` 는 **서버 미구현이라 비어 있음**이지 데이터 없음이 아니다 — 앱은 빈 목록·0·잠김으로 그리지 말고 「준비 중」 자리표시로 그린다. 한 조각이 두 상태를 겸하지 않는다: missing 조각의 `…Availability` 는 `available` 도 사유 값도 아닌 `null` 이다. ④ **제거:** 도메인 GET 이 머지되면 같은 PR 에서 그 조각을 병렬 조합에 넣고 `missingFragments` 에서 이름을 뺀다. 화면의 이름이 모두 빠지면 키도 사라진다. 앱은 키 부재를 「전부 실렸다」로 읽는다. ⑤ missing 은 실패가 아니다 — 화면은 200 이고 B04(선택 조각 없음)·B05 개정 대상이 아니다. 현재 화면별 목록은 [Business 구현](implementation-business-api.md) §4.1 | **2026-09-19 조재영 결정(N28).** GROMO-1897(PR #822·#823·#826·#829·#835)이 `ScreenReadUseCase.missing()` 으로 도입한 키를 앱이 분기 재료로 쓰도록 계약에 올린다. availability 로 위장하면 앱이 「잠김」·「비어 있음」을 사용자에게 사실처럼 보여 준다 |
| B28 | **rest 전용 BFF 화면은 없고, 도메인 rest 조회는 `home` 조각이 소비한다.** `GET /islands/{islandId}/rest-members`는 paused 세션을 가진 활성 주민을 읽는 전용 도메인 조회다. `home.restMembers`는 그 응답을 싣는 BFF 소비 필드이고, 별도 source of truth·쓰기·rest 전용 `/screens/rest`를 뜻하지 않는다. **세션 없는 공유 휴식**을 `restMembers`에 싣거나 rest 전용 조회를 부활하려면 planning-document/`rest-screen-spec.md` 개정과 별도 티켓이 먼저다 | GROMO-1958 정본 대조 · [focus-rest-session policy FR-P20](../focus-rest-session/policy.md) |
...
|rest(폐지)|session,restMembers|v0.3 이력. 현재는 `/screens/rest`가 없고 `home.restMembers`가 도메인 `rest-members` GET을 소비한다(B17·B28)|0|
...
| BG10 | ~~친구·편지 도메인 설계(레포에 LLD 없음)~~ → **해소: [친구·편지 설계](../friend-letter/)**(GROMO-1893 — 친구는 이미 구현돼 있었고 편지·요청 취소만 신규). ~~남은 것은 **가입 대기 신청 목록·공동 가계부·물고기 장 조회 계약**이며, 이 셋은 여전히 설계 전이라 해당 조각을 활성화하지 않는다.~~ → **해소: GROMO-1895** — 세 계약과 Data 내부 GET·허용목록이 들어갔다([Data 구현](implementation-data-api.md) §4). 화면 조합(1896/1897)이 이 GET 을 붙이면 조각을 켠다. 남은 BG10 항목은 작성자 표시 정보 batch 뿐이다. `mailbox`·`friends`·`raft` 의 친구·편지 조각은 friend-letter 설계의 내부 GET 과 허용목록이 들어간 뒤 켠다 |
| BG11 | ~~모닥불 휴식 주민이 섬 홈 장면에 보이는지~~ **home `restMembers` 해소** — 2026-09-19 재영님 결정: 쉬는 멤버도 home 에 싣는다(GROMO-1897, 다른 조각과 같은 필수 조각). ~~남은 것: 다른 섬 도서관을 어디서 여는지~~ → **해소: 열지 않음** — 2026-09-19 결정 V-읽기(GROMO-1904·1937). 도서관은 주민 전용이다 | ~~`library`의 타 섬 경로. 확인 전 구현하지 않는다~~ 타 섬 도서관 경로를 만들지 않는다. 방문자가 읽는 것은 주민 목록·게시판(공지·댓글·퀘스트)뿐이다 |
...
| BG17 | **해소(GROMO-1815)** — 기획 `construction`(`GET /v1/islands/{islandId}/constructions/current`)의 진행 상태는 별도 경로가 아니라 `construction-options.activeConstruction`으로 제공한다. `home`·`town-hall`·`board`는 공통 건설 옵션 조회의 정본 상태를 사용하며, `activeConstruction`은 없으면 명시적 `null`, 있으면 `BUILDING` 상태와 시작·완료·서버 시각 및 version을 담는다. 앱은 재실행·복귀 시 GET으로 복원하고 예정 시각 경과 뒤에도 서버 목록으로 완공을 확정한다 |
| BG18 | **해소(2026-09-19 재영님 결정 SH-BG18 — 구매 내역은 섬 귀속, 전체 이력 없음).** ~~섬 무관 전체 구매 이력 계약이 없다~~ — 기획 `my-orders`(`GET /v1/me/purchase-history`) | [island-shop LLD](../island-shop/low-level-design.md) §2.5 는 `/islands/{islandId}/shop/orders` 로 **한 섬 범위만** 정의한다. 여러 섬에서 산 개인 구매를 합친 이력이 없다. 화면(구매 내역, 프레임 74·75)이 전체를 보여줄지 섬 단위로 둘지 제품 결정이 필요하다 |

BG13~BG18 은 [기획 v1 대조표](planning-v1-mapping.md) §3 이 기획 계약 98개를 전수 대조하며 찾은 것으로, BG10 이 적어 둔 6종과 다른 집합이다. BG17 해소 뒤 미해결로 남은 다섯 항목은 기획 화면이 참조하지만 레포 도메인 LLD 에 계약이 없다.

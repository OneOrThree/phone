...
GROMO-1659(알림 서버 분리)·~~GROMO-1660(초대 링크 서버 분리)~~(→ A23(2026-09-13)으로 분리 범위 폐기 — 링크 원장은 Data 의 같은 DB)의 **공통 선행분**이다.
...
배치 선점 뒤 각 HTTP·Kafka 전송 직전에 `lease_token` 일치, 미완료, 미만료를 하나의 UPDATE로
대조하고 해당 행의 리스를 갱신한다. 앞 행의 지연으로 뒤 행이 만료·재선점됐으면 원 워커는
그 뒤 행을 전송하지 않는다. 아직 다른 워커가 선점하지 않았어도 만료된 토큰을 부활시키지 않는다.
이미 시작한 외부 호출이 갱신된 리스보다 오래 걸린 경우에는 수신 측 `eventId` 멱등과 위 완료
펜싱이 적용된다. 이 갱신은 진행 중인 HTTP·Kafka 호출의 취소나 exactly-once를 보장하지 않는다.

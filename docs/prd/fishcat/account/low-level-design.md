...
**2026-09-25 후속 제품 결정(RP-게스트쓰기):** 위 두 문단은 현재 구현 상태와 1992 당시 근거다. 2.0에서도 게스트 친구 요청·편지 등 커뮤니티 쓰기를 유지하고 소셜 로그인 대신 소유 확인된 이메일의 쓰기 자격 및 제재·차단 재결박을 적용한다. 1992 가드를 그대로 둔 채 이 정책이 구현됐다고 보지 않는다. 상점 구매의 소셜 로그인 가드는 유지한다. 상세 계약은 [신고센터 정책](../character-report/policy.md#rp-게스트쓰기--게스트-커뮤니티-쓰기와-제재-승계)을 따른다.

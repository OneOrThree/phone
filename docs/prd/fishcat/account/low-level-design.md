...
#### 게스트 제한과 기존 계정 충돌의 2단계 확인 (GROMO-1992 · 1994 구현)
...
**2026-09-25 후속 제품 결정(RP-게스트쓰기):** 위 두 문단은 현재 구현 상태와 1992 당시 근거다. 2.0에서도 게스트 친구 요청·편지 등 커뮤니티 쓰기를 추가 소유 확인 없이 유지한다. 1992 친구·편지 가드를 그대로 둔 채 이 정책이 구현됐다고 보지 않는다. 상점 구매의 소셜 로그인 가드는 유지한다. 새 게스트 발급 뒤 제재·차단 승계는 보장할 수 없는 미해결 위험이다. 상세 계약은 [신고센터 정책](../character-report/policy.md#rp-게스트쓰기--추가-확인-없는-게스트-커뮤니티-쓰기)을 따른다.

이 목록에서 **두 가지가 빠져 있고 전부 의도한 것이다.** ① **받은 친구 요청 수락**은 막지 않는다 — 정책이 막는 것은 게스트가 먼저 손을 내미는 명령이고, 받은 편지함·상세·닫기 같은 읽기·정리 표면도 같은 이유로 열어 둔다. ② **레거시 `POST /api/v1/friends/requests`**는 그대로 열어 둔다(동결된 1.x 앱 보존 · GROMO-1934 의 게스트 시간당 한도가 사는 유일한 축). 그 결과 **현재 구현에는 2.0 게스트 AT 로 레거시 경로를 부르면 1992의 회원 전용 가드를 지나지 않는 구멍**이 있다. RP-게스트쓰기에서는 친구 요청의 회원 전용 가드 자체를 제거하므로 이 구멍을 이메일 쓰기 자격 우회로 분류하지 않는다. 그러나 활성 계정의 제재 세대·양방향 차단 검사는 1.x와 2.0의 모든 친구 요청 진입점에서 공통 서비스의 커밋 직전 및 알림 전달 전에 적용해야 한다. 어느 한 경로라도 이 검사를 건너뛰면 현재 게스트 계정의 제재·차단 우회이므로 2.0 출시 조건을 충족하지 않는다. 새 게스트 발급으로 과거 계정 제재를 우회하는 별도 위험은 RP-게스트쓰기에 남는다.

**충돌의 2단계 확인(1994).** 「이미 다른 회원 계정에 연결된 소셜 계정이면 기존 회원 계정을 우선하되… 명시적으로 확인받는다」를 **상태를 새로 만들지 않고** 두 번의 `POST /auth/sessions` 로 구현한다.

| 단계 | 요청 | 서버 |
| --- | --- | --- |
| ① 안내 | 평소 로그인(게스트 AT + 제공자 자격) | 기존 409 `SOCIAL_ACCOUNT_ALREADY_LINKED`. 게스트 상태·데이터 그대로. 전용 코드를 새로 만들지 않았다 — 이 코드가 이미 「이 소셜은 남의 계정」을 뜻하고, 앱은 이 하나로 전환 확인 다이얼로그를 띄운다 |
| ② 확정 | 같은 자격 + `"accountSwitchConfirmed": true` + **새 `X-Login-Attempt-Id`** | 게스트를 기존 탈퇴 경로로 폐기한 뒤, 선택 AT 없이 대상 회원 계정으로 로그인 |

`accountSwitchConfirmed` 는 선택 boolean 이고 생략·`null` 은 `false` 다(문자열 `"true"`·`1` 은 400 — 결말이 되돌릴 수 없어 가장 좁게 읽는다). 기존 4필드를 바꾸지 않는 additive 확장이라 이 값을 모르는 앱은 예전처럼 ①에서 멈춘다. **자격 digest 에는 넣지 않는다** — digest 는 「원 자격을 들고 있다」의 증거이고 이 값은 자격이 아니라 의도다. 의도가 바뀌었으므로 앱은 §1 의 「수정한 사용자 의도에는 새 키를 쓴다」대로 **새 attempt id** 를 쓴다. 확인했는데 충돌이 실제로 없으면 이 값은 아무 일도 하지 않고 평소 승격/신규 가입이다(충돌 여부를 `AuthService.isLinkedToAnotherUser` 로 **폐기 전에** 확정한다 — 이 순서가 빠지면 게스트 데이터를 버리고 빈 신규 계정을 만든다).

「게스트의 고양이·섬 소속·개인 집중 기록은 폐기한다」의 **폐기는 탈퇴**다(`AccountWithdrawalService.withdraw`). 같은 문단의 「주민이 있는 섬의 게스트 방장은 방장 위임 또는 섬 정리를 끝내고, 진행 중인 집중 세션도 종료한 뒤 전환한다」가 이미 그 메서드의 계약이기 때문이다 — 방장이면 400 `HOST_WITHDRAW` 로 **전환 자체가 거절**되고 그 TX 가 통째로 롤백되어 아무것도 파기되지 않으며, 진행 중 집중 세션은 같은 TX 가 종결하고, 섬에 적립된 공동 물고기·주문 원장은 손대지 않는다(「이미 섬에 적립된 공동 물고기와 공동 거래 기록은 되돌리지 않는다」). 판정을 다시 쓰지 않은 이유가 그것이다.

전환은 **승격 경로에 가지를 내지 않는다**. 탈퇴가 커밋되면 그 게스트는 활성 게스트가 아니므로 `loginOrRegister` 의 `findActiveGuestByIdForUpdate` 가 비고 「이미 있는 소셜 계정으로 로그인」 분기를 그대로 탄다. 이때 **선택 AT 를 넘기지 않는 것이 필수**다 — 탈퇴가 그 세션을 폐기하고 `authGeneration` 을 올렸으므로 넘기면 자기가 만든 상태 때문에 401 이 된다. 선택 AT 의 세션 폐기 관문은 그보다 먼저 `gateOptionalSession` 이 본다.

**알려진 천장 둘.** ⓐ `withdraw` 의 마지막 단계가 소셜 벌크 DELETE 라 영속성 컨텍스트를 비워 로그인과 한 TX 에 담을 수 없다. 그래서 「탈퇴 커밋 → 로그인」 두 조각이고 **탈퇴만 커밋된 채 로그인이 실패하는 창**이 남는다. 그때 잃는 것은 이미 폐기에 동의한 게스트 데이터뿐이고 대상 회원 계정은 그대로라 선택 AT 없는 새 로그인으로 복구된다. 순서를 뒤집으면 「전환은 됐는데 게스트가 유령 주민으로 남는」 되돌릴 수 없는 쪽으로 깨진다. 재개 요청은 활성 게스트 검사가 걸러 탈퇴를 건너뛰고 로그인만 이어 한다. ⓑ 그 창에서 같은 attempt 로 재시도하면 `gateOptionalSession` 이 이미 탈퇴한 주체를 404 `USER_NOT_FOUND` 로 먼저 거절한다 — 앱의 답은 로컬 자격을 비우고 선택 AT 없이 다시 로그인하는 것이다.

**서버와 앱의 경계.** 서버가 책임지는 것은 여기까지다 — 게스트 계정 폐기, 그 계정의 **전 세션·bootstrap 폐기와 `authGeneration` 상승**, 기기 삭제 outbox(전부 `withdraw` 안에서 일어난다). 서버는 새 세션의 기기 등록을 대신하지 않는다. 앞 절 「계정·로그인 세션 전환의 이전 기기 정리」가 앱에 요구하는 **FCM 토큰 교체·B 세션 bootstrap 으로의 재등록·`deliveryTag` 대조**는 이 구현에 **없다**(등록 응답에 `deliveryTag` 자체가 아직 없다). 그 절의 활성 조건은 그대로 미충족이며, 이 2단계 확인이 그것을 해결했다고 주장하지 않는다.
...
**전환 전에 발송된 푸시의 수신 차단.** DELETE와 `auth.session.revoked`는 **이후 발송**만 막는다. 같은 FCM 토큰으로 A→B를 전환할 때 A 등록이 유효한 순간 발송돼 FCM/APNs에 대기 중인 알림은 B 재등록 뒤에도 도착한다. 기준 main 서버 `FcmPushNotificationClient`는 표시 알림을 `notification`(title/body) 페이로드로 보내 앱이 백그라운드·종료 상태면 OS가 앱 코드 없이 배너를 띄우고, 앱 `push.ts`는 포그라운드 `onMessage`·배너 탭 `onNotificationOpenedApp`·콜드스타트 `getInitialNotification`에서 대상 사용자·세션을 확인하지 않고 `saveToInbox`·로컬 배너·딥링크를 실행한다. 그래서 이 경합에서는 B가 A의 제목·본문을 본다.
...
후속 구현은 **OS가 앱 검증 없이 표시할 수 있는 원격 payload**(iOS alert, Android notification 블록)에 계정 연계 제목·본문을 **항상 싣지 않고** 비식별 일반 문구만 담는다. 계정 연계 내용은 앱 또는 `mutable-content`·App Group을 갖춘 iOS NSE가 현재 저장 태그와 대조해 일치할 때만 서버에서 조회해 보여 준다. NSE는 이 검증·조회에 성공했을 때만 표시 내용을 바꾸는 보강이며, NSE가 실행되지 않거나 제한 시간 안에 끝나지 않거나 조회가 실패하면 OS는 원격 payload의 비식별 문구를 그대로 표시한다. 실행 시간 제약 검증만으로 모든 NSE 실패를 없앨 수 없으므로, 원격 payload에 원문을 싣고 NSE가 가리는 설계는 채택하지 않는다. Android data-only 메시지도 OS 자동 표시가 없는 경우에만 앱 핸들러의 검증 뒤 로컬 표시로 쓰며 표시 블록에 원문을 싣지 않는다. 이 경로들의 iOS 백그라운드 전달·NSE 실행을 실제 기기에서 검증해야 하며, 노출 0이 확인되기 전에는 토큰 교체·재등록을 포함한 새 전환 흐름을 활성화하지 않는다. 전환 시 기존 표시 알림은 로컬에서 모두 지운다. 이 조건은 새 흐름의 활성 조건이며 현재 앱의 기존 노출을 해결했다고 주장하지 않는다.
...
아래 '현재'는 main 증거다. '추가'는 1757 및 선행 1659 통합에서 검증해야 하는 변경이다. user 행과 일부 관계·정산 근거는 남으므로 이 설계를 전체 데이터의 물리 삭제나 복원 불가능한 완전 익명화라고 부르지 않는다. 로그·분석 자료의 보존 기간만 [policy.md 로그·분석 보존 기간](policy.md#로그분석-보존-기간) 표가 정하며, 그 밖의 보존 기간은 새로 약속하지 않는다.
...
| user_blocks.blocker_id/blocked_id/created_at | 양쪽 NOT NULL users FK. 현재 탈퇴 정리 호출 없음; 차단 writer는 아직 미구현 | blocker 또는 blocked가 탈퇴자인 원 `user_blocks` 행은 모두 같은 TX에서 hard delete. 다만 신고센터 RP-차단의 재가입 승계용 별도 keyed 방향성 관계 표지는 원행 삭제 전에 내구 확정하고 차단 해제·차단자 탈퇴 때 파기. 원행을 남기거나 한 방향만 삭제하지 않음 |
...
| personal_appearances.user_id/clothes/decor/position/version | V64 유저당 1행, user_id PK·users FK | 같은 탈퇴 TX에서 본인 행 hard delete(GROMO-1950). 개인 외양 PATCH는 users 배타 잠금부터 잡아 탈퇴와 직렬화되고 탈퇴 뒤엔 404라 재생성되지 않음. 섬 외양(island_appearances)은 섬 자산이라 보존 |
| owned_products(owner_type='user').user_id/product_id/granted_ref/granted_at | V64 개인·섬 소유 공용 테이블. 지급 writer(상점 1781)는 아직 없음 | owner_type='user'인 본인 행만 hard delete(GROMO-1950). owner_type='island' 행은 user_id가 없고(CHECK) 섬 자산이라 보존. 구매 원장 shop_orders.payer_user_id는 이 행에 포함하지 않음 |
...
`getCallerForUpdate` → authGeneration/세션 폐기 및 필요한 위성 명령과 랭킹·chat/realtime 대상 user.withdrawn outbox 내구 기록(로그·GA4는 보존 기간 만료로 파기하므로 작업을 두지 않음) → 그룹 조건·내기 해제 환불·증거 동결·멤버십 이탈과 개인 설정 초기화 → bet participant 열람/표시 lease 3필드 nullify → group_challenge_members 원본 보고 파기 → 집중/통계/스크린타임 귀속 및 user_focus_tags 사용자/직군 연결 파기·group_announcements.user_id nullify → notification_sent_logs 수신자·사용자 상대 이력 파기 → 일간 리그 snapshot 삭제·주간 리그 개인 결과 파기/최소 정산 완료 마커 분리 → character_generation 본인 생성 이력 및 character_equipment 사용자 장착 행 삭제 → 개인 외양·개인 소유 보유품 삭제와 퀘스트 cohort 삭제·정산 수령자·퀘스트 작성자 null(GROMO-1950·1952) → 지갑·설정 삭제 → 본인 group_invite_links.inviter_id 비식별화·claim 클릭의 기기/GA4/IP 해시/UA 식별자 파기·차단 대상의 재가입용 최소 keyed 관계 표지 및 유효 신고 토큰·미결/재심 사건·활성/누진 제재의 안정적 주체와 모든 연결 제공자 digest 표지를 같은 TX에 내구 확정한 뒤 양방향 group_invites·원 user_blocks·friendships(status·deleted_at 무관)·pinned_users 및 본인 user_streaks 삭제와 신규 개인자료 정리 → user 직접 PII null 및 soft delete → socialAccounts bulk delete 순서를 유지한다. 중간 실패는 전체 rollback이다.
...
APP의 MDC user_id와 별도 user-activity 파일·일별 회전본을 갖고, APP 로그는 CONSOLE(stdout)로도 나가
Docker 컨테이너 로그·Datadog·dev Loki에 복사된다.
...
호스트 로그 바인딩(`/var/log/springboot`)과 S3 적재·Datadog 전송 경로를 설명한다.

**결정(재영님, 2026-09-19): 이 자료는 행 단위 삭제가 아니라 보존 기간 만료로 파기한다.** 탈퇴 시점에 파일·객체·외부
저장소에서 해당 사용자의 레코드를 찾아 지우거나 비식별화하지 않고, 중앙 탈퇴 TX에 로그 파기 작업이나 sink 폐기 fence를
...
**GA4/Firebase 분석 자료도 같은 방식이다.** 기준 main 앱의 `UserContext`는 사용자 UUID를 GA4 User-ID로 설정하고 설치
device_id를 모든 이벤트 공통 파라미터로 싣는다. 서버 `Ga4MeasurementClientImpl`은 `app_instance_id`로 초대 매치·가입
이벤트를 앱스트림에 보낸다. 이 연결은 GA4 사용자 삭제 요청 대신 **GA4 속성의 데이터 보존 기간 만료**로 파기한다.
보존 값은 policy.md 표(권장 2개월)이며 GA4 관리 화면에서 설정하므로 저장소 코드로 강제되지 않는다. 설정값이 확인되기
전에는 GA4 파기 완료를 주장하지 않는다. 앱은 탈퇴 성공 뒤 User-ID를 해제한 다음에만 이벤트를 보내 탈퇴 뒤의 새 사용자
연결로 만료가 늦어지지 않게 한다(현행처럼 User-ID가 붙은 채 `withdrawal_confirmed`를 보내는 순서는 고친다).

이전 판의 로그 행 단위 파기·sink 폐기 fence·GA4 사용자 삭제 요청·삭제 키 전용 앱 인스턴스 등록부·공유 식별자 판정
설계는 이 결정으로 대체한다.
...
| 같은 토큰 A→B 전환 직전 A 발송 푸시가 포그라운드·배너 탭·콜드스타트·백그라운드 data로 도착, 태그 없는 legacy 푸시, 같은 사용자 s1→s2 | A≠B면 토큰 교체 뒤 대기분 전달0, 태그 불일치 푸시의 보관함 저장·로컬 배너·딥링크·flush 0, 전환 시 기존 표시 알림 제거, OS 자동 표시 경로도 A 내용 노출0(원격 표시 payload는 항상 비식별 문구, NSE 미실행·시간 초과·조회 실패에도 원문 표시0), 확인 전 새 전환 흐름 비활성 |
| iOS NSE 미실행·제한 시간 초과·App Group 태그 조회 실패·내용 조회 실패 | OS가 비식별 문구만 표시, 계정 연계 제목·본문 노출0, 검증·조회 성공 시에만 내용 표시 |
...
| claim 클릭의 matched_device_id·app_instance_id·ip_hash·user_agent·Link 위성/이관 복사본·같은 기기 재시도 | 탈퇴자 클릭 식별자0·matched/claimed_at 보존, 재귀속·추가 보상0, 이관 대조가 파기 필드를 되살리지 않음·중간 실패 전체 rollback |

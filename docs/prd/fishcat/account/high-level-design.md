...
JWT 서명 키 교체는 로그인 복구 창과 별개다. 새 발급은 곧바로 새 키로 하되, 이전 키는 그 키로 발급된 AT/RT의 최대 만료(prod RT 30일·게스트 90일 등 환경 설정)까지 검증 전용으로 남겨 기존 세션을 끊지 않는다. 더 일찍 없애야 하면 전 세션 폐기 롤아웃으로 명시한다.
...
전환 전에 발송돼 대기 중인 푸시는 DELETE·세션 폐기로 막히지 않는다. 사용자가 바뀌는 전환은 재등록 전에 FCM 토큰을 교체하고, 등록마다 다른 `deliveryTag`를 푸시 data에 실어 앱이 보관함 저장·로컬 배너·딥링크·flush 직전 현재 태그와 대조한다. OS 자동 표시 경로는 회수할 수 없으므로 원격 표시 payload에는 항상 비식별 문구만 싣고 계정 연계 내용은 태그 검증 뒤 조회로만 보여 준다. NSE는 보강일 뿐이라 실행 실패·시간 초과에도 비식별 문구가 표시되며, 노출 0을 실제 기기에서 확인하기 전 새 전환 흐름을 활성화하지 않는다.
...
보존한 내기 결과뿐 아니라 진행 중 회차의 개설자·참가자 명단을 그룹원에게 보여 줄 때도 탈퇴자 행의 공개 userId는 null(목록 key는 회차별 참가 행 id 같은 비연계 값)로 치환한다. 명단 수·pot·판정·payout은 정산 사실로 유지하고 내부 중복 지급 방지는 기존 user_id 참조를 그대로 쓴다.
...
사용자 활동 로그도 정상적으로 UUID를 기록한다. 이 로그와 회전본·S3 적재본·외부 sink 복사본은 행 단위로 지우지 않고 **보존 기간 만료로 파기**한다(재영님 결정 2026-09-19). 기간 정본은 policy.md 로그·분석 보존 기간 표이며 로컬 logback maxHistory(APP 7일·user-activity 30일)와 S3 수명 주기 규칙(90일)이 각각 표의 자기 행을 구현한다. 기간이 없거나 설정 미확인인 sink는 파기 완료를 주장하지 않는다.
...
보존한 과거 메시지의 공개 응답에서는 탈퇴 발신자의 `senderId`를 null로 치환한다. 본문과 내부 `sender_id` 보존은 그대로이고, 사용자별 대체 식별자는 만들지 않는다.
...
알림 서버가 아직 적용하지 못하면 outbox에 넣었다는 이유만으로 성공을 반환하지 않는다. 동일 키 재시도는 같은 명령을 재전달한다. 완료 표시만 실패했으면 relay 재전달이 멱등 처리된다. 이미 적용된 false 명령을 재시도하는 사이 최신 설정이 true로 바뀌었다면 재시도 응답은 원 결과 false가 아니라 응답 시점 정본 현재 값 true다(LLD §2.7). APPLIED/SUPERSEDED 구분은 재적용 판정에만 쓴다. 버전은 모든 설정 필드에 공통 순서를 부여하되 적용 여부는 선택 필드마다 비교한다.
...
5. 알림 부분 명령·field mask·필드별 version을 양쪽 서비스에 연결한다. 이관 정지 창에서 기존 전역 version 명령을 drain한 장벽의 마지막 적용 version으로 각 필드 version을 seed하고 allocator는 그보다 큰 값부터 발급한다.

...
로그·분석 보존 기간 결정(GROMO-1942, 2026-09-19): 탈퇴자의 로그·S3 적재본·외부 sink 복사본과 GA4 연결은 행 단위로 지우지 않고 보존 기간 만료로 파기한다. 기간 정본은 policy.md 로그·분석 보존 기간 표이며 logback maxHistory와 S3 수명 주기 규칙이 같은 값을 구현한다. 위 보완들의 sink fence·GA4 삭제 입력·앱 인스턴스 등록부·공유 식별자 판정은 이 결정으로 대체됐다.

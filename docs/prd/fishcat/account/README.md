...
진행 중 내기·잔존 설정 규칙 보완: 탈퇴자 UUID 치환을 정산 결과 변환만이 아니라 진행 중 회차의 creatorUserId·참가자·세션 참가자까지 공통 출력 지점에서 적용한다. HLD와 검증 표에 남아 있던 설정 재시도의 원 결과 재생 문장도 응답 시점 정본 현재 값 계약으로 맞췄다.

전환 전 푸시·직접 참가 식별자 보완: DELETE·세션 폐기가 막지 못하는 전환 전 발송 푸시를 사용자 변경 시 FCM 토큰 교체와 등록별 deliveryTag 대조로 막고, OS 자동 표시 경로는 다음 보완에서 새 전환 흐름의 활성 조건으로 올렸다. 서버 GA4 MP가 user_id 없이 보내는 app_instance_id는 가입 TX에서 삭제 키 전용 등록부에 남겨 탈퇴 삭제 작업 입력으로 옮긴다.

OS 자동 표시·공유 식별자 보완: OS가 자동 표시하는 푸시 경로의 계정 간 노출 0을 새 전환 흐름의 활성 조건으로 올렸다(기준 NSE는 템플릿 그대로). 설치 단위 app_instance_id는 계정 전환마다 재설정하고, 여러 사용자에게 기록된 공유 값은 GA4 삭제 입력에서 빼 다른 계정 자료를 지우지 않는다.

원격 payload·식별자 세대 보완: OS가 앱 검증 없이 표시할 수 있는 원격 payload에는 항상 비식별 문구만 싣고 계정 내용은 태그 검증 뒤 조회로만 보여 주어, NSE가 실행되지 않거나 시간을 넘겨도 원문이 노출되지 않게 했다. GA4 삭제는 재설정 세대 표지와 함께 처음 기록된 탈퇴자 전용 app_instance_id로만 하고, 표지 없는 legacy 값은 등록부 도입 전 공유 가능성 때문에 삭제하지 않는다.

JWT 키·설정 이관·채팅 발신자 보완: JWT 검증 키는 로그인 복구 창과 분리해 발급 AT/RT의 최대 만료까지 유지하고, 조기 제거는 전 세션 폐기 롤아웃으로만 한다. 설정의 필드별 version은 기존 전역 명령을 drain한 장벽의 마지막 적용 version으로 seed해 늦은 legacy PUT이 정본을 덮지 않게 했다. 보존 채팅 메시지의 공개 응답은 탈퇴 발신자 senderId를 null로 치환한다.

로그·분석 보존 기간 결정(GROMO-1942, 2026-09-19): 탈퇴자의 로그·S3 적재본·외부 sink 복사본과 GA4 연결은 행 단위로 지우지 않고 보존 기간 만료로 파기한다. 기간 정본은 policy.md 로그·분석 보존 기간 표이며 로컬 logback maxHistory(APP 7일·user-activity 30일)와 S3 수명 주기 규칙(90일)이 각각 표의 자기 행을 구현한다. 위 보완들의 sink fence·GA4 삭제 입력·앱 인스턴스 등록부·공유 식별자 판정은 이 결정으로 대체됐다.

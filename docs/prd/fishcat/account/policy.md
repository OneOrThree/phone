...
- 서버가 `gromo`와 `focuscat`을 모두 Apple 허용 audience로 검증하기 전에는 iOS 2.0 전체 출시를 보류한다(A23 출시 선행 조건). 애플 버튼만 숨기고 출시하는 것은 Apple만 연결된 기존 사용자의 재진입 경로를 없애므로 허용하지 않는다.

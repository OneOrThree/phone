...
| A23 | 2.0 로그인 화면은 지역별로 노출 조합을 나눈다. `KR`에서는 카카오·애플·구글, 그 밖의 지역이나 판정 불가에서는 라인·애플·구글을 노출한다. 판정은 기기 로케일의 `regionCode`만 사용하고 스토어 국가·IP·GPS는 쓰지 않는다. iOS는 표의 순서로 표시하고 Android는 애플을 제외한다(GROMO-998 유지). 게스트는 모든 지역·플랫폼에서 제공자 버튼 아래 보조 액션으로 유지한다. 적용 범위는 2.0 화면뿐이며 동결된 1.x(`gromo`)는 기존 플랫폼별 노출을 유지한다. 애플 버튼은 서버 허용 audience 목록화 전까지 켜지 않는다. 국가 이동으로 가입 제공자가 기본 화면에서 사라질 수 있는 한계는 현재 수용하고, 계정 수단 변경·복구를 도입할 때 재검토한다. | GROMO-1967 · 2026-09-24 최종 결정. `regionCode` 기준의 한국/한국 외 노출 표와 게스트 유지 정책을 확정했다. 선행 조건은 `apple.client-id`를 `gromo`와 `focuscat`을 모두 허용하는 audience 목록으로 바꾸는 별도 티켓이다 |
...
**게스트 로그인은 유지한다.** 제공자 버튼 묶음 아래에 보조 액션(텍스트)으로 두고, 모든 국가·플랫폼에서 제공한다. 소셜 로그인을 요구하는 시점은 기존 정책 그대로 친구 추가·편지·상점 구매의 첫 시도다.

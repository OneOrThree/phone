...
| LOGIN-D01 | 2.0 로그인 화면은 지역별로 노출 조합을 나눈다. `KR`에서는 카카오·애플·구글, 그 밖의 지역이나 판정 불가에서는 라인·애플·구글을 노출한다. 판정은 기기 로케일의 `regionCode`만 사용하고 스토어 국가·IP·GPS는 쓰지 않는다. iOS는 표의 순서로 표시하고 Android는 애플을 제외한다(GROMO-998 유지). 게스트는 모든 지역·플랫폼에서 제공자 버튼 아래 보조 액션으로 유지한다. 적용 범위는 2.0 화면뿐이며 동결된 1.x(`gromo`)는 기존 플랫폼별 노출을 유지한다. 서버가 `gromo`와 `focuscat`을 모두 Apple 허용 audience로 검증하기 전에는 애플 버튼만 끄는 것으로 대신하지 않고 iOS 2.0 전체 출시를 보류한다. 국가 이동으로 가입 제공자가 기본 화면에서 사라질 수 있는 한계는 현재 수용하고, 계정 수단 변경·복구를 도입할 때 재검토한다. | GROMO-1967 · 2026-09-24 최종 결정. `regionCode` 기준의 한국/한국 외 노출 표와 게스트 유지 정책을 확정했다. 출시 선행 조건은 `apple.client-id`를 `gromo`와 `focuscat`을 모두 허용하는 audience 목록으로 바꾸는 별도 티켓이다 |
...
LOGIN-D01의 화면 표다. 2.0 로그인 화면에만 적용하며 동결된 1.x 는 기존 노출을 유지한다.

**국가 판정 기준**: 기기 로케일의 `regionCode`를 쓴다. `KR`이면 한국, 그 밖의 값이나 판정 불가이면 한국 외로 처리한다. 스토어 국가·IP·GPS는 쓰지 않는다.

| 국가 구분 | 플랫폼 | 노출 조합 | 버튼 순서 |
| --- | --- | --- | --- |
| 한국 (`KR`) | iOS | 카카오 · 애플 · 구글 | 카카오 → 애플 → 구글 |
| 한국 (`KR`) | 안드로이드 | 카카오 · 구글 | 카카오 → 구글 |
| 한국 외·판정 불가 | iOS | 라인 · 애플 · 구글 | 라인 → 애플 → 구글 |
| 한국 외·판정 불가 | 안드로이드 | 라인 · 구글 | 라인 → 구글 |

- 국가에 따라 제공자 조합과 순서를 함께 가른다. 한국은 카카오, 한국 외는 라인을 노출한다.
- 안드로이드에서는 애플을 노출하지 않는다(GROMO-998 유지).
- iOS의 애플 버튼은 첫 화면에서 다른 로그인 버튼과 같은 크기로 표시한다.
- 서버가 `gromo`와 `focuscat`을 모두 Apple 허용 audience로 검증하기 전에는 iOS 2.0 전체 출시를 보류한다(LOGIN-D01 출시 선행 조건). 애플 버튼만 숨기고 출시하는 것은 Apple만 연결된 기존 사용자의 재진입 경로를 없애므로 허용하지 않는다.
- 적용 범위는 2.0 화면뿐이다. 1.x `gromo`는 기존 정책( iOS 카카오·애플·구글 / Android 카카오·구글)을 유지한다.

**게스트 로그인은 유지한다.** 제공자 버튼 묶음 아래에 보조 액션(텍스트)으로 두고, 모든 국가·플랫폼에서 제공한다. 친구·편지·상점 등 로그인 뒤 기능의 게스트 허용 범위는 각 도메인 계약을 따르며, LOGIN-D01은 그 범위를 바꾸지 않는다.

2026-09-25 [RP-게스트쓰기](../character-report/policy.md#rp-게스트쓰기--추가-확인-없는-게스트-커뮤니티-쓰기)는 2.0 게스트의 친구 요청·편지 등 커뮤니티 쓰기를 소셜·이메일·전화번호 확인 없이 유지한다. 현행 1992 서버 가드는 이 목표와 다르다. 게스트 새 발급 뒤 제재·차단 승계는 보장되지 않는 미해결 위험이다. 상점 구매의 소셜 로그인 조건은 유지한다.

**알려진 제약**: 계정당 로그인 수단을 하나만 연결하고 수단 변경을 지원하지 않으므로, 한국에서 카카오로 가입한 사용자가 한국 외 로케일로 바꾸거나 한국 외에서 라인으로 가입한 사용자가 한국 로케일로 바꾸면 해당 버튼이 기본 화면에서 사라질 수 있다. 이 국가 이동 잠금은 현재 한계로 수용하며, 계정 복구·수단 변경을 도입할 때 정책을 다시 개정한다.

...
| Q07 | iOS 애플 가입자가 안드로이드로 옮겼을 때의 재진입 경로 | A23 이 안드로이드 애플 미노출(GROMO-998)을 유지하므로, 수단이 하나뿐이고 변경·추가·해제가 없는 계정에 들어갈 방법이 없다. 게스트 승격은 다른 계정 생성이지 복구가 아니다. 선택지는 ① 안드로이드 Apple 웹 OAuth(Services ID + `expo-auth-session`) 구현 ② 문의 경로의 운영 수동 전환 — 「로그인 수단 변경 불가」의 예외를 좁게 여는 정책 개정이 함께 필요하다 ③ 한계로 받아들이고 두기. 임의로 고르지 않는다. 범위는 iOS→안드로이드 이동자로 한정되며 국가 축의 잠금과 달리 예측 가능한 집합이다 |

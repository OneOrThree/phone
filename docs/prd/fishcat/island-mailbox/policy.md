...
신고·사용자 차단·제재는 [신고센터 정책](../character-report/policy.md)의 RP-일반신고·RP-표시차단·RP-누진제재를 따른다. 차단자 화면에서 상대 채팅을 숨기고 제재 중 쓰기를 제한하는 정책은 아래 M12의 집중·휴식 상태에 따른 허용과 별개의 검사다. 차단만으로 상대의 채팅 발송을 거절하지 않는다. 이 문서 연결이 해당 기능의 구현 완료를 뜻하지 않는다.

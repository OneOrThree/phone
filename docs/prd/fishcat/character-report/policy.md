...
이 문서는 2026-09-22 브레인스토밍의 확정 정책(RP)과 나머지를 구체화한 설계 초안(D)을 담는다. **정책 수립 중이며 구현 완료나 티켓 완료를 의미하지 않는다.** 결정 로그와 어긋나면 결정 로그가 우선한다.
...
- 긴급 신고는 별도 알림으로 우선 확인한다. 일반 신고의 정기 검토일까지 대기시키는 정책은 아니다. 담당자·알림 수단·긴급 분류와 대응 절차는 후속 결정한다.
...
### RP-증거기간 — 일반 신고 결과 통지 후 30일

- 일반 신고의 원문 증거는 처리 결과 통지 후 30일 보존하고, 기간이 끝나면 삭제한다. 접수일 기준 30일로 계산하지 않는다.
...
- 이 결정은 원문 증거의 기간에 한한다. 제재 이력·처리 메타데이터의 보존 기간, 미처리 사건의 방치 방지, 통지 실패·재심 시 기산점, 백업을 포함한 실제 파기 범위는 후속 설계에서 정한다.
...
- 초기화 대상은 누진 제재 단계다. 이 결정이 신고 원문 증거를 6개월 보존하라는 의미는 아니다. 원문 증거는 RP-증거기간의 결과 통지 후 30일을 따른다.
...
- 일반 사건의 증거 사본은 **처리 결과 통지 후 30일** 보존한 뒤 파기한다. 조사·분쟁·법적 보존 예외는 RP-증거기간을 따른다. 일반 게시글·채팅 전체의 보관 기간을 30일로 바꾸는 결정은 아니다.
...
전체 텍스트 신고의 서버 원문 사본 보존과 신규 이미지 저장소 미도입은 RP-통합증거에서 확정했다. 아래 최소 메타데이터·서비스 계약·실패 처리의 구체적인 설계는 제안이다. 입력은 대상 종류·ID·가능한 경우 버전·사유·설명이며, 클라이언트가 보낸 원문을 서버 원본으로 믿지 않는다. 채팅은 원 서비스가 권한·원문을 확인하는 계약으로 가져온다.
...
- 대기 중 친구 요청은 차단 중 수락할 수 없게 하고 새 알림도 만들지 않는다. 기존 미확인 편지는 차단 중 숨기고 상세 접근을 막되, 차단만으로 삭제하지 않는다. 이미 신고한 증거는 유지한다.
...
역할은 주 담당 1명·대체 담당 1명으로 제안하고, 실제 사람을 임의 배정하지 않는다. 일반 검토일은 주 2회라는 확정 기준에 맞춰 월·목을 초안으로 두며 팀 일정으로 변경할 수 있다. 긴급 건은 정기 검토일 밖에서도 담당 알림·대체 전달 경로를 둔다. 공개 24시간 처리 약속은 하지 않는다.
...
flowchart TD
    ...
    K --> L
    L --> O{유효한 보존 예외가 있는가}
    O -->|아니오| M[일반 원문 통지 후 30일 만료 시 파기]
...
접수 확인은 화면에 즉시 표시하고 결과는 서버의 앱 내 수신함에 영구적이지 않은 기록으로 남기는 안이다. 푸시는 내용 없는 알림 보조 수단으로만 쓴다. 수신함 기록 성공 시점을 결과 통지 시각으로 잡고 사용자 열람을 기다려 보존 기간을 늘리지 않는다. 외부 푸시 실패·재전송도 같은 결과를 재사용한다.
...
- 이의제기는 결과 통지 후 30일 이내 앱 내 본인 사건 화면에서 설명을 제출하는 안이다. 제재 중에도 접근 가능해야 한다. 원문이 남아 있는 동안 접수된 재심은 분쟁 중 보존 예외로 관리하고 담당·검토일을 둔다.
...
| --- | --- | --- |
| 일반 신고 원문·자유 설명·불필요한 프로필 사본 | 최초 결과 통지 후 30일 만료, 매일 파기 작업. 화면·API 조회는 만료 즉시 차단. 결과 재전송으로 연장하지 않음 | RP-증거기간 |
...
| 항목 | 현재 상태 | 다음 처리 |
| --- | --- | --- |
| 사용자 핵심 결정 RP 13건 | 결정 로그·정책 반영 | 변경 시 새 결정으로 이력 보존 |
...
정책 초안은 티켓의 6개 주제를 모두 다룬다. **문서 작성, 제품 정책 승인, 실제 구현·출시는 서로 다른 완료 단계**이며 현재는 확정 RP 결정과 검토 가능한 나머지 초안이 준비된 상태다.

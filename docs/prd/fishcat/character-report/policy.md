...
티켓 본문의 1.x 이미지 공유 전제는 2026-09-20 안수빈 댓글 #11163의 2.0 범위로 대체한다. 신고 대상은 게시판 공지·댓글, 섬 채팅, 친구 편지, 닉네임, 섬 이름·소개다. 누끼 캐릭터 공유와 이미지 증거 보존 A/B/C 선택지는 이번 범위에서 제외한다.
...
- 긴급 사안과 CSAM 대응은 별도 절차를 정한다. 일반 검토 대기 규칙을 그대로 적용한다고 간주하지 않는다.

근거: 본 대화의 두 번째 A 선택. 집단 허위 신고로 정상 콘텐츠가 전체에서 내려가는 것을 방지한다.

### RP-차단 — 직접 연락 차단과 상대 콘텐츠 숨김

2026-09-24 RP-표시차단이 아래의 **양방향 발송·요청 금지** 부분을 대체한다. 차단자 화면의 상대 콘텐츠 숨김은 유지한다.

- 한쪽이 차단하면 양방향 편지 발송과 친구 요청을 막는다.
- 차단한 사람의 화면에서 상대가 작성한 섬 채팅·댓글·공지를 숨긴다. 직접 연락만 차단하는 B안은 채택하지 않는다.
...
- 차단 해제, 기존 친구 관계·받아 둔 편지·대기 중 요청 처리, 상대방 화면의 노출, 알림·미리보기·프로필의 세부 처리는 아직 정하지 않았다.
...
2026-09-24 최신 `main` 대조: GROMO-1975의 차단 API·친구 목록/검색·받은 편지함 표시 필터는 이 정책의 **일부 구현**이다. 앱 차단 UI와 나머지 화면의 숨김은 후속 티켓에서 연결한다.

### RP-표시차단 — 차단자 화면에서 숨김, 발송은 허용 (2026-09-24 개정)

- `GET /blocks`의 사용자 ID 목록을 앱에서 받아 상대 ID와 대조한다. 이미 서버가 필터링하는 친구 목록·검색·받은 편지함 외에 **받은 친구 요청과 채팅·댓글·사용자 작성 공지**도 차단자 화면에서 숨긴다. 편지·요청의 목록뿐 아니라 ID로 진입하는 상세·알림 진입 화면·캐시에도 같은 규칙을 적용한다.
- **차단만으로 상대의 친구 요청·편지 발송을 서버에서 거절하지 않는다.** 요청·편지는 기존 관계·보관 규칙대로 생성될 수 있고, 차단자가 보는 화면에서 숨긴다. 차단 해제 후 아직 유효한 항목은 다시 보일 수 있다. 상대방 화면을 양방향으로 감추거나 친구 관계·편지 원문을 삭제하지 않는다.
- 앱이 열려 있지 않을 때 운영체제에 표시되는 푸시는 앱의 목록 필터만으로 숨길 수 없다. 현재 친구 요청은 서버에서 생성되고 푸시 경로를 탄다. **차단한 발신자의 친구 요청 알림이 보이지 않아야 한다는 출시 기준**을 충족하려면 알림 발송 경로에서 차단 관계를 확인해야 한다. 목록 필터만 구현한 상태를 전체 숨김 완료로 표시하지 않는다.
- 2026-09-22 RP-차단의 양방향 발송·요청 금지와 2026-09-24 RP-차단-정합의 같은 부분은 이 결정으로 대체한다. 차단자에게 상대 콘텐츠를 숨기고 앱 안에서 신고·차단을 제공하는 나머지 범위는 유지한다.

근거: 사용자가 `GET /blocks`를 앱에서 받아 편지·친구 요청의 상대 ID와 대조해 숨기는 방식을 제안하고, 발송 자체를 막지 않는 표시 방식의 차이를 설명받은 뒤 「ㅇㅇ 그렇게하면 문제 없을듯」이라고 선택했다. 푸시 예외는 현재 코드에서 확인된 구현 간극이며 사용자가 이미 수용했다고 기록하지 않는다.
...
- 커뮤니티 제한은 글·댓글·채팅·편지 작성을 막고, 개인 집중과 기록 조회는 유지한다.
...
| 커뮤니티 제한 | 경고 후 재위반 시 글·댓글·채팅·편지 작성 7일 제한 | 제재 적용 시점부터 7일 후 자동 해제 |
...
- 신고만 하면 신고한 콘텐츠를 숨긴다. 사용자가 함께 차단을 선택하면 RP-표시차단에 따라 차단자 화면에서 상대 콘텐츠·요청·편지를 숨긴다.
...
- 공지·댓글·섬 채팅·열려 있는 받은 편지·닉네임·섬 이름/소개 모두 신고 접수 시 서버 원문을 별도 증거 사본으로 확보한다. 원본의 수정·삭제가 확보한 사본을 바꾸지 않는다.
...
- 답장 주소가 없거나 확인되지 않은 사건에는 사후 결과 통지를 하지 않는다. 답장 전달 실패도 내부에 기록하고 재전송을 시도한다. 실패가 확정되면 무연락 사건으로 종결한다. 결과 통지 약속을 모든 신고자에게 하지 않는다.
...
| --- | --- | --- | --- |
| 공지 | 상세 더보기(BD-1a) | 공지 ID·버전·제목·본문·작성자 | 해당 공지 본문과 목록 미리보기 숨김 |
...
| 닉네임 | 사용자 프로필(SYS-2) | 사용자 ID·신고 시점 닉네임 | 신고한 닉네임을 중립 문구로 대체. 사용자 전체 차단과 구분 |
...
- 공지 등 수정 가능한 텍스트는 화면이 보고 있던 버전을 확인한다. 원문이 바뀌었고 과거 버전을 확보할 수 없으면 새 버전을 신고한 것처럼 접수하지 않고 내용을 다시 확인하게 한다.
- 증거 저장이 실패하면 성공 안내를 하지 않는다. 즉시 로컬 숨김은 유지할 수 있으나 ‘신고 접수 실패·재시도’를 표시한다. 재시도는 동일 요청 키로 중복 접수를 막는다.
- 외부 링크 문자열은 신고 대상 텍스트에 포함할 수 있으나 자동으로 방문·미리보기·다운로드하지 않는다. 이미지 첨부는 이번 최소 범위에 넣지 않는다.

### D3. 신고와 차단 UX·기존 관계

신고 화면에 **‘이 사용자도 차단’ 체크 항목을 기본 미선택**으로 제공하는 것은 RP-신고차단연계에서 확정했다. 아래의 기존 관계·실패 처리 세부사항은 제안이다. 신고·차단을 별도로도 실행할 수 있고 각각의 성공/실패를 표시한다. 섬 이름·소개처럼 사용자 귀속이 불분명한 신고에는 임의로 방장 차단을 붙이지 않는다.

차단은 다음과 같이 구체화한다.

- 기존 친구 관계는 자동 삭제하지 않고 차단 중 목록에서 숨긴다. 차단 해제 시 아직 존재하는 관계가 다시 보인다. 편지·새 친구 요청은 생성될 수 있으나 차단자 화면에는 표시하지 않는다.
- 대기 중 친구 요청과 기존 미확인 편지는 차단 중 목록·상세·앱 알림 진입점에서 숨기되, 차단만으로 삭제하지 않는다. 이미 신고한 증거는 유지한다. 서버가 발송한 푸시는 앱 목록 필터만으로 막지 못하므로 알림 경로에 별도 차단 판정이 필요하다.
- 차단 목록에서 본인이 한 차단을 해제할 수 있다. 서로 차단했다면 각자의 화면에 상대를 숨긴다. 차단한 사실을 상대에게 따로 알리지 않는다.
...
피신고자에게는 위반 규칙·제재 범위·시작/종료·이의제기 경로를 안내한다. 신고자 신원·편지 상대의 개인정보를 공개하지 않는다. 계정 전체 정지 중에도 본인 제재 안내·이의제기·개인정보 권리/탈퇴·로그아웃 경로는 접근 가능한 예외로 설계한다. 이용 정지는 계정 삭제가 아니다.

### D5. 반복·허위 신고와 이의제기

- 같은 신고자·같은 대상 버전은 한 사건에 병합한다. 추가 설명을 받을 수 있지만 동일 사건에 제재를 중복 적용하지 않는다.
- 일반 신고의 초기 운영 상한은 계정당 24시간 20건·분당 5회로 제안한다. 실제 오류는 재시도 가능 시각을 안내한다. 긴급 신고는 일반 일일 한도와 분리하되 중복 병합·요청 속도 제한은 유지한다. 긴급 분류를 선택했다고 자동 제재 우선권을 주지 않는다.
...
- 이의제기는 답장을 받은 신고자는 답장 후 30일, 무연락 사건의 피신고자는 본인 제재 통지 후 30일 이내 사건 ID를 적어 지원 채널로 제출하는 안이다. 제재 중에도 접근 가능해야 한다. 원문이 남아 있는 동안 접수된 재심은 분쟁 중 보존 예외로 관리하고 담당·검토일을 둔다. 신고자가 답장 주소를 남기지 않은 사건의 이의제기 인증·조회 범위는 후속 설계한다.
...
| --- | --- | --- |
| 일반 신고 원문·자유 설명·불필요한 프로필 사본 | 확인된 주소로 최초 결과 답장 발송 후 30일, 주소가 없거나 발송 실패가 확정되면 무연락 종결 후 30일 만료. 메일함·검색·백업에서 파기하며 답장 재전송으로 연장하지 않음 | RP-증거기간·RP-메일결과 |
| 일반 제재 최소 이력 | 사용자 ID·위반 분류·사건 ID·발생/조치 시각·단계·종료 시각만 유지, 6개월 무위반 초기화 후 불필요한 귀속 이력 파기 | 원문을 6개월 보관하지 않음 |
| 경고만 받은 사용자 | 경고 통지 시각부터 6개월 무위반이면 초기화하는 안 | RP-누적초기화의 미정 기산점 구체화 |
| 영구 정지 상태 | 현재 계정의 정지 상태·사유 코드만 최소 유지. 원문 증거와 상세 이력의 무기한 보존 근거로 쓰지 않음 | RP-누진제재·탈퇴 처리 근거 별도 확인 |
...
- 누끼 공유는 기다리지 않는다. 2.0 텍스트 UGC 공개와 함께 신고·차단·운영 처리·결과 통지를 제공한다. 이미 준비된 UGC 기능에도 같은 출시 조건을 적용한다.
- 공지·댓글·채팅·편지·닉네임·섬 이름/소개 생성·수정 전에 서버에서 정책 동의와 유해 텍스트 필터를 적용하는 안이다. 클라이언트 검사만으로 대체하지 않는다. 금지 표현·링크 규칙의 초기 목록과 오탐 복구를 운영 문서로 관리한다.
...
- 커뮤니티 제재 중에는 닉네임·섬 소개 수정으로 소통 제한을 우회하지 못하게 한다. 운영자가 확인한 정상화 요청은 별도 처리한다. 정지된 방장의 권한 승계는 기존 방장 정책과 연결해 주민 탈퇴나 섬 운영이 막히지 않게 한다.
- 제재 우회 의심만으로 기기·가족 전체를 차단하지 않는다. 기존 계정의 권한 검사를 모든 REST·실시간 쓰기에 적용하고, 동일 행위자 재가입 판단은 최소 정보·근거를 확보한 뒤 운영 검토로 처리한다.
...
| 후속 구현·검증 | [범위와 완료 증거](follow-up-scope.md) 작성 완료 | 티켓 분할·구현·운영 시연 |

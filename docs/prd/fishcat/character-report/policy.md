...
티켓 본문의 1.x 이미지 공유 전제는 2026-09-20 안수빈 댓글 #11163의 2.0 범위로 대체한다. **2.0에는 사용자 이미지·파일을 서버에 영구 저장하거나 다른 사용자에게 공유하는 기능을 넣지 않는다.** 신고 대상은 게시판 공지·댓글, 섬 채팅, 친구 편지, 닉네임, 섬 이름·소개와 방문자에게 보이는 퀘스트 제목인 텍스트 콘텐츠다. 집중 주제는 본인에게만 원문을 보여 주고 타인에게는 정형 문구로 대체해야 한다. 누끼 캐릭터 공유와 이미지 증거 보존 A/B/C 선택지는 이번 범위에서 제외한다. 기존 `POST /api/v1/character/moderation`은 base64 PNG를 입력받아 외부 검사 서비스에 보내고 결과만 반환한다. 인증된 `POST /api/v1/link-previews`는 제출된 URL의 외부 PNG·JPEG·GIF·PDF 등을 최대 10MiB까지 가져와 렌더링한 썸네일을 사용자별 Redis에 정상 결과 기준 5분 보관하고 조회 API로 제공한다. 두 경로 모두 사용자 간 미디어 공유와 구분되지만 실제 이미지·파일 처리 경로이므로 접근·개인정보·오남용·불법 자료 유입 대응을 출시 전에 검증한다.
...
- 재로그인·다른 기기에서도 이 숨김을 유지하려면 계정에 연결된 최소 숨김 표지(신고자·대상·**신고한 텍스트의 필드·리비전**)가 필요하다. 공지는 댓글 추가에도 공용 `notice.version`이 오르므로 숨김 판정에 그 값을 그대로 쓰지 않고 제목·본문 전용 리비전을 쓴다. 본문이 바뀌지 않은 댓글 추가만으로 기존 숨김이 풀리면 안 된다. 이 표지는 신고 원문·사유를 보존하는 자체 사건 DB가 아니다. 서버는 목록·상세·실시간·푸시 발송 직전에 권위 있는 `(신고자, 대상, 필드, 리비전)` 표지를 검사한다. 퀘스트 제목의 대상은 정의 ID뿐 아니라 `occurrenceId`·회차 `definitionRevision`을 포함한다. 기존 대기 전송을 폐기한다. 표지 적용과 대기 전송 배출이 확인되기 전에는 접수·영구 숨김 성공을 안내하지 않는다. 이미 기기에 전달된 내용을 회수한다고 약속하지 않는다.
...
- 텍스트로 제기된 아동 성착취·학대 의심과 외부 자료를 가리키는 신고의 긴급 대응 절차를 정한다. 신고용 이미지 첨부·증거 저장 파이프라인은 이번 범위에 넣지 않는다. 기존 캐릭터 이미지 검사 API와 링크 미리보기 API의 실제 파일 입력·외부 전송/가져오기·캐시는 별도 출시 검증 대상으로 둔다. 일반 검토 대기 규칙을 그대로 적용한다고 간주하지 않는다.
...
- 차단한 사람에게 상대가 작성한 섬 채팅·댓글·공지와 그 사람이 현재 수정한 공개 퀘스트 제목을 서버 응답·실시간 전송에서 제외하거나 중립 대체한다. 공지는 원 작성자뿐 아니라 **현재 제목·본문 리비전의 실제 수정 주체**가 차단 대상이어도 제외한다. 이를 판정하려면 제목·본문 필드별 리비전의 수정자 ID를 저장하고 섬 이름·소개도 `Group.version`과 별개인 필드별 텍스트 리비전·수정자 ID를 저장해야 한다. 퀘스트 제목은 정의 수정자 ID를 회차 스냅샷에도 저장해야 한다. 기존 리비전의 수정자가 미상이면 생성자에게 임의 귀속하지 않고 해당 텍스트를 차단자에게 중립 대체하거나 운영 검토 전까지 노출하지 않는다. 차단 대상의 닉네임·사용자 소개 등 프로필 텍스트와 그 사람이 현재 리비전을 수정한 **섬 이름·소개**도 차단자에게 보내는 서버 응답·알림에서 중립 표시로 대체한다. 같은 섬 소속과 섬 기능은 유지한다. 상세 ID 직접 조회, 목록·미리보기·푸시 알림도 같은 규칙을 따른다. 직접 연락만 차단하는 B안은 채택하지 않는다.
- 차단 대상이 탈퇴하면서 `user_blocks`와 작성자·수정자 FK가 제거돼도 기존 차단자에게 그 사람이 작성했거나 현재 리비전에서 수정한 남은 콘텐츠·섬 이름/소개가 다시 노출되면 안 된다. 탈퇴 처리 전에 차단자별로 원 작성자에 따른 공지·콘텐츠 숨김 표지는 콘텐츠 ID에, 현재 수정자에 따른 숨김 표지는 필드·텍스트 리비전에 결박해 별도 남긴다. 원 작성자 표지는 다른 사람이 수정해 새 리비전이 생겨도 유지하고 콘텐츠 삭제·차단 해제·차단자 탈퇴 때 파기한다. 수정자 표지만 해당 텍스트가 삭제되거나 대체 리비전으로 바뀔 때 파기한다. 목록·상세·실시간·알림에 같은 판정을 적용하며 원 작성자·수정자 신원이나 신고 원문은 표지에 넣지 않는다.
- 차단 대상이 탈퇴한 뒤 연결된 어느 로그인 제공자로 재가입해도 기존 차단자에게 다시 친구 요청·편지·채팅을 보내지 못하게 한다. 탈퇴 전에 차단 대상에 연결된 재활성화 가능한 모든 제공자·주체 쌍의 keyed digest와 차단 방향의 최소 관계 표지를 내구 확정하고, 재가입 게이트에서 새 계정에 관계를 결박해 양방향 직접 연락과 알림을 차단한다. 대상 콘텐츠 조회 뒤 신고·차단 전에 작성자가 탈퇴한 경우에도 유효한 조회 스냅샷 토큰의 안정적 대상 주체 표지로 최신 탈퇴 tombstone의 모든 연결 제공자 digest를 찾아 방향성 차단 표지를 만들고 재가입 계정에 결박한다. 토큰의 열람자·대상·유효기간을 검증하고 탈퇴한 계정 ID만으로 신원을 추정하지 않는다. 재결박·차단 세대의 실시간 적용이 확인되기 전에는 기존 차단자에 대한 직접 연락을 허용하지 않는다. 차단 관계마다 차단자에게만 보이는 불변 `blockRelationId`를 부여해 탈퇴 대상도 차단 목록에서 중립 항목으로 보여 주고 이 ID로 재가입 전 해제할 수 있게 한다. 기존 `DELETE /blocks/{blockedUserId}`와 별도로 관계 ID 기반 해제 계약을 제공하며 대상의 이전 제공자·주체·새 계정 ID를 차단자에게 공개하지 않는다. 차단 해제 또는 차단자 탈퇴 때 해당 표지를 파기하고, 보존 근거·최소 기간·키 교체를 출시 전에 확인한다. 제공자 없는 게스트의 재발급 우회는 RP-게스트쓰기의 미해결 위험으로 남는다.
...
### RP-게스트쓰기 — 추가 확인 없는 게스트 커뮤니티 쓰기

게스트의 친구 요청·편지·공지 등 커뮤니티 쓰기를 2.0에서도 유지한다. 소셜 계정 연결이나 이메일·전화번호 소유 확인을 쓰기의 필수 조건으로 삼지 않는다. 현재 2.0 친구 요청·편지 발송의 `SOCIAL_LOGIN_REQUIRED` 가드는 이 결정과 다르므로 후속 구현에서 제거한다. 상점 구매의 기존 소셜 로그인 조건은 바꾸지 않는다.

제재와 차단은 현재 게스트 계정·활성 세션에는 적용한다. 그러나 제공자·확인 연락처가 없는 게스트가 탈퇴하거나 새 게스트를 발급받으면 기존 계정과 동일인임을 서버가 증명할 수 없다. 따라서 새 게스트에게 7일/30일/영구 제재·누진 단계·차단 관계가 자동 승계된다고 주장하지 않는다. 기존 게스트 발급의 앱 제출 설치 UUID·5분 `guest_device_claims` 복구 창과 IP 제한은 내구 신원이 아니다. 신고 답장용 선택 이메일도 쓰기 신원으로 전용하지 않는다.

게스트 생성·친구 요청·편지·공지 등 커뮤니티 쓰기에 계정별·발신원별·전역 한도와 운영 감시를 적용해 대량 악용을 줄인다. 이는 우회 방지가 아니라 피해 완화다. 이 정책 아래에서는 제재받은 게스트가 새 게스트로 돌아오는 경로가 남는다. 신고·차단 및 제재의 실제 효과를 출시 조건으로 평가할 때 이 미해결 위험을 별도로 판단해야 하며, 문서 완료나 리뷰 종료를 우회 해결의 증거로 삼지 않는다.
...
- 예약된 초기화 작업은 예약 당시 마지막 확인 위반 시각과 제재 세대가 현재 값과 같고, 제한 해제 뒤 실제 6개월 무위반인 경우에만 조건부 적용한다. 새 위반·상위 제재가 확정되면 이전 예약 작업은 무효다. 초기화 시점에 **접수돼 아직 판단이 끝나지 않은 사건**의 주장된 행위 시각이 6개월 창 안에 있으면, 사건 ID·행위 시각·종전 누진 단계의 최소 초기화 tombstone을 같은 세대에 남기되 **현재 단계와 재가입 표지는 잠정적으로 초기화**한다. 미결인 과거 사건만으로 새 위반을 종전 단계에서 계산하지 않는다. 경계 뒤 별도로 발생한 위반은 잠정 초기화 단계로 판단·통지하고, 나중에 과거 사건이 확정되거나 뒤집히면 남은 유효 판단을 실제 행위·통지 시각 순으로 CAS 재계산해 현재 단계·해제 예약·제공자 표지를 정정한다. 이미 통지된 제한을 소급 연장하거나 통지 전 행위를 새 재위반으로 세지 않는다. 미결 자체로 단계를 올리지 않는다.
- 미결 사건의 누진 단계 fence는 일반 사건의 접수 후 30일 상향·추가 7일 종결, 주소 확인·결과 발송 및 30일 재심 창을 포괄하도록 **최초 접수 후 최대 90일** 유지한다. 사건 종결 뒤 신고자의 유효 재심 창이 끝나거나 90일 상한에 도달하면 tombstone을 파기하고, 이후 확정으로 종전 단계를 소급 복구하지 않는다. 조사·법적 보존 예외로 원문을 더 보존하더라도 누진 fence가 자동 연장되지는 않는다. 경고만 받은 사용자의 기산점과 최소 이력의 파기 조건은 D6을 따른다.
...
| 닉네임 | 사용자 프로필(SYS-2) | 사용자 ID·닉네임 변경 전용 단조 증가 텍스트 리비전·불변 스냅샷 | 신고한 닉네임을 중립 문구로 대체. 사용자 전체 차단과 구분 |
| 섬 이름/소개 | 섬 정보 더보기(SYS-2) | 섬 ID·이름/소개 필드별 텍스트 리비전·현재값·실제 수정자 ID (`Group.version` 사용 금지) | 신고한 필드를 중립 문구로 대체하고 섬 기능 접근은 유지 |
| 퀘스트 제목 | 퀘스트 상세·목록 더보기 | 퀘스트 정의 ID·`occurrenceId`·회차의 `definitionRevision`·회차 제목·실제 수정 주체·조회 시 서버 스냅샷 | 신고자에게 제목을 중립 대체하고 퀘스트 기능 접근은 유지 |
...
신고 화면에 **‘이 사용자도 차단’ 체크 항목을 기본 미선택**으로 제공하는 것은 RP-신고차단연계에서 확정했다. 아래의 기존 관계·실패 처리 세부사항은 제안이다. 함께 실행하면 먼저 신고의 불변 스냅샷 토큰과 재시도 키를 내구 예약한 뒤, **차단은 메일 접수 결과와 독립적으로 즉시 확정**한다. 앱은 신고 접수와 차단의 성공/실패를 각각 표시하며 메일 장애 때문에 차단을 지연시키지 않는다. 차단으로 원문이 화면에서 숨겨져도 예약된 신고는 토큰으로 재시도·완료할 수 있어야 한다. 섬 이름·소개처럼 사용자 귀속이 불분명한 신고에는 임의로 방장 차단을 붙이지 않는다.
...
운영자의 콘텐츠 전체 비공개·삭제는 검토한 **대상 종류·ID·필드·텍스트 리비전**이 현재 값과 같을 때만 조건부 적용한다. 그 사이 새 리비전으로 수정됐다면 과거 원문의 위반·계정 제재 판단은 유지할 수 있어도 새 리비전의 노출은 다시 검토한다. 과거 신고만으로 현재의 정상 텍스트를 내려서는 안 된다.
...
피신고자에게는 위반 규칙·제재 범위·시작/종료·이의제기 경로와 **이의제기 참조 ID**를 안내한다. 제재 적용과 같은 원자 경계에서 제한 중에도 접근 가능한 상태 화면에 통지를 내구 공개하고 통지 작업·30일 증거 보존 예외를 예약한다. 외부 통지는 24시간 안에 시도하고 실패/미시도는 기한에 무연락 전환·담당 경보를 남긴다. 상태 화면 공개와 통지 성공 또는 무연락 전환이 확인되기 전에는 증거를 파기하지 않는다. 참조 ID로 신고자 신원·편지 상대의 개인정보가 노출되지 않게 한다. 계정 전체 정지 중에도 본인 제재 안내·이의제기·개인정보 권리/탈퇴·로그아웃 경로는 접근 가능한 예외로 설계한다. 이용 정지는 계정 삭제가 아니다.

### D5. 반복·허위 신고와 이의제기

- 같은 신고자·같은 대상·필드·버전은 한 사건에 병합하되 새 사유·설명을 추가 기록하고 긴급 사유가 생기면 즉시 재분류·담당 알림을 수행한다. 원문이 이미 파기된 경우에도 새 긴급 위해 정황은 별도 안전 제보로 우선 평가하며 파기된 원문을 복원하지 않는다. 여러 신고자의 사건이 같은 대상·필드·버전을 가리키면 대상·필드·리비전별 위반 판단 ID로 묶어 한 번만 제재하고, 추가 설명은 각 신고자 사건에 남긴다. 한 번의 쓰기에서 공지 제목·본문이나 섬 이름·소개를 함께 바꾼 경우 각 필드 판단은 남기되 공통 `mutationId`로 묶어 누진 단계는 한 행위만 소비한다. 별도 쓰기 중에서도 직전 단계의 실제 통지 시각 이후 발생한 행위만 재위반으로 다음 단계에 올린다. 통지 전 행위 여러 개가 뒤늦게 확정되면 판단 기록은 각각 유지하되 누진 단계는 한 번만 반영한다. 판단 ID 발급과 별개로 제재 적용도 판단/`mutationId` 단일 소비 표지·피신고자 제재 세대 CAS로 원자화한다. 같은 판단의 동시 처리나 서로 다른 판단의 동시 적용이 단계를 중복 상승시키거나 유실시키면 안 된다.
...
- 이의제기는 답장을 받은 신고자는 답장 후 30일 이내 사건 ID로, 피신고자는 본인 제재 통지 후 30일 이내 **제재 통지·상태 화면에 제공한 이의제기 참조 ID**로 지원 채널에 제출하는 안이다. 신고자도 사건 ID만으로는 재심을 열 수 없다. 인증된 원래 신고자 계정 또는 사건에 결박된 확인된 답장 주소의 소유를 대조한 뒤에만 재심 접수·보존 예외·결과 통지를 허용한다. 참조 ID 자체도 인증 수단이 아니다. 피신고자는 정지 중에도 가능한 계정 인증 또는 제재 주체에 사전 결박된 확인된 연락 수단으로 본인임을 확인하고, 참조 ID의 제재 주체와 일치한 경우에만 재심 접수·증거 보존 예외·결과 통지를 허용한다. 인증 전 제3자의 제출은 접수나 보존 기간 연장으로 처리하지 않고 사건 존재도 공개하지 않는다. 피신고자의 참조 ID는 내부 사건 ID에 제한 접근으로 연결하되 신고자 신원·비공개 콘텐츠를 드러내지 않는다. 제재 중에도 통지·상태 화면과 지원 채널에 접근 가능해야 한다. 피신고자의 기한이 일반 원문 파기일보다 늦으면 연결 사건의 원문을 이의제기 기한까지 보존 예외로 묶고 종료 후 파기한다. 기한 내 인증된 재심은 접수 시각을 먼저 내구적으로 기록하고 파기 작업과 직렬화해 보존 예외로 관리한다. 파기 경계에서 동시에 들어온 재심을 원문 삭제 후 접수 완료로 처리하지 않는다. 신고자가 답장 주소를 남기지 않은 사건의 이의제기 인증·조회 범위는 후속 설계한다.
- 가능한 한 최초 판단자와 다른 담당이 재심한다. 오판이면 취소된 판단만 제재 적용 집합에서 빼고, 남은 확정 판단을 실제 행위·통지 순서와 각 단계의 적용 시각에 따라 재계산한다. 현재 제재 종류·종료 시각·예약 해제 작업과 재가입용 제재 표지를 동일한 제재 세대에서 조건부 정정하고 결과를 다시 알린다. 증거 파기 후 연락을 받으면 보유한 최소 조치 기록으로 검토할 한계를 안내하며 이미 파기한 원문을 복원했다고 하지 않는다.
...
| 신고·재심 기간의 제재 귀속 표지 | 신고 접수 시 확인한 피신고자의 재활성화 가능한 모든 제공자·주체 쌍의 keyed digest를 사건 ID에 원자적으로 최소 연결하고 탈퇴 전에 내구 확인한다. 심사 전 탈퇴·동일 제공자 재가입이 있어도 최종 판단을 새 계정에 승계한다. 위반 없음/검토 미완료 종결 뒤에도 신고자의 결과 답장 후 30일 이의제기 기한과 기한 내 접수된 재심이 모두 끝날 때까지 유지한다. 그때도 위반이 아니면 파기하고, 위반 확정 시 활성 제재·누진 단계 표지로 전환한다. 무연락 종결 사건은 별도 재심 권한이 확인되지 않는 한 원문 파기 시점에 파기한다. 법적 근거·최소 보존 기간을 출시 전 확인 | 탈퇴가 심사 중인 신고의 책임 주체를 지우지 않음 |
| 경고만 받은 사용자 | 경고 통지 시각부터 최소 누진 단계·초기화 예정 시각을 재활성화 가능한 모든 제공자·주체 쌍의 keyed 표지에 결박해 재가입 계정에 승계. 6개월 무위반이면 초기화하는 안 | RP-누적초기화의 미정 기산점 구체화 |
| 활성 기간형·영구 제재 | 현재 계정의 제재 상태·사유 코드·만료 시각을 최소 유지. 탈퇴 뒤 동일 로그인 제공자로 즉시 재가입해 남은 7일/30일 제한이나 영구 정지를 우회하지 못하도록 재활성화 가능한 모든 제공자·주체 쌍의 keyed digest와 활성 제재 종류·원래 만료 시각을 연결한 최소 표지를 가입 게이트에서 검사하고 새 계정에 남은 제재를 승계한다. 활성 제재를 이유로 가입 자체를 거절해 이의제기 인증을 잃게 하지 않는다. 승계된 계정은 제한 상태에서도 제재 안내·이의제기·개인정보 권리·탈퇴·로그아웃에 접근한다. 기간형 표지는 만료·이의제기 정정 뒤 파기하고 영구 표지는 보존의 적법 근거·기간을 확인한다. 원문 증거와 상세 이력의 무기한 보존 근거로 쓰지 않음 | RP-누진제재·탈퇴 처리 근거 별도 확인 |
| 탈퇴 후 누진 단계 | 경고 통지 시점부터, 그리고 기간형 제한이 만료된 뒤에도 재활성화 가능한 모든 제공자·주체 쌍의 keyed digest에 최소 누진 단계·마지막 확인 위반 시각·제재 해제 시각·6개월 초기화 예정 시각을 연결한다. 재가입 시 새 계정에 남은 단계를 승계하고, 해제 후 실제 6개월 무위반 조건이 성립한 뒤에만 표지를 파기한다. 오판 취소 시 즉시 정정 | RP-누적초기화의 제재 해제와 단계 초기화 구분 |
| 조사·분쟁·기관 보존 요청 | 사유·근거·담당·재검토일을 지정해 일반 만료와 분리. 근거 종료 시 불필요한 자료 파기 | RP-긴급조치·증거 보존 예외 |

제재의 7일·30일은 적용 시각부터의 기간으로 계산하고 6개월은 UTC 기준 달력 월로 계산하는 안이다(해당 일자가 없는 달은 말일). 위반 발생 시점·앞선 제재 통지 시점을 기록하고 단계 계산에서 반드시 비교한다. 직전 단계 통지 전에 발생한 행위는 신고·확정 시점이 늦어도 새 재위반으로 소비하지 않는다. 제재 후 새 행위가 확인되면 그 사실을 반영한다. 단순 미검토 신고가 있다고 자동으로 초기화나 파기를 무기한 미루지 않는다.

심사 전에 탈퇴해 실제 제재 안내를 받을 수 없는 피신고자에게는 보이지 않는 상태 화면 공개를 통지로 세지 않는다. 제공자 표지로 재가입한 첫 인증 시 실제 안내를 열고 30일 이의제기 시계를 시작한다. 7일·30일 기간형 제재의 원문은 제재 결정 뒤 재가입을 최대 90일 기다리고, 그 안에 돌아오면 안내 후 30일까지 보존한다(일반 상한 120일). 원문 파기 뒤 이의가 제기되면 남은 검증 가능 근거로 재심하고 입증할 수 없는 누진 단계는 취소한다. **영구 정지는 90일 대기만으로 해제하거나 자동 취소하지 않는다.** 탈퇴 주체의 영구 정지에는 재심 가능한 원문 또는 충분한 검증 가능 결정 근거를 별도 접근 제한 증거로 보존하고 적법한 보존 근거·기간·정기 재검토·파기 조건을 출시 전에 확정한다. 이를 보장하지 못하면 영구 정지 재가입 승계의 출시 조건이 미충족이다. 이 기간은 D6의 후속 구현안이며 법적 근거와 실제 메일함 파기 가능성을 출시 전에 검증한다.
...
계정 탈퇴는 [계정 정책](../account/policy.md)의 정본을 따른다. 신고 사본이 생기면 기존 탈퇴 처리에 저장물별 파기·적법 보존 예외를 추가해야 한다. ‘신고가 있으니 계정 전체를 무기한 보존’하거나 기존 전체 파기 약속과 충돌한 채 출시하지 않는다. 원 `user_blocks` 관계의 양방향 hard delete는 유지하되, RP-차단의 별도 keyed 재가입 승계 표지는 원행 삭제 전에 확정하고 차단 해제·차단자 탈퇴 때 파기한다.

### D7. 필터링·제재 적용과 공유 기능의 출시 순서

- 2.0은 텍스트 UGC만 출시하며 사용자 이미지·파일의 영구 저장·타인 조회·공유는 열지 않는다. 기존 캐릭터 이미지 검사 API와 외부 파일을 가져와 썸네일을 임시 캐시하는 링크 미리보기 API의 입력·외부 전송·보존은 F6에서 별도로 점검한다. 텍스트 UGC 공개와 함께 신고·차단·운영 처리·결과 통지를 제공한다. 이후 이미지·파일 공유를 도입한다면 별도 정책·신고 증거 방식·모더레이션·저장/삭제·운영 검증을 마친 뒤 출시 여부를 다시 판단한다.
...
- 제재 우회 의심만으로 기기·가족 전체를 차단하지 않는다. 기존 계정의 권한 검사를 모든 REST·실시간 쓰기에 적용한다. 전체 정지는 적용 시 기존 인증 세션·실시간 연결을 폐기하고 토큰 갱신, 새 CONNECT 및 모든 보호된 읽기·쓰기 요청에서 최신 제재 상태·세대를 재검사한다. 제재 안내·이의제기·개인정보 권리·탈퇴·로그아웃만 허용한다. 각 UGC 쓰기의 커밋은 읽은 제재 세대와 현재 세대가 같을 때만 허용하고, 세대가 바뀌면 새 상태를 다시 검사한다. 운영자의 제재 확정은 진행 중인 쓰기와 직렬화하고 이미 예약된 실시간·outbox 전송을 배출한 뒤 완료로 표시한다. 제재 완료 뒤 늦게 커밋된 새 커뮤니티 콘텐츠나 알림이 남으면 안 된다. **활성 7일·30일 제한과 영구 정지 계정의 연결된 어느 제공자 재가입**은 탈퇴 뒤에도 남는 최소 keyed 표지로 새 계정에 남은 기간/영구 제재를 승계해야 한다. 가입 자체를 거절해 본인 인증·이의제기 경로를 끊지 않는다. 기간형 표지는 원래 만료 시각에 효력을 종료하고, 보존 근거를 확인하지 않은 영구 표지에 실효성을 주장하지 않는다. 다른 행위자의 재가입 여부는 최소 정보·근거를 확보한 뒤 운영 검토로 처리한다.

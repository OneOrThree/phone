...
- 재로그인·다른 기기에서도 이 숨김을 유지하려면 계정에 연결된 최소 숨김 표지(신고자·대상·버전)가 필요하다. 이는 신고 원문·사유를 보존하는 자체 사건 DB가 아니며, 구체적인 저장·파기 방식은 후속 구현에서 정한다. 서버 표지 확정 전 로컬 숨김만 됐다면 영구 숨김 성공으로 안내하지 않는다.
...
- 커뮤니티 제한은 글·댓글·채팅·편지의 생성·수정·삭제와 닉네임·섬 이름·소개 수정을 막고, 개인 집중과 기록 조회는 유지한다. 신규·기존 공지의 수정·삭제 권한이 있더라도 제재 중에는 실행할 수 없다. 운영자가 유해 콘텐츠를 삭제·복구하는 조치는 별도 권한으로 수행한다.
...
| 커뮤니티 제한 | 경고 후 재위반 시 커뮤니티 UGC 생성·수정·삭제 및 닉네임·섬 이름·소개 수정 7일 제한 | 제재 적용 시점부터 7일 후 자동 해제 |
...
- 이의제기는 답장을 받은 신고자는 답장 후 30일, 피신고자는 본인 제재 통지 후 30일 이내 사건 ID를 적어 지원 채널로 제출하는 안이다. 제재 중에도 접근 가능해야 한다. 피신고자의 기한이 일반 원문 파기일보다 늦으면 그 사건의 원문을 이의제기 기한까지 보존 예외로 묶고 종료 후 파기한다. 원문이 남아 있는 동안 접수된 재심도 분쟁 중 보존 예외로 관리하고 담당·검토일을 둔다. 신고자가 답장 주소를 남기지 않은 사건의 이의제기 인증·조회 범위는 후속 설계한다.
...
- 커뮤니티 제재 중에는 신규·기존 공지의 수정·삭제를 포함한 모든 UGC 쓰기와 닉네임·섬 이름·소개 수정으로 소통 제한을 우회하지 못하게 한다. 운영자가 확인한 정상화 요청은 별도 처리한다. 정지된 방장의 권한 승계는 기존 방장 정책과 연결해 주민 탈퇴나 섬 운영이 막히지 않게 한다.

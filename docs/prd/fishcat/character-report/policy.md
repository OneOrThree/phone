...
- 예약된 초기화 작업은 예약 당시 마지막 확인 위반 시각과 제재 세대가 현재 값과 같고, 제한 해제 뒤 실제 6개월 무위반인 경우에만 조건부 적용한다. 새 위반·상위 제재가 확정되면 이전 예약 작업은 무효다. 초기화 시점에 **접수돼 아직 판단이 끝나지 않은 사건**의 주장된 행위 시각이 6개월 창 안에 있으면, 사건 ID·행위 시각과 종전 누진 단계만 담은 최소 초기화 fence를 같은 세대에 남긴다. 사건이 위반으로 확정되면 신고·검토 시각이 아닌 실제 행위 시각으로 종전 단계를 CAS 재계산하고 재가입용 제공자 표지도 함께 정정한다. 위반이 아니면 fence를 파기한다. 미결이라는 이유만으로 제재 단계를 올리지 않는다.
- 미결 사건의 누진 단계 fence는 일반 사건의 접수 후 30일 상향·추가 7일 종결, 주소 확인·결과 발송 및 30일 재심 창을 포괄하도록 **최초 접수 후 최대 90일** 유지한다. 그 안에 확정되지 않으면 해당 사건으로 과거 누진 단계를 소급 복구하지 않고 fence를 파기한다. 조사·법적 보존 예외로 원문을 더 보존하더라도 누진 fence가 자동 연장되지는 않는다. 경고만 받은 사용자의 기산점과 최소 이력의 파기 조건은 D6을 따른다.
...
심사 전에 탈퇴해 실제 제재 안내를 받을 수 없는 피신고자에게는 보이지 않는 상태 화면 공개를 통지로 세지 않는다. 제공자 표지로 재가입한 첫 인증 시 실제 안내를 열고 30일 이의제기 시계를 시작한다. 원문은 제재 결정 뒤 재가입을 최대 90일 기다리고, 그 안에 돌아오면 안내 후 30일까지 보존한다(일반 상한 120일). 90일 뒤 돌아와 이의를 제기했는데 원문이 이미 파기됐다면 증거 없는 제재를 유지하지 않고 취소한다. 보존 예외의 별도 적법 근거·재검토는 그대로 적용한다. 이 기간은 D6의 후속 구현안이며 법적 근거와 실제 메일함 파기 가능성을 출시 전에 검증한다.

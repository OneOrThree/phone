...
- 차단 대상이 탈퇴하면서 `user_blocks`와 작성자 FK가 제거돼도 기존 차단자에게 그 사람이 작성한 남은 콘텐츠가 다시 노출되면 안 된다. 탈퇴 처리 전에 해당 차단자별 남은 콘텐츠 ID에 비식별 숨김 표지를 남기고, 목록·상세·실시간·알림에서 계속 제외한다. 표지는 원 작성자 신원이나 신고 원문을 보존하지 않으며 해당 콘텐츠가 삭제될 때 함께 파기한다.

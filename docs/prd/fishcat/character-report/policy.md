...
티켓 본문의 1.x 이미지 공유 전제는 2026-09-20 안수빈 댓글 #11163의 2.0 범위로 대체한다. **2.0에는 사용자가 이미지·파일을 업로드해 서버에 저장하거나 다른 사용자에게 보여주는 기능을 넣지 않는다.** 신고 대상은 게시판 공지·댓글, 섬 채팅, 친구 편지, 닉네임, 섬 이름·소개인 텍스트 콘텐츠다. 누끼 캐릭터 공유와 이미지 증거 보존 A/B/C 선택지는 이번 범위에서 제외한다. 기존 캐릭터 이미지 검사 API는 업로드·저장·공유 기능의 근거가 아니다.
...
- 텍스트로 제기된 아동 성착취·학대 의심과 외부 자료를 가리키는 신고의 긴급 대응 절차를 정한다. 앱 안의 CSAM 이미지·파일 업로드나 자동 탐지 파이프라인을 이번 범위에 넣지 않는다. 일반 검토 대기 규칙을 그대로 적용한다고 간주하지 않는다.
...
- 차단한 사람에게 상대가 작성한 섬 채팅·댓글·공지를 서버 응답·실시간 전송에서 제외한다. 상세 ID 직접 조회, 목록·미리보기·푸시 알림도 같은 규칙을 따른다. 직접 연락만 차단하는 B안은 채택하지 않는다.
...
- 2.0은 텍스트 UGC만 출시하며 사용자 이미지·파일 업로드·저장·타인 조회는 열지 않는다. 텍스트 UGC 공개와 함께 신고·차단·운영 처리·결과 통지를 제공한다. 이후 이미지·파일 공유를 도입한다면 별도 정책·신고 증거 방식·모더레이션·저장/삭제·운영 검증을 마친 뒤 출시 여부를 다시 판단한다.

...
### RP-게스트쓰기 — 게스트 커뮤니티 쓰기와 제재 승계

게스트의 친구 요청·편지·공지 등 커뮤니티 쓰기를 2.0에서도 유지한다. 소셜 계정 연결을 쓰기의 필수 조건으로 삼지 않는다. 현재 2.0 친구 요청·편지 발송의 `SOCIAL_LOGIN_REQUIRED` 가드는 이 결정과 다르므로 후속 구현에서 제거하거나 쓰기 자격 가드로 교체한다. 상점 구매의 기존 소셜 로그인 조건은 바꾸지 않는다.

쓰기 자격은 소유를 확인한 이메일 주소에 결박한다. 게스트 로그인·읽기·신고·제재 안내·이의제기에는 이메일 확인을 요구하지 않는다. 미확인 주소를 신고 결과 답장란에 적었다는 사실만으로 쓰기 자격을 부여하지 않는다. 서버는 주소 원문 대신 정규화 주소의 버전 있는 keyed digest를 커뮤니티 쓰기 자격 및 제재·차단 재결박에 사용한다. 이메일 확인 뒤 새 계정이 발급되면 쓰기 전에 해당 digest의 활성 7일/30일/영구 제재, 해제 뒤 6개월 누진 단계, 차단 관계를 조회하고 남은 상태를 결박한다. 탈퇴·주소 변경·제재 적용은 기존 주소의 필요한 최소 표지가 확정되기 전 완료하지 않는다. 확인·조회·재결박이 불확실하면 커뮤니티 쓰기만 보류하며, 제재 중에도 인증·안내·이의제기 접근은 유지한다.

서로 다른 새 이메일을 확인해 새 게스트를 만드는 우회까지 동일인이라고 단정할 수 없다. 게스트 발급의 설치 UUID·5분 `guest_device_claims` 복구 창이나 IP 제한을 내구 신원으로 취급하지 않는다. 확인 주소별·계정별·전역 쓰기 및 인증 발송 한도와 운영 검토를 보완책으로 두되, 완전한 제재 우회 방지라고 홍보하지 않는다. 이메일 원문 처리·digest 보존 기간·키 교체·접근 권한의 법적 근거와 실제 발송 가능성은 출시 전에 검증한다.

...
- 재로그인·다른 기기에서도 이 숨김을 유지하려면 계정에 연결된 최소 숨김 표지(신고자·대상·**신고한 텍스트의 필드·리비전**)가 필요하다. 공지는 댓글 추가에도 공용 `notice.version`이 오르므로 숨김 판정에 그 값을 그대로 쓰지 않고 제목·본문 전용 리비전을 쓴다. 본문이 바뀌지 않은 댓글 추가만으로 기존 숨김이 풀리면 안 된다. 이 표지는 신고 원문·사유를 보존하는 자체 사건 DB가 아니다. 서버는 목록·상세·실시간·푸시 발송 직전에 권위 있는 `(신고자, 대상, 필드, 리비전)` 표지를 검사하고 기존 대기 전송을 폐기한다. 표지 적용과 대기 전송 배출이 확인되기 전에는 접수·영구 숨김 성공을 안내하지 않는다. 이미 기기에 전달된 내용을 회수한다고 약속하지 않는다.
...
| 탈퇴 후 누진 단계 | 기간형 제한이 만료돼도 동일 제공자·주체의 keyed digest에 최소 누진 단계·마지막 확인 위반 시각·제재 해제 시각·6개월 초기화 예정 시각을 연결한다. 재가입 시 새 계정에 남은 단계를 승계하고, 해제 후 실제 6개월 무위반 조건이 성립한 뒤에만 표지를 파기한다. 오판 취소 시 즉시 정정 | RP-누적초기화의 제재 해제와 단계 초기화 구분 |

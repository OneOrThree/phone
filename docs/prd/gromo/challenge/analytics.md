...
그런데 **받은 쪽에 계측 절이 없었다.** `docs/prd/gromo/challenge/`에 이벤트 표가 없었고,
...
8. **대시보드는 `group_challenge_result_interrupted`를 반드시 `reason`으로 breakdown한다.**
   합계 카드(총 발생 수)를 만들려면 리뷰를 거친다 — §5.3이 "지연(`blocking_overlay`)을 유실 두
   사유와 합산하지 않는다"고 정했지만, **문서 계약만으로는 GA4 탐색에서 이벤트를 원시 카운트로
   집계하는 관성을 못 막는다.** 문서를 안 읽고 이름만 보고 카드를 만들면 그대로 사고가 재현된다.
9. §6의 갭이 닫히기 전에는 해당 지표를 KPI 대시보드에 올리지 않는다.
   특히 G2가 열려 있는 동안 `prd.md` §5의 "참여 취소율" 행은 **측정 개시 전** 상태다.
10. `analyticsEvents.ts`의 죽은 정본 주소(`docs/app/challenge-impl-2026-08/contract.md`·

...
- [x] **키워드 3개씩 본인 확인** — **완료 (2026-08-14).** 담당자 본인이 고른 표현이다.
      이것은 배선 PR(GROMO-1588)의 차단 조건이었고, 확인 후 해제됐다. 남이 대신 정하면
      D10에서 실명을 뺀 이유(공개 화면 · 되돌릴 수 없음 · 본인 동의)가 그대로 재현된다 —
      **키워드를 바꿀 때마다 이 확인을 다시 받는다.** OTA로 즉시 나가므로 리뷰가 못 막는다
...
- [ ] **GA4 콘솔에 이벤트 범위 커스텀 측정기준 등록** — `category` · `contact_id` · `is_recommended`.
      ...
      같은 선행 조건이 `docs/prd/gromo/onboarding/prd.md`의 `step_viewed`에도 적혀 있다(그쪽은 미등록 상태) —

...
- **QA/분석:** OWNER/MEMBER matrix, 위임 뒤 나가기, 강퇴·권한 변경 경합과 실패 복구를 검증한다. 기존 성공 전용 이벤트는 진단에만 쓰고 운영 성공률로 보고하지 않는다.

## 착수·출시 gate

- **착수:** 역할 변경과 나가기의 최신 상세 재조회·오류 복귀 계약, 02 `F02-P1` 아이콘 route/store 경계를 확정한다.
- **출시:** OWNER/MEMBER 권한, 위임 후 이탈, 강퇴, 공지 권한 rollback, 늦은 응답의 E2E·접근성 회귀를 통과한다. 2xx 뒤 최신 detail·목록 수렴을 확인하고, 권한·검증·네트워크 4xx/5xx에는 false success·낙관 상태 잔류가 없어야 한다. 나가기의 `MEMBER_ONLY`는 본인 미소속 사후조건으로 멱등 종료한다. `NOT_FOUND`는 비활성 요청자와 그룹 부재가 모두 가능하므로 인증·그룹 scope를 재확인해 그룹 부재면 복귀하고, 비활성 요청자면 성공 처리 없이 세션을 안전 복구하며, 의미 불명·확인 실패는 오류로 남긴다. 강퇴의 같은 코드는 최신 detail로 target 부재를 확인해야만 행을 제거한다.

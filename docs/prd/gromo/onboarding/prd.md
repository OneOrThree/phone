...
여기에 제품 설득이 얹힌다 — 집중을 얼마나 빠르게 시작하는지, 누구와 함께할 수 있는지, 집중 뒤 어떤 기록과 보상이 남는지를 현재 기능으로 보여준다.
...
4. **첫 애착** — 캐릭터를 만나고(character_intro), 원하면 내 물건으로 만들어(cutout) 본 뒤 이름을 짓는다.
...
- 어제 사용시간 **자가 추측·분석·요약**(`UsageGuessStep`·`YesterdayScreenTimeStep`) · 실시간 랭킹 · 핸드폰 관리 설득 — 파일은 있으나 시퀀스에서 빠져 있다.
...
| `character_intro` | 항상 | — | ○ |
| `cutout_experience` | 캐릭터 생성 완료 또는 `건너뛰기` 선택 | ○ (항상 노출) | ○ |
...
`cutout_experience`는 사진 선택 부담으로 이탈하지 않도록 능동 `건너뛰기`를 항상 제공한다. 표의 예외 조건은 별도로 하단 `다음` CTA도 열어, 생성 중 실패한 사용자가 갇히지 않게 한다.
...
- **보류 스텝 6종의 처분**: `UsageGuessStep`·`YesterdayScreenTimeStep`·`LiveRankingStep`·`PhoneManageStep`·`NotificationPermissionStep`·`GromoStartStep`은 시퀀스에 없다. 되살리든 지우든 결정이 필요하다.
...
| 수집값 인메모리 보관 | 중도 이탈이 기기에 흔적을 남기지 않음 | 앱이 죽으면 처음부터 — 조건부 스텝 포함 최대 12개를 다시 밟아야 한다 |
...
| 누끼 체험 건너뛰기 제공 | 사진 선택 부담으로 인한 이탈 방지 | 캐릭터 생성 체험 없이 기본 그로몬으로 완료할 수 있다 |

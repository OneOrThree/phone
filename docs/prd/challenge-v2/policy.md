...
⚠️ **정본의 이 문단은 2026-08-11에 두 번 정정됐다. 인용문은 2차 정정 후 문면이다.**

**1차 — 「측정 경계」 근거 철회.** 초판은 *"15분 눈금이라 그보다 좁은 간격은 눈금 하나가 두 창에
걸친다"* 며 측정 경계를 들었는데, 그 모델이 틀렸다 — 눈금은 벽시계가 아니라 **하루 누적 사용량**이다.
반례: A에서 10분 쓰고 **15분 넘게 쉬고** B에서 5분 쓰면, A 구간엔 아무 눈금도 안 터져 **A=0분**,
B에서 누적 15분에 도달해 **B=15분**으로 잡힌다(실제 A 10분·B 5분).
**간격을 벌려도 오차가 남는다** — 출처가 창 인접이 아니라 양자화다.

**2차 — 「두 창이 서로 다른 행동을 잰다」 철회.** 1차 정정 후에도 남아 있던 이 문장이 거짓이다.
**간격은 행동 분리를 보장하지 못한다.** E5 경계에서 실제로 통과하는 반례: A `09:00–10:00` ·
B `10:15–11:00`(간격 정확히 15분) 에서 `09:50–10:25` 를 **끊지 않고** 집중하면 A 10분·B 10분으로
**두 목표가 모두 달성된다.**

**다만 이중 계상은 없다** — 창별 집계가 `LEAST(유효종료, winEnd) − GREATEST(시작, winStart)` 로
창마다 잘라 더하므로(`FocusSessionRepository.sumOverlapSecondsInWindow`) A 몫 `09:50–10:00` 과
B 몫 `10:15–10:25` 는 **서로소**다. 깨지는 건 「분리 보장」이고 남는 건 「비용 부과」다
(위 반례에서 20분을 얻으려 35분을 연속 집중했고 15분은 어디에도 안 잡힌다).
**진짜 휴식 강제는 간격이 아니라 세션 단절 판정이 필요하다 — 잔존 위험으로 수용.**

> **이 배치에서 GROMO-1270 이 구현한 것은 판정식(요일 ∧ 간격)이지 「분리 보장」이 아니다.**
> 완료 조건도 그렇게 읽어야 한다.

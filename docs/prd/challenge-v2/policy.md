...
>
> **왜 15분 간격인가 — 근거는 중복 보상이다.** 창 A가 끝나자마자 창 B가 시작하면 **하나의
> 연속된 행동**이 두 목표에 기여해 보상이 둘 나온다. 가장 거친 측정 단위(스크린타임 15분)만큼은
> 떨어뜨려야 두 창이 서로 다른 행동을 잰다고 말할 수 있다. 창 FOCUS의 5분 관용치도 이 안에 들어온다.

⚠️ **정본의 이 문단은 2026-08-11에 정정됐다.** 초판은 *"15분 눈금이라 그보다 좁은 간격은 눈금
하나가 두 창에 걸친다"* 며 **측정 경계**를 근거로 들었는데, 그 모델이 틀렸다 — 눈금은 벽시계가
아니라 **하루 누적 사용량**이다. 반례: A에서 10분 쓰고 **15분 넘게 쉬고** B에서 5분 쓰면,
A 구간엔 아무 눈금도 안 터져 **A=0분**, B에서 누적 15분에 도달해 **B=15분**으로 잡힌다
(실제 A 10분·B 5분). **간격을 벌려도 오차가 남는다** — 출처가 창 인접이 아니라 양자화다.

**GROMO-1270 구현에 미치는 영향은 없다**(판정식은 그대로). 다만 **이 조항이 측정 오차를
해소한다고 읽으면 안 된다** — 창별 스크린타임은 창당 최대 한 눈금(15분)까지 어긋날 수 있다.

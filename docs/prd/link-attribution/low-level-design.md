...
한 Flyway 파일로 rename · 컬럼 추가 · 신설 3개를 적용한다. 번호는 구현 시점의 최대 번호 다음이다 — #745 가 V51·V52 를 쓰므로 **V53 이상**이고, 열린 PR 의 번호까지 확인한다. **데이터 이관은 없다.** slug·id·클릭 이력·인덱스는 그대로 남는다.

```sql
-- 1) rename — FK·인덱스·제약은 이름과 무관하게 따라온다. 이름 정리는 하지 않는다
...
### 1.3 `links` (구 `group_invite_links`)
...
ALTER TABLE public.links ADD CONSTRAINT ck_links_type_shape CHECK (
...
ALTER TABLE public.links ADD CONSTRAINT ck_links_revoked CHECK ((status = 'REVOKED') = (revoked_at IS NOT NULL));

-- V21 의 전체 unique 를 「active INVITE 1개」 부분 unique 로 바꾼다 (그룹 획득 LLD §2.1)
ALTER TABLE public.links DROP CONSTRAINT uq_invite_links_group_inviter;
CREATE UNIQUE INDEX uq_links_invite_active ON public.links (group_id, inviter_id) WHERE type = 'INVITE' AND status = 'ACTIVE';
CREATE INDEX idx_links_campaign ON public.links (campaign_id) WHERE campaign_id IS NOT NULL;
```

- 기존 행은 전부 `type=INVITE`·`status=ACTIVE` 가 된다. 기존 초대 링크의 동작은 바뀌지 않는다.
...
### 1.4 `link_clicks` (구 `invite_link_clicks`)

rename 만 한다. 컬럼·인덱스(`idx_invite_clicks_match` · `idx_invite_clicks_link` · `idx_invite_clicks_device`)는 그대로다. Referrer 설치는 링크가 없을 수 있어 이 테이블에 넣지 않는다(§1.5).
...
CREATE TABLE public.install_referrers (
    ...
    click_id          uuid          REFERENCES public.link_clicks (id),
    gclid             varchar(256),
...
CREATE UNIQUE INDEX uq_install_referrers_device ON public.install_referrers (device_id);
CREATE INDEX idx_install_referrers_link   ON public.install_referrers (link_id, received_at) WHERE link_id IS NOT NULL;
CREATE INDEX idx_install_referrers_meta   ON public.install_referrers (meta_campaign_id, received_at) WHERE meta_campaign_id IS NOT NULL;
CREATE INDEX idx_install_referrers_source ON public.install_referrers (source, received_at);
```
...
CREATE INDEX idx_skan_postbacks_received ON public.skan_postbacks (received_at) WHERE signature_valid;
```

기기·유저 식별자 컬럼이 없다. 캠페인 귀속은 §2.6 규칙으로 읽기 시점에 한다.

### 1.7 엔티티·패키지

`com.oneorthree.phone.invitelink` 를 `com.oneorthree.phone.link` 로 옮긴다. `GroupInviteLink` → `Link`, `InviteLinkClick` → `LinkClick`, 신규 `Campaign` · `InstallReferrer` · `SkanPostback`. 공개 컨트롤러(`LinkPublicController` · `WellKnownController` · `InviteLinkController`)는 [HLD §7](high-level-design.md#7-배포-순서) 5단계까지 남긴다.
...
활성이면 `link_clicks` 에 한 행을 넣고(`ip_hash = SHA-256(ip + LINK_IP_SALT)`, 현행 `IpHasher`) `LandingView` 를 돌려준다. 비활성이면 행 없이 `state=EXPIRED` 뷰를 돌려준다. 없는 slug 는 404.
...
1. 같은 `device_id` 행이 있으면 **그 행을 그대로 돌려준다**(먼저 온 쓰기가 이긴다, `uq_install_referrers_device` 충돌 시 재조회).
...
4. 응답의 `matched` 는 **활성 링크로 귀속된 LINK 일 때만** `true` 다. META·GOOGLE 은 `matched=false` 지만 앱은 `source` 로 fingerprint 생략을 판단한다(정책 L12).
...
| 입력 | 규칙 | 응답 |
| --- | --- | --- |
| `slug` (INVITE) | 현행 그대로: 그 링크의 가장 최근 `matched=true`·미claim 클릭에 붙인다. `deviceId` 가 오면 `matched_device_id` 가 같은 클릭을 먼저 찾는다. 셀프 초대·이미 claim 은 no-op | 200. 없는 slug 는 현행 `SLUG_NOT_FOUND` 유지 |
...
2. **캠페인**: 그 채널에서 `campaigns.skan_source_identifier` 가 포스트백 `source_identifier` 와 같으면 그 캠페인이다. 포스트백 자릿수가 더 적으면(크라우드 익명성이 낮을 때 2~3자리) 앞자리가 일치하는 캠페인이 **정확히 하나일 때만** 귀속하고, 둘 이상이면 채널 단위로 남긴다.
3. 집계는 `signature_valid = true` 이고 `app_id` 가 우리 앱인 행만 쓴다.

### 2.7 탈퇴

`UserService.erasePersonalData` 를 부르기 **전에**(그 메서드는 영속성 컨텍스트를 비우는 탈퇴 절차의 마지막이다) 같은 트랜잭션에서 두 벌크 UPDATE 를 실행한다.
...
```jsonc
// LandingView — INVITE
{"type": "INVITE", "slug": "k3m9x2pa", "state": "ACTIVE", "groupName": "새벽 공부방", "inviterName": "재영", "destination": null}
// LandingView — CAMPAIGN, 폐기
{"type": "CAMPAIGN", "slug": "q7w2e4rt", "state": "EXPIRED", "groupName": null, "inviterName": null, "destination": null}
...
// POST /internal/install-referrers 요청
{
  ...
  "decryptFailed": false,
  "referrerClickAt": "2026-09-13T01:02:03Z", "installBeginAt": "2026-09-13T01:04:10Z", "installVersion": "1.4.0"
}
// ReferrerResult
...
```jsonc
// POST /internal/skan-postbacks 요청 — 정규화 필드 + 원문
{"transactionId": "…", "version": "4.0", "adNetworkId": "…", "appId": 6774498679, "sourceIdentifier": "5239",
 ...
 "signatureValid": true, "raw": { "…Apple 원문 그대로…": "" }}
```
...
| `installs.referrer` | `install_referrers.received_at` 이 기간 안, LINK 는 링크의 캠페인, META 는 `external_id` 대조 | 결정 |
...
| `POST /l/match` | 없음 | `{os, deviceId, appInstanceId?}` | 200 `MatchResult`, 실패는 `{matched:false}` |
| `POST /l/referrer` | 없음 | `{os:"android", deviceId, appInstanceId?, referrer, referrerClickTimestampSeconds?, installBeginTimestampSeconds?, referrerClickTimestampServerSeconds?, installBeginTimestampServerSeconds?, installVersion?}` | 200 `ReferrerResult`, 실패는 `{matched:false, source:"UNKNOWN"}` |
...
| `POST /.well-known/skadnetwork/report-attribution/` (끝 슬래시 없는 경로도) | 없음(서명) | Apple JSON | 200 · 400 JSON 아님 · 500 저장 실패 |
...
- 시각은 `…ServerSeconds` 를 우선하고 없으면 기기 시각을 쓴다.
...
- 처리 순서: JSON 파싱(실패 400) → `app-id` 가 우리 앱(`6774498679`)이 아니면 저장 없이 200 → `attribution-signature` 를 **버전별 Apple 공개키**로 검증 → data-api 저장(실패 500) → 200.
- 서명 대상 필드의 순서·구분자는 버전마다 다르다. Apple 문서의 버전별 규칙을 코드 상수로 두고, 모르는 버전은 `signatureValid=false` 로 저장한다.
...
- Cloudflare 봇 챌린지·WAF 가 이 경로의 POST 를 막지 않게 예외를 둔다(Infra).
...
- `/l/match`·`/l/referrer` 는 IP 당 분당 30회로 제한한다(business-api Redis, 키 `linkrl:<route>:<ip>`). 초과도 200 `{matched:false}` 다.
...
- **Android 스토어 버튼은 신규다**(현 템플릿엔 App Store 버튼만 있다). 두 템플릿 모두 `androidStoreUrl` 을 받아, 설정값 `LINK_ANDROID_PACKAGE` 가 비어 있으면 버튼을 숨긴다. 값이 있으면 `https://play.google.com/store/apps/details?id=<패키지>&referrer=<URL 인코딩된 "slug=<slug>&click=<clickId>">` 다.
...
| # | 변경 | 호환 |
| --- | --- | --- |
| 1 | 파서: `g=` 없는 `/l/{slug}` 도 유효. 매치 응답 `type` 분기 — CAMPAIGN 이면 `destination` 으로 이동, 모르는 목적지는 앱 홈 | 서버는 구 필드 유지 |
| 2 | Android 첫 실행: `InstallReferrerClient` 로 읽어 `POST /l/referrer` 1회. 응답을 받으면 완료 플래그. `source` 가 LINK·META·GOOGLE 이면 `/l/match` 생략, 아니면 기존 매치. `attributionId` 보관 | 구 앱은 호출하지 않을 뿐 |
...
- env: `CONSOLE_SLOT_<NAME>_HASH`(bcrypt, 팀원 수만큼) · `CONSOLE_COOKIE_KEY`(32바이트 이상). 슬롯 이름은 env 이름에서 뽑은 짧은 영문이고 `X-Actor`·`created_by` 에 쓴다.
...
| 잠금 | Redis `console:fail:slot:<slot>` 5회/15분, `console:fail:ip:<ip>` 20회/15분. 넘으면 "15분 뒤 다시 시도하세요" |
...
| 없는 slug | 만료 랜딩 200 | — |
| match·referrer 에서 data-api 오류·무응답(3초) | 200 `{matched:false}` | WARN |
| referrer 복호화 실패 | 200 `{source:"META", matched:false}` | `decrypt_failed=true` |
| SKAN JSON 아님 | 400 | WARN |
| SKAN 다른 앱 | 200 | 미저장, WARN |
| SKAN 서명 실패 | 200 | `signature_valid=false`, INFO |
| SKAN 저장 실패(5초) | 500 | ERROR + 원문(식별자 없음) |
| claim 대상 없음·기기 불일치 | 200 | DEBUG |
...
| 스키마 | V52 의 `invite_claim_intents` · `invite_claim_confirmations` · `invite_click_migrations` · `invite_click_frozen_rows` · `invite_link_frozen_rows` 를 DROP 하는 마이그레이션. **실행 전 행 수 0 확인, 0 이 아니면 멈추고 보고한다.** V52 의 `group_members` 변경은 용도를 확인해 membershipEpoch 전용이면 함께 되돌린다 |
...
| 층 | 검증 |
| --- | --- |
| data-api (Testcontainers) | 마이그레이션 뒤 기존 slug 의 방문·매치·claim 회귀 · `ck_links_type_shape` 위반 거절 · active 부분 unique 에서 동시 발급이 한 slug 로 수렴 · 같은 `device_id` 동시 referrer 2건이 1행 · LINK referrer 가 클릭을 소진해 다른 기기 fingerprint 가 그 클릭을 못 가져감 · 캠페인 slug claim 의 기기 불일치 no-op · `attributionId` 기기 불일치 no-op · SKAN 재전송 1행 · 탈퇴 뒤 두 테이블 `claimed_user_id` NULL · 집계의 KST 경계(`to` 날 23:59 포함, 다음 날 00:00 제외) · SKAN 자릿수 축소 시 후보 2개면 채널 단위 |
| business-api | referrer 판별 표(§4.1 다섯 규칙 + 깨진 인코딩 · 빈 문자열 · 변조된 Meta payload) · SKAN 서명 검증(버전별 Apple 문서 예시 포스트백을 픽스처로) · 다른 `app-id` · match·referrer 가 data-api 5xx·타임아웃에도 200 `{matched:false}` · 강등 랜딩 · IP 신뢰(사설 피어만) · 레이트리밋 초과도 200 · 콘솔 잠금 · 쿠키 변조 · 슬롯 제거 즉시 무효 · 폼 토큰 재사용 거절 |
| 전환 리허설 (dev) | nginx 전환 전후로 같은 slug 의 클릭 → 매치 → claim 이 끊기지 않음, 원복 reload 도 같음 |

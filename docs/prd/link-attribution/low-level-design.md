...
| `POST /internal/campaigns/{id}/links` | 콘솔 | `{destination}` — `campaigns` 행 배타 락 아래 ACTIVE 확인 → link INSERT(§2.5) | 201 `{slug, url, playStoreUrl, destination}`(`playStoreUrl` 은 `slug` 만 심은 콘솔용이고 캠페인 `platform` 이 `IOS` 면 `null`, §4.4) · 409 `CAMPAIGN_ARCHIVED` · 409 `CAMPAIGN_LINK_ISSUE_DISABLED`(`CAMPAIGN_LINK_ISSUE_ENABLED=false`, §6.1) · 422 `DESTINATION_NOT_ALLOWED` |

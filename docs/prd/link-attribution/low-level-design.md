...
| `installs.fingerprint` | `link_clicks.matched_at` 이 기간 안 이고 그 클릭을 가리키는 `install_referrers.click_id` 가 없음 — **이때 「가리킴」은 `installs.referrer` 와 **같은 조건을 통과한 행**만 센다(source LINK·META, `COALESCE(install_begin_at, received_at) >= REFERRER_COUNT_SINCE`). 집계에서 빠지는 소급 referrer 가 과거 fingerprint 설치를 소급해서 지우면 이미 보고한 수치가 1 → 0 으로 줄기 때문이다. 클릭 `os` 가 캠페인 `platform` 과 맞을 때만(`ALL` 은 둘 다) | 확률 |

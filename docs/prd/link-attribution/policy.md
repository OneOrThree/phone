...
| L07 | SKAN 포스트백의 귀속은 **기본이 광고 네트워크(= 채널) 단위**다. 캠페인 단위는 광고 네트워크가 자기 캠페인과 source-identifier 대응을 알려줄 때만 콘솔에서 수동 매핑한다 | source-identifier 는 광고 네트워크가 정한다. 우리가 발급해 광고 계정에 넣는 값이 아니다. 매핑된 자릿수보다 포스트백 자릿수가 적어 후보가 둘 이상이면 채널 단위로 남긴다 |
...
| L17 | 개인정보: 원본 IP 는 저장하지 않는다(현행). `link_clicks`·`install_referrers` 의 `claimed_user_id` 는 **탈퇴 트랜잭션에서 NULL** 로 만든다. SKAN 포스트백에는 기기·유저 식별자가 없다 | A22 ⓐ 의 익명화 요구를 같은 트랜잭션으로 닫는다 |
| L18 | `/l/match`·`/l/referrer` 는 **어떤 실패에도 200 `{matched:false}`** 로 응답한다. SKAN 수신은 저장 실패만 5xx 이고, 서명 실패·중복·다른 앱의 포스트백은 200 이다 | 앱은 4xx·5xx 를 받으면 재시도 루프를 돈다(`InviteMatchResponse` 계약). SKAN 저장 실패는 Apple 의 재전송에 기대지 않고 로그로 잡는다 |

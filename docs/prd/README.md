...
| Catus 2.0 | 현재 개발 중인 2.0 제품과 1.x→2.0 전환 계약 | `app/app-dev/` | [`fishcat/`](fishcat/) |

## 분류 원칙

- 기존 GROMO의 그룹·챌린지·코인·스크린타임·그로몬 기능은 `gromo/`에 둔다.
- Catus의 섬·고양이·낚시·물고기 기능과 신규 API 계약은 `fishcat/`에 둔다.
- Catus 출시를 위해 기존 GROMO 데이터나 API를 승계하는 문서는 `fishcat/`에 둔다. 문서 안에서 `gromo/` 문서를 호환 근거로 참조할 수 있다.

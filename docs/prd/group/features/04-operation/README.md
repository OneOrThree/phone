...
- **출시:** OWNER/MEMBER 권한, 위임 후 이탈, 강퇴, 공지 권한 rollback, 늦은 응답의 E2E·접근성 회귀를 통과한다. 2xx 뒤 최신 detail·목록 수렴을 확인하고, 권한·검증·네트워크 4xx/5xx에는 false success·낙관 상태 잔류가 없어야 한다. `NOT_FOUND|MEMBER_ONLY`는 최신 서버 응답 또는 후속 detail·목록으로 이미 나감·대상 부재가 확인될 때만 멱등 종료로 수렴한다.

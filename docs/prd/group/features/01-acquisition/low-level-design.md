...
| 구현 상태 | **🟡 일부 구현** — 주요 흐름은 구현됨; 생성 submit의 같은 순간 이중 실행 잠금과 비공개 그룹의 초대 링크 서버 강제는 미구현 |
...
| 비공개 그룹 가입 강제         | UI는 초대 링크 경로를 제공         | 서버 링크 수신자 검증은 미구현      |
...
7. 비공개 그룹은 공개 검색에 노출되지 않으며, 서버가 `inviteSlug`를 가입 허가 조건으로 강제하기 전에는 링크 수신자 전용 가입을 완료로 판정하지 않는다.

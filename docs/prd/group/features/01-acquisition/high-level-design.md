...
- 초대 가입은 기존 `JoinGroupRequest.appInstanceId`를 best-effort로 전달한다. 검색 가입은 아직 전달하지 않으며 `GRP-01` 후속 앱 작업에서 같은 계약으로 맞춘다. 값이 있으면 서버 GA4와 S-LOG에, 없으면 S-LOG에만 성공 결과를 남기며 분석 식별자 조회 실패가 가입을 막지 않는다.

### 2.1 구형 초대 링크 전환

| 그룹·링크                            | 서버 강제 이후 계약                                                                  |
| ------------------------------------ | ------------------------------------------------------------------------------------ |
| 공개 그룹 + slug 없는 구형 링크      | 정원·멤버십 같은 기존 가입 조건으로 허용. slug는 권한 증명이 아니라 attribution 없음 |
| 공개 그룹 + 현재 slug 링크           | 기존 가입 조건으로 허용하고 유효한 slug만 attribution에 사용                         |
| 비공개 그룹 + 현재 유효한 slug       | slug가 해당 그룹과 일치할 때만 가입 허용                                             |
| 비공개 그룹 + 누락·무효·타 그룹 slug | 동일한 일반 오류로 거절하고 멤버십을 만들지 않음                                     |

...
- **앱:** 생성·찾기·초대 UI, 현재 비공개 안내 완화, 구형 비공개 링크의 새 링크 재요청 안내, 검색·초대 가입의 식별자→시도 이벤트→API 순서, 중복 요청 잠금, 성공 뒤 목록 재확인과 `groupId` 목적지 전환.
- **서버:** 비공개 가입 시 그룹·slug·발급자 활성 멤버십·폐기 상태를 강제하고, 멤버십 이탈 시 링크 폐기와 구형·직접 요청 오류 계약을 확정.
- **QA/분석:** 직접·복원·검색 가입의 GA4/S-LOG 결과와 `result_track`, 0개 목록 전 cold invite F3, 생성·가입 성공 뒤 목록 실패, 중복 탭·늦은 응답을 회귀 검증한다.

...
| 구현 상태 | **🟡 일부 구현** — 핵심 흐름은 구현됨, 집중 버튼 중복 전환 잠금과 열린 화면의 자정 경계 자동 갱신은 미구현 |
| 별도 문서 | 챌린지의 상세 실행 계약과 테스트는 별도 챌린지 LLD로 이관 예정                                             |
...
| 검증                                                    | 상태                     |
| ------------------------------------------------------- | ------------------------ |
| 상세·공지 중 한 영역 실패 시 다른 영역과 집중 진입 유지 | ✅ 구현·화면 테스트 있음 |

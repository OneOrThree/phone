...
| 구현 상태 | **✅ 주요 흐름 구현됨** — 그룹방 조회·부분 실패·공지 권한·집중 진입이 현재 코드에 있음 |
| 별도 문서 | **챌린지는 그룹방 안에 노출되지만 상세 정책·설계·구현은 별도 챌린지 PRD로 이관 예정**  |
...
```mermaid
flowchart TD
    Enter["방 진입 · 복귀 · 새로고침"] --> Load["그룹방 정보 독립 조회"]
    Load --> D["상세 · 멤버"]
    Load --> N["공지"]
    Load --> Child["하위 활동 기능"]
...
- 상세와 공지는 서로 독립적으로 성공하거나 실패할 수 있다. 한 영역 실패가 다른 영역과 집중 진입을 막지 않는다.
- 하위 활동 기능의 오류는 그룹방 전체를 거짓 빈 상태로 만들지 않는다.
...
- 챌린지 관련 화면·API·동시성·계측·테스트는 별도 문서가 생기면 그 문서를 정본으로 연결한다.

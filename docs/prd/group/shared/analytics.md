...
- 같은 episode의 최초 `group_created` 또는 **선행 `group_join_attempted`와 `join_method`가 일치하는** `group_joined(search|invite|deferred_invite)`만 전환 1건으로 센다. `user_pseudo_id`가 같고 30분 이하인 결과만 인정하며 재전송·두 번째 성공은 무시한다. 이후 성공한 전체 0개 목록을 다시 본 시점에만 새 episode를 연다.

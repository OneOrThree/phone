...
| 공통    | `group_viewed`                  | screen·C     | `GroupScreen` focus cycle당 1회                | `group_entry`; rerender 제외                                              |
...
| 01 획득 | `group_joined`                  | result·S     | 서버가 가입을 commit                           | `join_method`; 앱 중복 발행 금지                                          |
...
| 04 운영 | `group_left`                    | result·S-LOG | 서버가 나가기·강퇴를 commit                    | GA4 귀속 확인 전 운영 로그로만 사용                                       |
...
| `group_count_bucket` | `1 \| 2_5 \| 6_10 \| unknown`                                             |
...
7. 이벤트·대시보드 책임 역할과 DebugView 증거가 [구현 상태 정본](./implementation-status.md)에 배정되기 전에는 해당 분석 게이트를 완료로 표시하지 않는다.

...
| --------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| 01 획득<br>`GRP-01`         | 🟡 일부 구현             | 공개 찾기, 직접·복원 초대, 생성·가입, 성공 뒤 전체 목록 재확인과 초대 대상 방 이동                              | 현재 비공개 UI의 invite-only 문구 즉시 완화, 비공개 가입 서버 강제, 검색 가입 `appInstanceId` 전달, 생성 submit의 같은 순간 이중 실행 잠금 | 백엔드·앱·분석·QA        | 앱은 현재 문구를 검색 비노출·링크 공유 사실로만 완화하고, 서버는 비공개 그룹과 유효한 `inviteSlug`의 API·오류 계약을 합의      | 다음 앱 배포 전 과장 문구 제거; invite-only 주장 전 서버 강제·우회 가입 회귀, 검색·초대 `group_joined` GA4/S-LOG 분기 검증 통과         |
...
- 01: 공개 검색은 비공개 그룹을 제외하지만([`GroupService.searchGroups`](../../../../back/src/main/java/com/oneorthree/phone/group/service/GroupService.java#L197-L207)), 가입 처리에는 `isPrivate`·`inviteSlug` 허가 검증이 없다([`GroupService.joinGroup`](../../../../back/src/main/java/com/oneorthree/phone/group/service/GroupService.java#L255-L307)). 생성·프로필 화면은 현재 invite-only를 보장하는 문구를 노출한다([`GroupCreateScreen`](../../../../app/src/screens/group/GroupCreateScreen.tsx#L56-L60), [`GroupProfileEditScreen`](../../../../app/src/screens/group/GroupProfileEditScreen.tsx#L303-L306)). 초대 가입은 `appInstanceId`를 전달하지만 검색 가입은 `joinMethod`만 전달해 GA4 `group_joined`가 생략된다([`GroupInviteSheet`](../../../../app/src/screens/group/components/GroupInviteSheet.tsx#L244-L254), [`GroupFindSheet`](../../../../app/src/screens/group/components/GroupFindSheet.tsx#L209-L215)).

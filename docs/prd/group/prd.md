...
| 활성화    | 7일 내 그룹 연결 행동 경험률 | 소속이 확정된 사용자 중 그룹 맥락의 집중 세션 진입     | 이벤트 계약·기준선 필요         |
...
- v1의 `7일 내 그룹 연결 행동 경험`은 **소속 확정 사용자 중 7일 안에 `focus_session_started(entry_source=group_card|group_room)`가 한 번 이상 있는 비율**로 기준선을 만든다. 진입점에서 정한 source가 `FocusCategory → FocusSession`까지 보존된 이벤트만 포함하고 `unknown|home_fab`은 제외한다. 챌린지 행동을 포함할지는 별도 챌린지·성장 실험 PRD에서 결정한다.

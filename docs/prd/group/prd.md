...
| 기준 | 2026-08-10 `origin/main`(`595bd822`) 앱·서버에서 확인한 현재 기능과 아직 검증되지 않은 제품 가설을 분리한다.                                      |
...
- 개인 카드 순서와 아이콘은 서버 그룹 속성이 아니라 현재 계정·기기의 표현으로 설계한다. 해당 탐색 UI와 로컬 저장은 하위 02 기능의 P1이며, 이 상위 생애주기 계획에서는 P2에 포함된다.
...
현재 생애주기 전체의 행동 기준선과 충분한 표본이 없다. P0~P2는 정확성·이해도·계측 품질을 통과시키고, P3에서 최소 4주 기준선을 수집한 뒤 목표값을 정한다.
...
이벤트의 의미상 소유 기능은 [통합 HLD](./high-level-design.md#6-기능별-상세-정본)에서 정하고, 공통 이름·속성·발행 규칙은 한 이벤트 사전을 사용한다. 공통 원칙은 `화면 도달 → 실제 노출 → 수락된 의도 → 성공 결과`이며, 자동 전환·다시 그리기·취소·재시도는 사용자 행동으로 세지 않는다.
...
| ----- | ------------------- | ------------------------------------------------------ | ------------------------------------- |
| P0    | 생애주기 정본 확정  | 상위 PRD·IA, 기능 경계, 이벤트·오너·현재 상태 확인     | 문서 충돌과 주체 미정 0건             |
| P1    | 획득·운영 신뢰성    | 생성·찾기·초대·가입, 권한·위임·강퇴·나가기의 실패 복구 | 중복·권한 누수·잘못된 소속 수렴 0건   |
| P2    | 소속 이후 탐색 개선 | 카드 덱·요약·집중/방 진입·첫 안내                      | 하위 카드 덱 수용 기준 통과           |
| P3    | 기준선 수집         | 획득→활동→재방문 퍼널과 정성 평가                      | 4주 이상 데이터·분모·오너 확정        |
| P4    | 성장 실험           | 그룹 가치 경험이 잔존에 미치는 인과효과                | 별도 실험 PRD·표본·MDE·중단 기준 승인 |
...
1. `7일 내 그룹 연결 행동 경험`에 포함할 도메인 성공 이벤트의 정확한 목록과 분모.
2. 생애주기 대시보드와 기능별 이벤트 품질의 오너.
3. 비활성·무응답 그룹을 사용자에게 어떻게 설명하고 재탐색을 도울지.
4. P3 기준선 이후 목표치와 P4 실험의 최소 효과 크기·표본.

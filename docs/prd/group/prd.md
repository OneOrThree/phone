...
게스트는 소셜 로그인 사용자와 동일하게 취급한다 — 그룹 탐색·생성·가입·멤버 전용 정보 접근에 별도 인증 흐름을 두지 않는다(GROMO-1509로 서버 제약 해제).
...
| G-04 | 가입         | 그룹의 가입 가능 상태·정원·공개 범위·기존 소속·강퇴 상태를 서버가 판정한다.  | 서버가 멤버십을 확정한다.                             |
...
3. **공개와 비공개 획득 경로를 섞지 않는다.** 공개 그룹은 찾기, 비공개 그룹은 초대 링크가 기본 경로다. 서버 강제 전 UI는 검색 비노출·링크 공유만 설명하고 링크 수신자 전용 가입을 보장하지 않는다.
4. **모르는 값은 0이나 없음이 아니다.** 요청 실패·부분 목록·범위 불명은 명시적 상태로 표시한다.
5. **부분 실패를 격리한다.** 공지나 하위 기능의 실패가 방 진입을, 카드 요약 실패가 전체 방을 불필요하게 막지 않는다.
6. **비가역 행동은 최신 검증과 확인을 거친다.** 생성·가입·강퇴·위임·나가기는 화면의 낡은 상태만으로 확정하지 않는다. 새 멤버십 생성과 LEFT 재활성화는 공개·비공개나 유입 경로와 무관하게 서버가 `WAITING|ACTIVE` 그룹에서만 허용한다. 이미 활성 멤버의 `ALREADY_MEMBER` 응답은 멤버십을 바꾸지 않고 기존 방 이동 계약을 유지한다.
...
| 활성화    | 최초 소속 후 7일 내 그룹 연결 행동 경험률 | `G-P3` 이후 계정 최초 멤버십 생성 시각 cohort 중 그룹 맥락의 집중 세션 진입 | 서버 cohort·GA4 연결 gate 필요  |
| 재방문    | D7 그룹 재방문율                          | 그룹 화면·방을 본 사용자 중 다른 날짜에 다시 도달                           | 기준선 필요                     |
| 운영 검증 | 권한·나가기 수렴 gate                     | 역할·오류 코드별 E2E에서 서버 결과와 최신 화면이 일치                       | v1 출시 gate; 행동 KPI 아님     |
...
v1은 `권한·나가기 성공률`을 행동 KPI에서 제외한다. 현재 운영 이벤트는 일부 API 성공 결과만 기록해 유효 시도 분모와 최신 detail·목록의 화면 수렴을 연결할 수 없다. 역할·오류별 E2E를 출시 gate로 사용하고, attempt→server commit→화면 reconciliation 상관 계약을 별도 승인한 뒤 `G-P3` 기준선으로 승격한다.
...
| `G-P1` | 획득·운영 신뢰성    | 생성·찾기·초대·가입, 권한·위임·강퇴·나가기의 실패 복구 | 과장 안내 제거, 구형 private 링크 전환, inactive 가입·중복·권한 누수·잘못된 소속 수렴 0건 |
| `G-P2` | 소속 이후 탐색 개선 | 카드 덱·요약·집중/방 진입·첫 안내                      | 02 기능의 `F02-P1` 구현·`F02-P2` 형성평가 통과                                            |
| `G-P3` | 기준선 수집         | 획득→활동→재방문 퍼널과 정성 평가                      | `F02-P3` 제한 출시와 4주 데이터·분모·분석 검증 완료                                       |
| `G-P4` | 성장 실험           | 그룹 가치 경험이 잔존에 미치는 인과효과                | `F02-P4` 확대 판단과 별도 실험 PRD·표본·MDE·중단 기준 승인                                |
...
### 9.1 `G-P3` 활성화 cohort v1

- cohort opener는 앱 화면 이벤트가 아니라 commit된 서버 snapshot에서 만든 `group_first_membership_created_v1` fact다. 인증된 비게스트 계정의 모든 `group_members` 이력에서 가장 이른 non-null `created_at`을 `t0`로 사용한다. 이 값은 Hibernate가 행 영속화 때 부여한 생성 시각이며 트랜잭션 commit 시각이라고 주장하지 않는다. rollback된 행은 snapshot에 존재하지 않는다. 그룹 생성자의 OWNER 멤버십과 검색·초대 가입은 같은 규칙으로 포함한다.
- user 연결 키는 서버 `group_members.user_id`와 앱이 JWT UUID로 설정한 GA4 `user_id`다. raw UUID를 이벤트 parameter로 다시 보내지 않는다. `user_pseudo_id`나 시간 근접만으로 서버 cohort와 집중 이벤트를 연결하지 않는다.
- `cohort_contract_start_utc` 이후에 **생애 최초 멤버십 `created_at`이 있는 계정만** 신규 활성화 분모에 넣는다. 재가입은 기존 행의 최초 `created_at`을 유지하고, 추가 그룹 가입·생성도 새 cohort를 만들지 않는다. 기준 시각 이전부터 소속인 사용자는 별도 기존 멤버 관찰군으로만 보고 신규 활성화율과 합치지 않는다.
- 서버 결과가 `ga4`인지 `s_log_only`인지는 분모에 영향을 주지 않는다. 실제 멤버십 행이 서버 이력에 있으면 포함한다. `group_joined`, `group_created`, `group_membership_reconciled`, 양수 `group_viewed`는 획득·화면 수렴 검증 신호일 뿐 cohort opener가 아니다. `ALREADY_MEMBER`·미리보기·시도 이벤트도 새 cohort를 만들지 않는다.
- 분자는 `[t0, t0 + 7×24시간)`에 같은 GA4 `user_id`로 `focus_session_started(entry_source=group_card|group_room)`가 한 번 이상 있는 계정이다. `unknown|home_fab`은 제외하고 계정당 분모·분자를 각각 최대 1건으로 dedupe한다. `t0 + 7×24시간 <= as_of_utc`인 성숙 cohort만 보고한다.
- 쿼리는 `cohort_version=group_activation_v1`, `cohort_contract_start_utc`, `as_of_utc`, 서버 snapshot과 GA4 export 날짜를 함께 기록한다. 기존 nullable `created_at`의 backfill·최초 멤버십 `created_at` 보존·GA4 `user_id` 연결을 검증하지 못하면 `G-P3` 기준선과 `G-P4` 실험을 시작하지 않는다.

### 9.2 기타 후속 결정

- 챌린지 행동을 그룹 연결 경험에 포함할지는 별도 챌린지·성장 실험 PRD에서 결정한다.

...
- **정확한 내 순위의 화면 반영** — `GET /league/me/rank`는 top-100 밖에서도 정확한 순위를 SQL로 계산해 준다. 앱이 이 API를 못 쓰게 하던 호출당 `LEAGUE_RANK_VIEWED` 활동 로그는 서버에서 제거됐다 — 이제 화면 포커스마다 호출해도 계측이 부풀지 않으므로, 앱을 배선하면 U2 스토리가 그대로 풀린다(현재는 top-100 목록 위치로 대체 중, `useLeagueRanking.ts`).

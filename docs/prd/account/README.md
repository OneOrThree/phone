...
OS 자동 표시·공유 식별자 보완: OS가 자동 표시하는 푸시 경로의 계정 간 노출 0을 data-only 검증 표시나 비식별 문구로 확보하는 것을 새 전환 흐름의 활성 조건으로 올렸다(기준 NSE는 템플릿 그대로). 설치 단위 app_instance_id는 계정 전환마다 재설정하고, 여러 사용자에게 기록된 공유 값은 GA4 삭제 입력에서 빼 다른 계정 자료를 지우지 않는다.

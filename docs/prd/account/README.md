...
전환 전 푸시·직접 참가 식별자 보완: DELETE·세션 폐기가 막지 못하는 전환 전 발송 푸시를 사용자 변경 시 FCM 토큰 교체와 등록별 deliveryTag 대조로 막고, OS 자동 표시 잔존은 한계로 적었다. 서버 GA4 MP가 user_id 없이 보내는 app_instance_id는 가입 TX에서 삭제 키 전용 등록부에 남겨 탈퇴 삭제 작업 입력으로 옮긴다.

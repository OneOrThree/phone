...
| 회차 시작 − 30분 (**창형만**) | 참여 모집 묶음 | 아직 들어올 수 있는 마지막 알림 시점. **슬롯이 조용한 시간(23–07)에 걸리면 이월하지 않고 버린다** — 07:00으로 미루면 이미 참가 마감(창 시작)이 지나 "참여하세요"가 거짓말이 된다. 창이 07:30 이전에 시작하는 챌린지는 모집 알림 없이 돈다(수용) |
...
그렇다 — 대표적으로 참여 모집(`CHALLENGE_SESSION_OPEN`)은 07:00에 도착해봐야 창 시작 전
마감이 이미 지났다. 이월 대상은 **결과·환불처럼 통지 자체가 목적인 것**으로 한정한다.

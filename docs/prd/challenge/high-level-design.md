...
**ENDED는 기존 카드 배열에 섞지 않고 별도 필드로 싣는다 (N38 보강).** 설치된 구앱은
`challengeList`의 **모든 원소를 카드로 렌더**하고 비활성 상태를 "비활성 카드"로만 처리한다 —
ENDED를 같은 배열에 넣으면 서버 먼저 배포되는 순간 그룹방에 **끝난 챌린지들이 카드로 되살아난다**.
그래서 결과 조립용 ENDED는 응답의 **별도 키**(예: `endedResultSources[]`)로 additive하게 주고,
카드 배열은 종전대로 ACTIVE만 담는다. 신규 앱만 그 키를 읽는다.

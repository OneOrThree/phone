...
flowchart TB
    subgraph app["앱 (React Native)"]
        A1["GroupRoomScreen<br/>카드 · 시트"]
        A4["GroupListScreen<br/>내 그룹 카드 덱 (0개 상태 포함)"]
        A0["root 오버레이 계층<br/>결과 모달 렌더·큐 소유 (N56)<br/>활성 조건: 그룹 흐름 focus"]
    ...
    subgraph server["서버 (Spring Boot)"]
        ...
        S7["GroupBetQueryController<br/>참가자 스코프 조회 · 결과 ack"]
...
| **root 오버레이 계층** (앱) | **결과 모달 렌더·큐의 소유** — 조회·순서·순차 노출·선점·ack **+ 노출·닫기 계측 발행 + 잔액 재조회**. 활성 조건만 그룹 흐름 focus | 결과의 판정·문구(§D3가 정본) |

**`BetJudge`가 판정을 독점한다.** 카드 진행률·참가 자격 가드·정산이 전부 같은 함수를 부른다.
규칙이 두 곳에 구현되면 "내 카드엔 달성인데 돈은 못 받았다"가 발생한다.

### 결과 모달의 소유자 — 화면도 탭 셸도 아니라 root 오버레이 계층

**트리거가 화면 좌표에서 사건으로 바뀌었으므로(N56) 모달을 어느 화면 안에 두느냐가 설계
결정이 된다.** 이전 판은 `GroupRoomScreen` 안에 "카드·시트·모달"로 묶어 뒀는데, 그 배치는
**그룹방에 들어온 사람만 결과를 본다**는 폐기된 트리거를 구조로 다시 굳힌다.

| 축 | 결정 |
|---|---|
| 소유자 | **root 오버레이 계층** — `GroupScreen`은 `MainTabs` 안이고 `GroupRoom`은 root Stack sibling이라, 탭 계층이 그리면 방이 push된 동안 그 위에 못 뜬다. `GroupListScreen`(카드 덱)·`GroupRoomScreen`(그룹방) 중 **어느 것이 활성이든 같은 큐**를 쓴다. **「그룹 흐름 focus」는 활성 조건(수명)이지 렌더 위치가 아니다** |
| 화면의 책임 | 화면은 큐를 **소유하지 않는다.** 자기 데이터만 그리고, **모달은 root 오버레이 계층이 얹는다** — 화면이 바뀌어도 모달은 살아 있다. 「그룹 흐름 focus」는 **재조회 수명 조건**이지 렌더 주체가 아니다 |
| 조회 시점 | 셸이 활성화될 때(`GET /me/challenge-results`) + **포그라운드 복귀·`BET_RESULT` 푸시 수신**. 셸 안의 **화면 전환만으로는 다시 부르지 않는다** — 재조회 계기는 아래 0개 그룹 표가 정본이고 소속 수와 무관하게 같다 |
| 배타 | 다른 blocking overlay(첫 안내 등)와 **겹쳐 띄우지 않는다.** **overlay queue에서 slot을 확보한 뒤에만 claim한다** — 결과 모달도 그 큐가 말하는 blocking overlay다(my-groups IA · challenge IA §4.3). ⚠️ **그러려면 조정자 자체가 root에 있어야 한다** — 지금 queue는 `GroupScreen` 소유라 그 화면 focus가 끝나면 대기를 중단하는데, `GroupRoom`은 root sibling이라 **push되는 순간 그 focus가 끝난다.** 결과 푸시로 방에 착지하면 root 모달이 slot을 영영 못 받는다. **조정자를 root로 올리는 것이 이 계약의 선행**이고, 우회하면 코치마크 배타가 다시 깨진다. slot 없이 claim하면 코치마크가 열린 동안 `BET_RESULT`가 와도 **가려진 채 선점·ack된다.** ⚠️ N04(공용 큐 신설 안 함)는 **앱 구현체를 새로 만들지 않는다는 뜻**이지 그 계약 밖에서 돈다는 뜻이 아니다 |
| 구현 | **후속 GROMO-1575·1576.** 이번 배치는 소유자를 지정하는 데서 멈춘다 |

**소유자를 비워 두면 구현자가 각자 다르게 답한다.** "도달한 화면 위에 연다"만 적고 누가 얹는지를
안 정하면 화면마다 자기 모달을 갖게 되고, 그 순간 큐·가드·ack이 화면 수만큼 복제된다 —
N56이 없애려던 것이 모양만 바꿔 돌아온다.

**0개 그룹 상태에서도 결과 조회는 돈다.** 모든 그룹에서 탈퇴한 사용자는 카드 덱에 카드가
**0장**이라, 이 상태를 조회 대상에서 빼면 **결과 푸시를 열지 않는 한 자기 정산 결과를 볼 경로가
하나도 없다.** 그런데 그 사람은 C8·N19로 여전히 정산 대상이고 돈이 이미 움직였다.

| 축 | 결정 |
|---|---|
| 조회 여부 | **돈다.** 소속 0개는 큐를 끄는 조건이 아니다 — 소스가 참가자 스코프라 멤버십과 무관하다(N53) |
| 렌더 자리 | 카드 덱의 **0개 빈 상태 화면 위**에 그대로 오버레이한다. 빈 상태 문구·CTA는 그룹 문서가 정본이고 모달은 그 위에 얹힐 뿐이다 |
| **큐 소진** | **종단 상태가 아니다.** "지금 미확인이 없다"일 뿐이고, 소속 0개에서도 **결과는 나중에 새로 생긴다** |
| **재조회 계기** | ① 셸 활성화(그룹 탭 진입·화면 focus 복귀) ② **앱 포그라운드 복귀** ③ **`BET_RESULT` 푸시 수신**. 셋 다 0개 상태에서도 똑같이 동작한다 |
| 폴링 | **원칙적으로 하지 않는다.** 상한·중단 조건을 새로 규정해야 한다. ⚠️ **단 하나 예외가 필요하다** — **`BET_RESULT`가 실제로 나가지 않은 사용자**는 ③을 못 받고, 화면을 계속 보고 있으면 focus·foreground도 재발화하지 않아 **세 계기가 전부 안 걸린다.** 그런데 **그 대상을 앱이 판정할 수 없다** — 권한·토큰 발급은 앱이 알지만, 서버 등록 실패(`putDeviceToken()`이 PUT 실패를 삼키고 `void` 반환, `push.ts:34-41`)·**다른 기기의 토큰 덮어쓰기**(서버는 사용자당 `deviceToken` 1개, `User.java:78-80`)·**앱 내 알림 off**(`PushNotificationService.sendIfAllowed` `:55-57`)는 전부 앱 밖에 있다. 뒤 둘은 **기기 등록이 완벽해도** ③을 지운다. ⚠️ **quiet hours는 여기 넣지 않는다 — `BET_RESULT`에서는 미발송이 아니라 이월이다**(`BetEventNotificationService:365-371`이 `sendIfAllowed` 전에 `DEFERRED`로 돌리고 종료 후 발송, N44). 이월을 미발송으로 분류하면 정상 건에도 타이머가 켜진다. 그래서 예외를 "채널 없는 기기"로 좁히는 서술은 폐기하고, **GROMO-1577이 ⓐ 서버가 「실제 발송 여부」를 알려주고 그때만 타이머를 켜는 안**(`sendIfAllowed()`가 이미 그 불리언을 반환한다) **ⓑ 푸시와 무관한 조건 없는 제한적 재조회** 중에서 고른다(policy §D3 표). **근사 술어는 쓰지 않는다.** 그 외에는 위 세 계기로 충분하다 (W2가 그룹방 쪽 정산 지연 구간을 같은 축으로 닫았다) |
| `BET_VOID_REFUND` 수신 | **재조회 계기가 아니다** — 그 사건은 푸시가 이미 알렸으니 즉시 다시 부를 이유가 없다. **다만 이것만으로 이중 통지가 막히지는 않는다** — 아래 주 |
| 왜 | GROMO-1415의 완료 기준("**그룹을 탈퇴한 참가자도 자기 정산 결과를 모달로 본다**")의 마지막 조각이다. N53이 데이터 축, N56이 화면 축을 열었어도 **0개 랜딩이 조회를 끄면 탈퇴자만 다시 사각**이 된다 |

> **환불 결과가 모달에 뜨는 것은 사유마다 다르다 — 계기에서 뺀 것으로 갈음되지 않는다.**
> `BET_VOID_REFUND`를 계기에서 빼도, 포그라운드 복귀나 다음 그룹 탭 진입에서 도는 **일반 재조회**가
> 같은 회차를 다시 싣는다(`RESULT_STATUSES`에 `VOIDED`·`REFUNDED`가 있다). 그러니 사유로 갈라야 한다.
> - **삭제 환불(`CHALLENGE_DELETED`)** — 애초에 큐에 들어오지 않는다. 서버가 `/me/challenge-results`에서
>   제외하고(FR-44-4) 앱도 `challengeResult`가 한 번 더 거른다. **N48의 이중 통지 금지는 여기에 걸린다.**
> - **인원 미달 무산 · 24시간 자동 환불** — **§D3이 결과로 친다**("돈이 움직이지 않기로 확정된 사건도
>   전부 알린다"). 이건 이중 통지가 아니라 정상 노출이다.
>
> 즉 계기에서 뺀 의미는 "**그 푸시를 받았다고 즉시 다시 부르지 않는다**"이지 "그 결과를 영영 안
> 보여준다"가 아니다. 앞선 판이 이 둘을 뭉뚱그려 적었다(codex 리뷰).
>
> **"소속 0개면 새 결과가 생길 수 없다"는 틀렸다.** C8·FR-40·N19가 **그룹 탈퇴 = 취소와 같은
> 규칙 — 시작된 회차는 정산 대상 유지(판정도 지급도 그대로)** 를 정한다. 그래서 시작된 회차에
> 참가한 뒤 모든 그룹을 나가도 **그 회차는 나중에 정산되고 결과 행이 새로 생긴다.** 첫 조회를
> 1회로 끝내면 정산 **전**에 셸이 활성화된 사용자는 그 뒤 정산이 끝나도 재조회 계기가 없어
> 머무는 내내 아무것도 못 본다 — **N56이 탈퇴자를 위해 트리거를 옮긴 목적이 0개 상태에서
> 그대로 깨진다.** 그래서 수명은 "1회 조회"가 아니라 **위 세 계기로 다시 여는 것**으로 정의한다.
...
**개설 시점이 넷이다.** ① 매일 00:05 스케줄러(보증장치) ② `join-week` / `join-next` lazy — 스케줄러만
...
**`ensureSession`이 단일 진입점**이다. `UNIQUE(bet_id, session_date)` 덕에 네 경로가 겹쳐도
안전하고, 스케줄러는 "아무도 예약하지 않은 회차"를 메우는 역할만 한다.

**비활성 요일의 단건 예약(`join-next`, N45)도 같은 진입점을 탄다** — 다음 활성일 회차를 lazy
생성한 뒤 참가한다. 카드 조회에서 미리 만들지 않는 이유는 아무도 들어오지 않은 OPEN 회차가
매번 생겨 인원 미달 무산·알림이 나가기 때문이다(N35 캐치업과 같은 함정).
...
**카드 조회는 ACTIVE만 담는다. 결과 큐는 별도 엔드포인트다 (N53 — N38 재정의).** 이 조회
응답은 최상위가 `GroupChallengeResponse[]` **배열**이라 형제 키를 둘 자리가 없고, ENDED를 그
배열에 넣으면 구앱이 **끝난 챌린지를 카드로 렌더**한다(구앱은 원소 전부를 카드로 그린다).
게다가 이 조회는 **그룹 멤버십 검증으로 시작**하므로 탈퇴자는 자기 결과를 볼 수 없다.

그래서 결과 모달 큐는 **`GET /me/challenge-results`**(참가자 스코프, 그룹·챌린지 상태 무관)에서
가져온다 — ENDED·탈퇴 두 경우가 한꺼번에 닫힌다. **삭제는 여기서 닫히는 게 아니라 제외된다** —
쿼리가 `c.deletedAt IS NULL`로 거르고 앱도 `voidReason !== CHALLENGE_DELETED`를 한 번 더 본다.
삭제 환불은 `BET_VOID_REFUND` 푸시가 알리는 사건이라 모달까지 열면 이중 통지다(정본 policy §D3
「대상」행 · N48). 카드 응답에는 "지난 결과 한 줄"용
`lastSettledSession`만 남는다.

**이 큐를 누가 들고 있는지는 §1이 정한다** — 개별 화면도 그룹 탭 셸도 아니라 **root 오버레이 계층**이다(N56). 소스가
멤버십과 무관하므로 **소속 0개 상태에서도 조회가 돈다**: 전원 탈퇴자에게는 그게 유일한 인앱
경로다.
...
sequenceDiagram
    ...
    Note over S: 시작 전 참가 → 회차 시작까지<br/>시작 후 참가(하루형) → min(참가+5분, 회차 종료)<br/>지나면 아무것도 하지 않는다 — 정산 대상으로 남긴다
...
**취소 마감은 참가 시점으로 갈린다 (N22).**

| 언제 참가했나 | 취소 마감 |
|---|---|
| **회차 시작 전** (창형·예약분) | **회차 시작까지** — 유예 없음 |
| **회차 시작 후** (하루형) | `min(참가 + 5분, 회차 종료)` |

하루형은 회차 시작이 자정이라 당일 참가자에게 "시작 전"이 영영 오지 않는다 — **5분 유예가
유일한 취소 창**이고 그게 오탭 방어의 전부다. 회차 종료 상한은 유예가 정산 구간으로 흘러드는
것을 막는다. **`max(시작, 참가+5분)`으로 쓰면 안 된다** — 창 시작 4분 전 참가자가 창이 열린
뒤까지 취소할 수 있어 "시작 후 환불 없음"이 깨진다(창 초반을 보고 발을 빼는 각도).
...
> | 변경 트랜잭션이지만 유저는 읽기만 (챌린지 생성·참여 등) | `findActiveByIdForShare` (공유) | 처리 중 **계정** 탈퇴를 막는다 |
>
> **`users` 공유 락은 그룹 탈퇴를 막지 못한다.** 그룹 탈퇴·강퇴는 `group_members`를 바꾸는데
> 참여 경로는 그 행을 잠그지 않는다 — 탈퇴가 참가 행을 보기 전에 스캔을 끝내고 커밋하면,
> 뒤이어 참가가 커밋돼 **그룹에 없는 사람의 유료 예약**이 남는다(탈퇴 정산 정리에서도 빠진다).
> 그래서 참여는 **활성 `group_members` 행을 탈퇴와 충돌하는 모드로 잠그거나**, 회차 락 획득
> 직후·차감 직전에 **멤버십을 재검증**한다.
...
- **묶음 키와 dedup 키를 분리한다** — 묶음은 `(유저 × 그룹 × 슬롯)`, **dedup은 사건 단위
  `(유저 × kind × 대상 id)`**(대상 id = 회차 id 등 그 알림이 가리키는 것). 둘을 같게 두면 슬롯이
  `SENT`가 된 **뒤에** 생긴 후보가 충돌로 스킵된다 — 13:01 개설 알림 뒤 13:03 정산 결과,
  **또는 13:01·13:03에 회차 두 개가 각각 정산**되는 경우(같은 `BET_RESULT`)까지. 어느 쪽이든
  **돈이 걸린 결과 알림이 유실**된다. 사건 단위로 잡으면 같은 kind라도 대상 회차가 다르면 별도
  클레임이라 보충 발송이 성립하고, **이미 보낸 사건만** 다음 묶음에서 빠진다.
- 한 슬롯에 미발송 사건이 여럿이면 **묶어서 한 건**으로 보낸다(N20) — 발송 후 각 사건에 대해
  `SENT`를 기록한다. 즉 클레임은 사건마다, 푸시는 슬롯마다다.
...
- 결과적으로 클레임 행의 유니크 키는 **`(user_id, kind, subject_id)`** 이고(슬롯·그룹은 묶음
  메타로 함께 저장), 상태는 `PENDING`(클레임·리스 10분) → `SENT` 또는 삭제(실패)다.
...
| 회차 시작 − 30분 (**창형만**) | 참여 모집 묶음 | 아직 들어올 수 있는 마지막 알림 시점. **슬롯이 조용한 시간에 걸리면 조용한 시간 종료 시각과 참가 마감(창 시작)을 견준다** — 종료 시점에 이미 마감이면 이월해봐야 "참여하세요"가 거짓말이라 버리고, 종료 시점에도 참가할 수 있으면 이월해 그때 보낸다(N44). **조용한 시간은 유저 설정이라 종료가 07:00이라는 보장이 없다** — "07:00로 미루면 무조건 마감 뒤"로 두면 09:00 종료 유저의 하루형 모집(마감 자정)이 영구 침묵한다. 창 시작이 조용한 시간 종료보다 이른 챌린지는 모집 알림 없이 돈다(수용) |
...
| `settle_after` − 15분 (**`SCREEN_TIME × DURATION`(하루형) 하나만 제외** — `FOCUS × DURATION`을 포함한 나머지 3조합은 전부 대상) | **사일런트** | 큐가 비워질 시간을 남긴다. **창 종료 직후가 아니라 정산 직전**인 이유는 15분 눈금 보고의 마지막 버킷이 확정된 뒤 읽어야 하기 때문이다(policy §B6). 제외 이유는 아래 11:30 항목 |
| 무효화·환불 커밋 직후 | `BET_VOID_REFUND` | 돈이 돌아온 이유를 그 자리에서 알린다 — 삭제된 챌린지의 회차는 결과 모달에서 빠지므로(FR-44-4) 이 푸시가 **유일한 통지 경로**다 (K3 해소) |
| 정산 직후 | 결과 묶음 | 결과가 확정되자마자. **조용한 시간이면 그 구간 종료 시각으로 이월**(N44) — 하루형은 자정 정산이라 그대로 두면 결과 알림이 매번 소멸한다 |
| 11:30 | 사일런트 (SCREEN_TIME 하루형) | 12:00 정산 전 마지막 보고 기회. **이 조합은 위 `settle_after − 15분` 규칙에서 제외**한다 — 둘 다 적용하면 11:30·11:45 두 번 깨우는데 사일런트는 dedup 예외라 실제로 중복 발송된다. 30분 여유가 큐 flush에 유리해 11:30을 남긴다 |

**조용한 시간** 필터는 표시 푸시에만 적용한다. 사일런트는 무음이므로 예외다 —
심야 창의 정산이 데이터를 못 받는 문제가 이걸로 풀린다.

구간은 **유저 설정**(`nightStartTime`·`nightEndTime`)이고 23–07은 기본값일 뿐이다. 아래 문단들이
"종료 시각"이라고 쓰는 곳에 07:00을 대입해 읽으면 안 된다 — 이 문서가 그렇게 굳혔다가 실제로
버그를 냈다(2026-08-10 PR #574 리뷰).

**단, 이월이 무의미해지는 알림은 버린다 (N44 단서).** 이월 시점에 **행동할 수 없게 된** 푸시가
그렇다 — 대표적으로 참여 모집(`CHALLENGE_SESSION_OPEN`)이 조용한 시간 종료 시점에 **이미 참가
마감**을 지났으면 도착해봐야 "참여하세요"가 거짓말이 된다.

**판정은 종류가 아니라 이월 시점 기준이다.** "모집은 무조건 버린다"로 굳히면 안 된다 — 조용한
시간은 유저 설정(`nightStartTime`·`nightEndTime`)이라 임의의 `HH:mm`이고 종료가 07:00이라는
보장이 없다. 야간을 07–09로 둔 유저의 하루형 08:00 모집은 **참가 마감이 자정**이라 09:00에
보내도 온전히 유효한데, 종류로 버리면 그 유저는 모집 알림을 영구히 못 받는다. 그래서 클레임마다
**조용한 시간 종료 시각 vs 그 회차의 참가 마감**을 견줘 이월/종결을 가른다. 결과·환불처럼 통지
자체가 목적인 것은 마감 개념이 없으므로 항상 이월된다.

**조용한 시간에 걸린 표시 푸시는 버리지 않고 구간 종료 시각으로 미룬다 (N44).** 버리면
`FOCUS × DURATION`이 통째로 침묵한다 — 하루형의 `closes_at`·`settle_after`가 **둘 다 자정**이라
회차 종료 알림도 결과 알림도 매번 필터 안에서 생성되기 때문이다. N40의 08:00 예외는 *모집*
알림만 구제하므로 결과는 여전히 유실된다. 클레임 행을 `DEFERRED`로 두고 종료 시각이 도래한
슬롯에서 발송하되, **묶음·dedup은 원래 슬롯 기준**을 유지한다(같은 밤에 쌓인 결과가 아침에 한
건으로 나간다). 사일런트는 예외 그대로 즉시 발송한다.

**이월 행에는 다음 시도 시각(`next_attempt_at`)을 함께 박는다** — 값은 *그 유저의* 조용한 시간
종료 시각이고, flush는 **도래한 이월분만** 집는다. 없으면 5분 flush가 종료 시각까지 같은
`DEFERRED` 전량을 매 틱 다시 잠그고 회차·참가자·설정을 재조회한 뒤 그대로 되돌려 쓴다 — 자정
정산분이 기본 구간(종료 07:00)을 기다리면 **참가자당 최대 84회**다. 불필요한 행 잠금·WAL 쓰기에 더해, 대상이 많으면
그 틱에 실제로 보내야 할 새 클레임까지 뒤로 밀린다.

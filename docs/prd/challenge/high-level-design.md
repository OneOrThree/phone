...
| 폴링 | **원칙적으로 하지 않는다.** 상한·중단 조건을 새로 규정해야 한다. ⚠️ **단 하나 예외가 필요하다** — **서버에 푸시 채널이 등록되지 않은 기기**는 `BET_RESULT`를 못 받고, 화면을 계속 보고 있으면 focus·foreground도 재발화하지 않는다. **세 계기가 전부 안 걸려** 화면을 나갔다 오기 전까지 결과가 안 뜬다. 그런 기기에 한해 **상한 있는 제한적 재조회 타이머**를 두거나 채널 유무와 무관한 정산 신호를 찾는다(GROMO-1577). **판정 기준은 권한 상태도 토큰 발급 여부도 아니라 「서버 등록 성공 여부」다** — `registerPushToken()`(`app/src/services/push.ts:227-250`)은 권한 거부(`:240`)와 `getToken()` 실패(`:245`)에 `null`을 주지만, **`putDeviceToken()`이 PUT 실패를 자기 `catch`에서 삼키고 `void`를 돌려주므로**(`:34-41`) **서버가 토큰을 못 받아도 토큰을 반환한다.** 권한·토큰 어느 축으로 적어도 **채널 없는 기기가 예외에서 빠진다.** ⚠️ **지금 앱은 이 값을 노출하지 않으므로, GROMO-1577은 `putDeviceToken()`이 등록 성공 여부를 반환하도록 고치는 것을 예외 배선의 선행 과제로 포함한다.** 그 외에는 위 세 계기로 충분하다 (W2가 그룹방 쪽 정산 지연 구간을 같은 축으로 닫았다) |
...
가져온다 — ENDED·삭제·탈퇴 세 경우가 한꺼번에 닫힌다. 카드 응답에는 "지난 결과 한 줄"용

...
flowchart TB
    subgraph app["앱 (React Native)"]
        ...
        A0["그룹 탭 셸<br/>결과 모달 오버레이 소유 (N56)"]
...
| 축 | 결정 |
|---|---|
| 소유자 | **root 오버레이 계층** — `GroupScreen`은 `MainTabs` 안이고 `GroupRoom`은 root Stack sibling이라 탭 화면이 그리면 방 위에 못 뜬다. 아래 「그룹 탭 셸」은 **활성 조건(수명)**을 가리키는 말이지 렌더 위치가 아니다. 종전 표기: **그룹 탭 셸** — `GroupListScreen`(카드 덱)·`GroupRoomScreen`(그룹방) **위에** 뜨는 오버레이. 두 화면 중 어느 것이 활성이든 같은 큐를 쓴다 |
...
**이 큐를 누가 들고 있는지는 §1이 정한다** — 개별 화면이 아니라 **그룹 탭 셸**이다(N56). 소스가

...
**조용한 시간에 걸린 표시 푸시는 버리지 않고 07:00으로 미룬다 (N44).** 버리면
`FOCUS × DURATION`이 통째로 침묵한다 — 하루형의 `closes_at`·`settle_after`가 **둘 다 자정**이라
회차 종료 알림도 결과 알림도 매번 필터 안에서 생성되기 때문이다. N40의 08:00 예외는 *모집*
알림만 구제하므로 결과는 여전히 유실된다. 클레임 행을 `DEFERRED`로 두고 07:00 슬롯에서
발송하되, **묶음·dedup은 원래 슬롯 기준**을 유지한다(같은 밤에 쌓인 결과가 아침에 한 건으로
나간다). 사일런트는 예외 그대로 즉시 발송한다.
...
정산분이면 07:00까지 **참가자당 최대 84회**다. 불필요한 행 잠금·WAL 쓰기에 더해, 대상이 많으면

...
| 폴링 | **원칙적으로 하지 않는다.** 상한·중단 조건을 새로 규정해야 한다. ⚠️ **단 하나 예외가 필요하다** — **서버에 푸시 채널이 등록되지 않은 기기**는 `BET_RESULT`를 못 받고, 화면을 계속 보고 있으면 focus·foreground도 재발화하지 않는다. **세 계기가 전부 안 걸려** 화면을 나갔다 오기 전까지 결과가 안 뜬다. 그런 기기에 한해 **상한 있는 제한적 재조회 타이머**를 두거나 채널 유무와 무관한 정산 신호를 찾는다(GROMO-1577). **판정 기준은 권한 상태가 아니라 `registerPushToken()`(`app/src/services/push.ts:227-250`)의 토큰 반환 여부다** — 이 함수는 권한 거부(`:240`)뿐 아니라 `getToken()`·`putDeviceToken()` 실패도 `catch`(`:245-249`)로 흡수해 `null`을 돌려주므로, **권한은 승인됐는데 채널이 없는 기기**가 권한 축 예외에서 빠진다. 그 외에는 위 세 계기로 충분하다 (W2가 그룹방 쪽 정산 지연 구간을 같은 축으로 닫았다) |

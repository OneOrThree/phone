...
| 폴링 | **원칙적으로 하지 않는다.** 상한·중단 조건을 새로 규정해야 한다. ⚠️ **단 하나 예외가 필요하다** — 알림 권한을 거부한 사용자는 `push.ts`가 FCM 토큰을 등록하지 않아 `BET_RESULT`를 못 받고, 화면을 계속 보고 있으면 focus·foreground도 재발화하지 않는다. **세 계기가 전부 안 걸려** 화면을 나갔다 오기 전까지 결과가 안 뜬다. 권한 없는 기기에 한해 **상한 있는 제한적 재조회 타이머**를 두거나 권한과 무관한 정산 신호를 찾는다(GROMO-1577). 그 외에는 위 세 계기로 충분하다 (W2가 그룹방 쪽 정산 지연 구간을 같은 축으로 닫았다) |

...
| **root 오버레이 계층** (앱) | **결과 모달 렌더·큐의 소유** — 조회·순서·순차 노출·선점·ack. 활성 조건만 그룹 흐름 focus | 결과의 판정·문구(§D3가 정본) |
...
| 화면의 책임 | 화면은 큐를 **소유하지 않는다.** 자기 데이터만 그리고, 모달은 셸이 얹는다 — 화면이 바뀌어도 모달은 살아 있다 |
...
| 배타 | 다른 blocking overlay(첫 안내 등)와 **겹쳐 띄우지 않는다.** 공용 오버레이 큐는 **만들지 않는다**(N04) — 애드혹 배타로 끝낸다 |

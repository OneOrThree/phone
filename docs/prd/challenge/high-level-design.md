...
| 배타 | 다른 blocking overlay(첫 안내 등)와 **겹쳐 띄우지 않는다.** **group IA의 overlay queue에서 slot을 확보한 뒤에만 claim한다**(my-groups IA · challenge IA §4.3) — 결과 모달도 그 큐가 말하는 blocking overlay다. slot 없이 claim하면 코치마크가 열린 동안 `BET_RESULT`가 와도 **가려진 채 선점·ack된다.** ⚠️ N04(공용 큐 신설 안 함)는 **앱 구현체를 새로 만들지 않는다는 뜻**이지 그 계약 밖에서 돈다는 뜻이 아니다 |
...
| 폴링 | **하지 않는다.** 상한·중단 조건을 새로 규정해야 하고, 위 세 계기로 충분하다 (W2가 그룹방 쪽 정산 지연 구간을 같은 축으로 닫았다) |

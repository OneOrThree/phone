...
> **창형 30분 그레이스의 근거**: 창을 걸친 세션(11:30~12:30, 창 09:00~12:00)은 클리핑상 30분이
> 창 안에 들어가지만, `/focus-session/start`를 앱이 호출하지 않아 **서버는 세션이 끝나야 존재를
> 안다**. 그레이스 0이면 창 안에서 집중한 30분이 통째로 사라진다.
...
| 시각 | 무엇 | 왜 그 시각인가 |
|---|---|---|
| 00:05 | 회차 자동 개설 | 자정 직후, 하루가 시작되기 전 |
| 회차 시작 − 30분 | 참여 모집 묶음 | 아직 들어올 수 있는 마지막 알림 시점 |

...
| **N37** | 정산은 **창 겹침 ACTIVE 집중 세션이 남아 있으면 틱을 스킵**하고 재시도 (EARLY는 예외 — N32 시점값 수용) | 집계는 완료 세션만 계수(`ended_at IS NOT NULL`)라 그레이스(30분)보다 오래 도는 세션의 창 안 집중분이 0으로 굳는다 — 정산 불가역(B8)이라 승자가 패자로 확정. 잠정 클리핑 포함은 "정산 후 취소" 악용을 연다. 자동 마감이 대기 상한, 24h 환불이 최후 방어선 | 잠정 클리핑 포함 / 그레이스 연장 / 현행 유지(창 안 집중분 소실) |

...
| **N41** | 알림 dedup 아이덴티티는 **(유저 × 그룹 × 슬롯 × 이벤트 종류)** — 슬롯만으로 잡지 않는다 | 슬롯이 이미 발송된 뒤 생긴 후보가 충돌로 스킵된다: 13:01 개설 알림 뒤 13:03 정산 결과면 **돈이 걸린 결과 알림이 유실**. kind를 넣으면 보충 발송이 성립하고 동시 후보 묶음은 그대로 (2026-08-09 3차 리뷰) | 슬롯 단위 클레임 (기존) / 슬롯 닫힐 때까지 후보 집계 후 일괄 발송 |
...
| **N43** | 창 사용분 보고는 **그룹 멤버 + 시작된 회차의 참가자**가 할 수 있다. 앱의 보고 대상 탐색축도 그룹 목록이 아니라 **내 OPEN 회차** | C8·N19로 탈퇴자도 시작된 회차의 정산 대상으로 남는데, 멤버 전용이면 마지막 보고를 못 보낸다 — SCREEN_TIME은 미보고 = 미달성이라 **목표를 지켜도 패배 확정**된다 (2026-08-09 4차 리뷰) | 멤버 전용 (기존) / 탈퇴 시 참가 해제(C8·N19와 충돌) |

...
> `lastSettledSession`을 챌린지마다 실어주므로 조회 1건이면 끝나고, 앱에서 날짜 역산과
> 자정 걸침 판정이 통째로 사라진다.
...
| **N24** | 결과 모달은 **내가 참가한 회차만**, 회차일 내림차순 순차 큐 | 남의 회차 결과로 화면을 막지 않는다. `lastSettledSession` 하나로 조회 1건에 끝난다 | 모든 챌린지 / 한 모달에 합침 |
...
| **N22** | 취소에 **참가 후 5분 유예**를 더한다 | 하루형은 회차 시작이 자정이라 "시작 전"이 영영 안 온다. 오탭으로 코인이 빠지고 되돌릴 수 없는 상태를 막는다 | 하루형 취소 불가 / 진행분 기준 / 참가 마감 당기기 |

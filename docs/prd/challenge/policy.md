...
|---|---|
| 트리거 | 그룹방 진입 1회 |
| 대상 | 각 챌린지의 `mySettledSessions`(**내가 참가한** 정산 완료 회차, **최근 30일·최대 10건**) 전부 — **ENDED 챌린지 포함**(N38, 카드만 안 그린다) |
...
| **N52** | **참가자 0명 회차는 결과가 아니다** — 별도 종료 상태(`UNUSED`)로 닫고 결과 큐·내역·알림에 싣지 않는다. `VOIDED`는 환불 대상이 실제로 있는 경우(정확히 1명)로 한정 | 보증 스캔이 매 활성일 회차를 세우므로 아무도 안 들어온 회차가 흔한데, 그걸 `VOIDED`로 닫으면 "참가자가 부족해 무산" **0명 결과**가 내역에 쌓이고 **직전의 진짜 결과를 밀어낸다** (2026-08-09 9차 리뷰) | 전부 VOIDED (기존) / 참가자 0이면 회차 삭제(이력이 사라진다) |

...
**두 근거 중 하나는 이 사례에서 안 깨진다.** 15분 근거(눈금 하나가 두 창에 걸친다)는 눈금이
`:00/:15/:30/:45` 정렬이라 앞 창은 `23:45–00:00`, 뒤 창은 `00:00–00:15` 눈금 안이다 —
**공유하는 눈금이 없어 경계 측정이 섞이지 않는다.** 남는 건 중복 보상 근거인데, 연속 20분이지만

...
**⚠️ 알려진 빈틈 — 자정 경계는 이 판정식이 못 본다 (미해결 · GROMO-1498).**
...
**막을지는 결정되지 않았다.** 막으려면 `86400`초 이동 구간과 요일 마스크 회전(`d±1`)이 필요하고,
그건 §A6-1(자정 걸침 금지)이 없앤 2구간 전개를 다른 형태로 되살린다. 분석은
[`challenge-v2/low-level-design.md`](../challenge-v2/low-level-design.md) §2-2-bis 에 있다.
**이 문단이 없으면 §A5만 읽은 구현자는 규칙이 완결됐다고 믿는다.**

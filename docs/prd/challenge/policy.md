...
**⚠️ 알려진 빈틈 — 자정 경계는 이 판정식이 못 본다. 막지 않기로 확정했다(2026-08-11 · 재영님).**
판정식은 **같은 날짜 안의 선형 구간**만 비교한다. 그래서 아래가 통과한다.

```
매일 23:50–23:59 집중
매일 00:00–00:10 폰금지   ✓ 통과 — 같은 날 기준 23시간 40분
                          ✗ 그러나 월 종료 ↔ 화 시작은 1분
```
...
**결정 — 막지 않는다 (2026-08-11 · 재영님).** 판정식은 지금 그대로 둔다.

| | |
|---|---|
| **왜** | 막으려면 `86400`초 이동 구간과 요일 마스크 회전(`d±1`)이 필요하고, 그건 §A6-1(자정 걸침 금지)이 없앤 **2구간 전개를 다른 형태로 되살린다.** 판정식이 "구간 하나 대 구간 하나"라는 성질을 잃는 대가가, 실제 도달 빈도에 비해 크다 |
| **무엇을 받아들이나** | 자정 양옆에 창을 둔 조합에서 **중복 보상이 가능하다**(끊기지 않은 한 번의 집중이 두 목표에 기여). 해외 기기에서는 측정 근거까지 깨진다 |
| **되돌리려면** | `conflicts()` 에 `±86400` 이동 구간과 회전 마스크 비교를 넣고 경계값 표를 그에 맞게 확장한다. 이 문단이 그때의 출발점이다 |

**그래도 이 문단을 남기는 이유** — §A5만 읽은 구현자가 규칙이 **완결됐다고 믿지 않게** 하기
위해서다. 빈틈은 실재하고, 다만 **알고 받아들인** 것이다. 분석 전문은

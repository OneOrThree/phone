...
| **N40** | 하루형 모집 알림은 **당일 08:00** (시작−30분 슬롯은 창형 전용) | 하루형의 시작−30분 = 전날 23:30 — 회차 미생성(00:05 개설) + 조용한 시간(23–07)에 이중으로 막혀 영영 못 나간다 | 모집 알림 없음 / 전날 저녁 발송 |
...
| **N44** | 조용한 시간(23–07)에 걸린 표시 푸시는 **버리지 않고 07:00으로 이월**. 묶음·dedup은 원래 슬롯 기준 유지 | 하루형은 `closes_at`·`settle_after`가 둘 다 자정이라 종료·결과 알림이 **매번** 필터에 걸린다 — 폐기하면 FOCUS 하루형의 결과 알림이 영구 침묵(N40의 08:00 예외는 모집만 구제) (2026-08-09 4차 리뷰) | 폐기 (기존) / 조용한 시간 해제 / 하루형 정산 시각 이동 |

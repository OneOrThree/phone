...
> ⚠️ **이유가 GROMO-1498 로 바뀌었다 — 「시간대를 평가조차 하지 않는다」가 아니다.**
> 종전 서술은 *"요일 교집합이 선행 게이트라 시간대 축은 평가되지 않는다"* 였는데, 이제 게이트는
> **이동 `m` 마다** 걸리고 `rotate` 가 마스크를 돌린다. `m = −1` 이면 `rotate(화목, −1) = 월수` 라
> **A의 월수와 교집합이 생겨 시각 비교가 실제로 일어난다.** 그래도 전부 허용인 것은 이동한 A가
> B에서 하루만큼 멀어지기 때문이다 — 검산(N3, `m = −1`): `b[0] − GAP = 11:45` 이고
> `a[1] + shift = 12:00 − 24h` 라 `11:45 < −12:00` 이 거짓이다.
>
> 이 표가 지키는 것은 **결과**(요일이 안 겹치면 허용)이고, 그 결과는 그대로다. 다만 **하나라도
> 409면 게이트가 아니라 회전 부호를 의심**해야 한다 — 종전처럼 "게이트가 안 걸렸다"로 읽으면
> 진짜 원인을 놓친다.

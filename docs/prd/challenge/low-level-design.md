...
>
> **⚠️ ack만으로는 부족하다 — 표시 선점(claim/lease)이 앞에 하나 더 필요하다.**
> 같은 계정의 두 기기가 동시에 조회하면 둘 다 `acknowledged=false`를 받는다. 여기서 ack를
> **노출 뒤**에 두면 두 기기가 모두 모달을 띄우고, **노출 앞**에 두면 선점 직후 렌더가 중단됐을 때
> (앱 종료·백그라운드·다른 오버레이) 서버에는 확인으로 남아 **어느 기기에서도 다시 못 본다.**
> ack는 되돌릴 수 없으므로 후자가 더 나쁘다. 그래서 **선점과 확인을 다른 상태로 둔다**(IA §4.3).
>
> | 축 | 계약 |
> |---|---|
> | 순서 | **선점(lease) → 노출 → ack.** 노출이 실제로 일어난 뒤에만 확인 처리한다 |
> | 선점 | 만료가 있는 점유. 성공한 기기만 렌더한다 |
> | 선점 실패 | **영구 제외가 아니다** — 이번 회차만 건너뛴다. 응답이 **상대 지연(`retryAfterMs`)** 을 돌려주고, 앱은 그만큼 뒤에 **1회 재조회를 예약**한다(폴링 아님). **절대 만료 시각을 실어 보내지 않는다** — 기기 시계로 해석하면 시계가 빠른 기기는 살아 있는 lease를 다시 요청해 기회를 소진하고, 느린 기기는 만료 뒤에도 한참 안 띄운다(policy B17·A13) |
> | 왜 그 예약이 필요한가 | 재조회 계기 셋(셸 활성화·포그라운드 복귀·`BET_RESULT`)에는 "lease 만료"가 없다. 선점 기기가 노출 전에 죽으면 패배 기기는 **포커스된 채로 영영 다시 시도하지 못한다** |
> | 만료된 소유권 | **렌더 직전에 활성 claim을 재검증한다.** A가 선점 직후 정지해 렌더 전에 멈추고 lease가 만료된 뒤 B가 재선점하면, A가 깨어나 **최초 성공 응답만 믿고** 띄워 두 기기가 모두 렌더한다 — 그 뒤의 `acknowledged_at IS NULL`은 이미 뜬 모달을 못 되돌린다. 선점 응답에 **claim 토큰(버전)** 을 실어 주고, 검증 실패 시 **자기 선점을 폐기하고 큐 판정을 처음부터** 한다 |
> | 상태 전이 | `선점 → 활성 claim 검증 → 노출 → ack`. 검증·노출 사이에 실패하면 lease만 만료되고 `acknowledged_at`은 비어 있어 다음 조회에서 결과가 살아 돌아온다 |
> | 저장 | 확인 상태(`acknowledged_at`)와 **분리해서** 둔다 — 만료로 풀려야 하는 값과 되돌릴 수 없는 값을 한 컬럼에 담지 않는다 |
>
> lease의 수명·저장 위치·엔드포인트 형태는 **GROMO-1577**이 확정한다. 여기서 고정하는 것은
> 위 표의 **순서와 분리 원칙**이다.
>
...
- ⚠️ **그 선택이 곧 구앱 전환 계약이다.** 플래그 방식은 구앱에 additive라 안전하지만, **구앱은
  `acknowledged`를 무시하고 claim API도 부르지 않으므로 그 기기에서는 N58이 작동하지 않는다** —
  신앱이 `claim → 노출 → ack`를 끝낸 회차를 구앱 기기가 로컬 마커 부재로 다시 띄우고, 동시 조회
  시에는 lease도 우회한다. 즉 **다기기 중복 방지는 모든 기기가 신앱일 때만 성립한다.**
  선택지는 셋이다 — ⑴ **전환 구간 중복을 수용**(구앱 기기는 지금과 같은 수준이라 후퇴가 아니다)
  ⑵ **서버 필터링**(구앱까지 덮이는 유일한 수단이나 additive 이점을 잃는다) ⑶ 강제 업데이트·
  capability 협상(비용이 크다). **policy N58은 ⑴을 수용으로 정했고**, ⑵로 갈지는 이 티켓이 정한다.
- **ack 실패 시 재시도는 계약이다**(IA §4.3) — 로컬 마커가 있어 결과를 **제외하는 분기에서도 ack를
  재시도하도록 예약**한다. 안 하면 서버에 영영 미확인이 남아 배지가 안 사라지고, 기기를 바꾸거나
  재설치하면 그 결과가 다시 뜬다. 재시도 수명은 **그 세션 안**으로 한정한다(영구화되면 조회마다
  요청이 쌓인다).
- **순서도 계약이다**: `오버레이 slot 확보 → 선점(lease) → 활성 claim 검증(+lease 원자 갱신) →
  노출 → ack`. 검증과 연장은 한 번의 쓰기여야 TOCTOU가 닫힌다.
- **수용하는 실패**: 네트워크 단절·강제 종료에서 "노출 유실 없음"과 "기기 간 정확히 1회"는 동시에
  성립하지 않는다. **기기 간 중복 노출을 수용하고 유실을 막는다**(IA §4.3의 근거 참조).
- ⚠️ **여기까지가 계약이고 나머지 상세는 GROMO-1577 몫이다** — 오프라인 큐잉 방식, claim 저장 위치,
  lease 수명 값은 이 문서에서 정하지 않는다. **현재 구현은 아직 로컬 마커 단독**이다
...
> [`docs/conventions/date-axis.md`](../../conventions/date-axis.md) 에서 한 번 정리했다.

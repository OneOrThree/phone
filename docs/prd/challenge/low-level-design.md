...
| L6 | **해외 기기에서는 창 축(KST)과 스크린타임 누적 리셋 축(기기 로컬 자정)이 어긋난다** — 아래 |
...
dayKey 와 세그먼트 경계만 로컬 축). 기기 타임존이 KST 가 아니면 **KST 자정이 로컬 자정이 아니라서
자정 양옆의 두 KST 창이 같은 로컬-일 누적 안에 들어간다.**

[§A5](policy.md) 가 자정 인접을 막는 **결정 자체는 이것과 무관하게 선다** — 남은 근거가 「중복 보상」
이기 때문이다(GROMO-1498). 다만 그 문단이 함께 들던 *"자정 리셋이 측정을 분리해 준다"* 는 **국내
기기에서만 참**이고, 이 한계가 그 조건부를 가리킨다.

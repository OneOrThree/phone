...
flowchart TB
    ...
    Q2 --> C{"POST .../ack<br/>CAS 선점"}
    C -->|"성공(내가 첫 확인)"| M
    C -->|"실패(다른 기기가 먼저)"| X3["제외 — 렌더하지 않는다"]
    M --> A["노출 직후 로컬 마커 기록"]
    A -.->|"ack 실패"| R["재노출 없이 ack만 재시도"]
```

**노출보다 선점이 먼저다.** 같은 계정의 두 기기가 동시에 조회하면 둘 다 `acknowledged=false`를
받는다. 여기서 모달을 먼저 띄우고 나중에 ack하면 `acknowledged_at IS NULL` 원자 UPDATE는 **쓰기
하나만 막을 뿐 이미 뜬 모달 두 개를 되돌리지 못한다** — 서버 가드를 두고도 결과가 두 번 보인다.
그래서 **CAS에 성공한 기기만 렌더한다.** 실패한 쪽은 "다른 기기가 이미 확인했다"는 뜻이므로 조용히
제외한다. 로컬 마커는 이 순서가 성립한 **뒤에** 기록한다(노출 직후) — ack 실패 창의 보완재라는
역할은 그대로다.

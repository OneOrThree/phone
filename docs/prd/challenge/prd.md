...
> | 결정일 | 2026-08-08 |
> | 최종 갱신 | 2026-08-09 (PR #547 코드 리뷰 15건 반영 — 결정 N31~N36 · `mySettledSessions` 큐 · K5 해소. 직전: §4.0 「조건 한눈에」 신설 · §1.1 용어 신설 · N25~N30) |
...
최신순 최대 10건)로 충분하다 — 날짜를 따로 부르지 않는다. (FR-44-1 ~ FR-44-4)
...
**FR-44-2** 데이터는 **챌린지 조회 1건**의 `mySettledSessions`(내가 참가한 정산 완료 회차,
  최신순 최대 10건)로 충분하다 — 날짜를 따로 부르지 않는다. 요일 반복에서는 챌린지마다 직전
  회차일이 달라(월수금 vs 화목) 날짜 조회로는 덮을 수 없다. **최신 1건 필드로는 큐가 성립하지
  않는다** — 참가 안 한 최신 회차가 내 결과를 가리고, 안 본 결과 여럿이 1건으로 접힌다.

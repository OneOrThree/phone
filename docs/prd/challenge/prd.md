...
> | 결정일 | 2026-08-08 |
> | 최종 갱신 | 2026-08-09 (PR #547 코드 리뷰 4차 3건 반영 — 결정 N43·N44 · 24h 데드라인 × 백오프 캡. 직전: 3차 7건 N41·N42, 2차 12건 N37~N40, 1차 15건 N31~N36·K5 해소, 미해결 K12 신설) |
...
| 참가 방법 | 회차마다 직접 참여. 주간 일괄 참여 단축 제공 (화면 문구 「이번 주 남은 날 전부」) | FR-31 |
...
| 참여 취소 | `(회차 시작 전 OR 참가 후 5분 이내) AND 회차 종료 전` → 전액 환불 | FR-39 |
...
| 묶음 단위 | 묶음 = (유저 × 그룹 × 시간슬롯) · **dedup 키는 여기에 이벤트 종류(kind)까지** (N41) | 설계 |
| 실패 | 발송 전 슬롯 선점(`PENDING`) → 성공 시 `SENT`, 실패 시 선점 해제 → 다음 틱이 다시 집는다 (발송 후 기록 방식은 다중 인스턴스에서 이중 발송) | 설계 |
| 조용한 시간 | 23–07시는 **표시 푸시만** 차단하되 **버리지 않고 07:00으로 이월**(N44) — 하루형은 자정 정산이라 폐기하면 결과 알림이 영구 침묵한다. 사일런트는 예외(즉시) | 설계 |
...
회차는 띄우지 않는다. 데이터는 챌린지 조회 1건의 `mySettledSessions`(내가 참가한 정산 완료 회차,
...
조건을 한 표로 모으는 과정에서 값이 하나로 정해지지 않은 지점이다. **여기서 판단하지 않는다** —
어디와 어디가 다른지만 적는다.

| # | 축 | 내용 |
|---|---|---|
| K1 | 시작 | **무산 판정 시각**. FR-36·HLD·IA는 "참가 마감 시 &lt;2명", LLD는 `settle()` 안(= `settle_after` 이후) — 창형은 그 사이 OPEN으로 남는다 |
| K2 | 정산 | **조기 정산 전제**. FR-24는 조건 2개인데 LLD는 `now ≥ join_closes_at`를 필수 전제로 추가 → "조기 정산은 사실상 창형에서만" |
| K3 | 알림 | **삭제 환불 푸시 타입 부재**. FR-44-4는 "푸시로 알린다"인데 FR-43 표·감지기 목록 어디에도 해당 `data.type`이 없다 |
| K4 | 알림 | **사일런트 2회 중복**. `settle_after`−15분(→ 11:45)과 11:30 규칙이 SCREEN_TIME 하루형에서 겹치는데, 사일런트는 dedup 예외라 실제로 두 번 나간다 |
...
| K6 | 개설 | **하루형 목표분 상한**. DB는 1~1440인데 FR-9-2 카피는 "최대 3시간". policy에는 하루형 상한 규정 자체가 없다 |
...
| K8 | 개설 | `canParticipate=false`(스크린타임 권한 미허용)가 이 PRD에만 있고 정본(policy)에 없다 |
| K9 | 참조 | policy §8이 `FR-1`·`FR-7`·`FR-13` 등을 **옛 뜻으로** 인용 — 현행 PRD의 같은 번호와 내용이 다르다 |
| K10 | 참조 | 알려진 한계 번호가 한 칸 어긋남 — policy §9.4 `L3`=2일 보존 vs 이 문서 §8 `L3`=회수 계정 없음 |
| K11 | 삭제 | **진행 중 삭제 경고의 수치 출처**. FR-12-1은 걸려 있는 **모든** OPEN 회차의 인원·적립금을 요구하는데, 카드 조회(`GET .../challenges`)의 `bet.session`은 **오늘 회차 1건**뿐이다 — 주간 일괄 참여로 예약된 미래 회차가 덮이지 않는다. 별도 프리플라이트를 두는지, 카드 응답을 늘리는지 정해진 곳이 없다 |
| K12 | 참여 | **비활성 요일의 "다음 회차 미리 참여" 버튼** (2026-08-09 팀 리뷰 제안). 현재는 비활성 요일 카드에 `nextSessionAt` **표시만** 하고, 미래 회차 예약 경로는 `join-week`(주간 일괄)뿐이다 — 월수금 챌린지를 화요일에 열면 "수요일 회차 참여" 버튼이 없다. 기반은 이미 있다(`ensureSession` lazy 생성 · 예약분 취소 창 = 회차 시작까지 · N15 즉시 전액 에스크로). 결정할 것: **① 허용 범위**(다음 회차 1건만 / 이번 주 남은 전부 — 후자는 `join-week`와 중복) **② `join-week` 버튼 숨김 규칙**(남은 회차 ≤1)과의 관계 **③ N14**("매 회차 직접 결심")와 상충하지 않는지. 스코프 추가라 이 PR에서 정하지 않는다 |
...
**FR-9-2** 분 단위 입력은 **시간으로 환산해 함께 보여준다**(`90분 · 1시간 30분`). 상한도 같은
  단위로(`최대 3시간`). 목표가 커질수록 "분"만으로는 감이 오지 않는다.
...
**FR-24** 팟 분배 트리거는 **① 참가자 전원 확정** 또는 **② 회차 종료 + 그레이스**.
...
**FR-39** 취소 가능 조건은 `(회차 시작 전 OR 참가 후 5분 이내) AND 회차 종료 전`이며 전액
...
**FR-44-2** 데이터는 **챌린지 조회 1건**의 `mySettledSessions`(내가 참가한 정산 완료 회차,

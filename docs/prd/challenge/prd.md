...
| K6 | 개설 | **하루형 목표분 상한 — 세 문서가 어긋난다** (2026-08-09 7차 리뷰에서 재확인). DB/API 계약은 `1~1440`, FR-9-2·개설 UX 카피는 "최대 3시간", policy에는 규정 자체가 없다. 챌린지는 **생성 후 불변**이라 계약이 넓은 쪽으로 굳으면 UI가 거부하는 값이 영구히 저장될 수 있다. **결정 필요** — ① 전 조합 180분 ② FOCUS 180분 / SCREEN_TIME은 "이하" 목표라 더 크게(예: 720분) ③ 1440 유지하고 UX 카피를 고친다. **재영님 결정 대기** |

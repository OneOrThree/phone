...
| # | 조건 | 위반 시 | 근거 |
|---|---|---|---|
| 1 | **그룹장(OWNER)만** 만든다. 게스트도 OWNER이면 허용 | `NOT_OWNER` 403 | §3.1 · 설계 · GROMO-1509 |
...
| 8 | 창형끼리 **`요일 교집합 ≠ ∅` ∧ `간격 &lt; 15분`** 이면 불가 (카테고리 무관). 요일 교집합은 **하루 이동(`m = 0, ±1`)마다** 본다 — 자정 인접이면 `월` ↔ `화` 도 걸린다(GROMO-1498) | `CHALLENGE_WINDOW_OVERLAP` 409 | FR-5 · FR-3 |
...
| 창 종료 (창형) | `CHALLENGE_WINDOW_END` | 회차 종료 (**알림** 감지 크론 · 15분 주기) | 그룹원 | **❌ 싣지 않는다** | FR-43 · FR-44 |
| 하루 마감 (일 목표형) | `CHALLENGE_ENDED` | 회차 종료 (같은 크론·디스패처) | 그룹원 | **❌ 싣지 않는다** | FR-43 · FR-44 |
...
> 이 표와 [IA §4.2](./information-architecture.md)다.
...
**결과 모달**(푸시가 아닌 인앱 트리거) — **미확인 결과가 있으면 도달한 화면 위에**, 내가 참가자였던
회차 중 결과를 안 본 것을 회차일 내림차순 순차 큐로. 트리거는 화면이 아니라 **사건**이라 카드 덱·
그룹방·결과 푸시 착지가 모두 같은 진입점이다(N56). 1회 가드는 **서버 확인 표시(ack)** 가 정본이고
세션 id 로컬 마커는 ack 실패 창의 보완재다(N58). 대상은 `SETTLED`·`FORFEITED`·`VOIDED`·
...
요일 교집합은 **하루 이동(`m = 0, ±1`)마다** 판정하므로, 자정을 사이에 둔 `월 23:50–23:59` ↔
`화 00:00–00:10` 처럼 **원래 요일이 달라도 걸린다**(GROMO-1498 · [정책 §A5](policy.md))
...
| 창 종료 (창형) | `CHALLENGE_WINDOW_END` | 회차 종료 | 그룹원 | ❌ |
| 하루 마감 (일 목표형) | `CHALLENGE_ENDED` | 회차 종료 | 그룹원 | ❌ |
...
> 나눠 둔 이유는 **앱의 레거시 딥링크 폴백이 창형 문자열에 걸려 있어서**이며, 통합하려면 앱
> 배선이 먼저다. 타입별 `data` 키와 `link` 유무는 [IA §4.2](./information-architecture.md)가 정본이다.

**FR-44** 회차 종료 푸시는 **승패를 싣지 않는다** — 스크린타임은 보고 전이라 아직 확정이 아니다.

### 4.9 결과 모달

**FR-44-1** **아직 확인하지 않은 결과가 있으면**, **내가 참가자였던 회차** 중 그것을 **회차일
  내림차순 순차 큐**로 **도달한 화면 위에** 보여준다. 트리거는 화면 좌표가 아니라 사건이라
  그룹 탭 랜딩(카드 덱)·그룹방·결과 푸시 착지가 모두 같은 진입점이다 — **그룹방을 못 들어가는
  탈퇴자도 자기 결과를 본다** (N56).
**FR-44-1-1** 1회 가드는 **서버 확인 표시(ack)** 가 정본이다. 모달이 뜬 순간 ack 하고, 세션 id
  로컬 마커는 **ack 요청이 실패한 창**에서 같은 모달의 반복을 막는 보완재로 남는다. ack 실패 시
  재노출하지 않고 ack만 재시도한다 (N58).
...
| L5 | **기기의 그날 UTC 오프셋이 `+09:00` 이 아니면** KST 고정이라 "내 하루"와 앱의 하루가 어긋난다 — 한국 타깃 서비스라 수용. (지역이 아니라 오프셋이다: `Asia/Tokyo` 는 해외지만 일치하고, DST 존은 날짜에 따라 갈린다) |

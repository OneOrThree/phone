...
> | 결정일 | 2026-08-08 |
> | 최종 갱신 | 2026-08-09 (§4.0 「조건 한눈에」 신설 · §1.1 용어 신설 · 결정 N25~N30 반영) |
...
| 실패 | 발송 실패 시 이력을 남기지 않는다 → 다음 틱이 다시 집는다 | 설계 |
...
회차는 띄우지 않는다. 데이터는 챌린지 조회 1건의 `lastSettledSession`으로 충분하다 — 날짜를 따로
부르지 않는다. (FR-44-1 ~ FR-44-4)
...
| K5 | 결과 | **`VOIDED` 모달**. FR-44-3·LLD는 대상에 포함, IA는 `lastSettledSession` 정의에서 제외 |
...
**FR-44-2** 데이터는 **챌린지 조회 1건**의 `lastSettledSession`으로 충분하다 — 날짜를 따로
  부르지 않는다. 요일 반복에서는 챌린지마다 직전 회차일이 달라(월수금 vs 화목) 날짜 조회로는
  덮을 수 없다.

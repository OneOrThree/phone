...
> §2 의 「서버 축 = KST 고정」과 정면 충돌하므로 영향 조사를 마쳤고(≈272 파일, 재계산 불가 데이터·리그 정산·날짜 멱등키 위험 — 요약은 [Fishcat 결정 로그](../prd/fishcat/decision-log.md) D8) 별도 티켓(1930 예정)에서 단계 계획을 세운다.
> **그 전까지 이 문서가 현행이며 코드·규약을 지금 고치지 않는다.**

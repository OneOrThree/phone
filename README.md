...
gromo/
  app/
    app-dev/             # React Native 앱 (Expo) — iOS 스크린타임 익스텐션 포함
    assets/              # 디자인 원본 에셋 (앱 아이콘·캐릭터·영상)
    scripts/             # 로컬 웹 실행기 (local-web.sh / local-web.command)
  server/
    data-api/            # Spring Boot 백엔드 (Java 17 + PostgreSQL)
    observability/       # dev 관측 스택 설정 (Prometheus·Grafana·Loki·Datadog)
    scripts/             # docker-compose 5종 (dev·local·prod·datadog·observability)
  loadtest/              # k6 부하테스트 하네스 (GCP 러너 + 대시보드)
  docs/                  # 팀 공유 문서 (prd/ · conventions/)
```

> 개발 가이드와 컨벤션은 `CLAUDE.md`(루트 / `app/app-dev/.claude/` / `server/data-api/`)에 정리되어 있어요.
> 부하테스트는 `loadtest/README.md`, dev 관측 스택은 `server/observability/README.md` 참고.
> DB 스키마는 Flyway 마이그레이션(`server/data-api/src/main/resources/db/migration/`)이 소스예요.

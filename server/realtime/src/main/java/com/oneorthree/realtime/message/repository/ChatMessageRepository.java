 ...
 * id 는 유니크라 그 문제가 없다.
 *
 * <h2>«더 작은 id 는 이미 보인다» — 저장 입구가 지킨다</h2>
 * id 를 INSERT 때 발급하면 같은 방 동시 발신에서 더 작은 id 가 더 늦게 커밋될 수 있고, 그러면 커서를 들고
 * 위로 스크롤하던 클라이언트가 그 한 건을 건너뛰고 안 읽음 배지가 덜 센다. 그래서 메시지 저장은 이 인터페이스의
 * {@code save} 가 아니라 {@code ChatMessageAppender} 로만 한다 — 방 잠금 아래에서 방의 마지막 id 보다 큰 id 를
 * 발급해, 방 안에서는 id 순서가 곧 커밋 순서다(GROMO-1741).
 */

...
인가는 세 층이다: SUBSCRIBE(**구독 시도 창 → Data 정본 1회**) → SEND(**발신 시도 창 → 형식 →
Data 정본 1회 → 성공 창**) → 전달 직전(`ChatOutboundChannelInterceptor` 가 **JWT 재검증 + 사건에
동반된 수신 집합 대조**).

**SUBSCRIBE 에도 상한이 필요하다** — 같은 목적지에 `subscription id` 만 바꿔 연타하면 프레임마다 동기
HTTP 가 나가 인바운드 채널과 Data API 가 고갈된다. 발신 창과 키를 나눈 이유는 응원을 보낸 직후
재구독하는(재연결 직후가 그렇다) 클라이언트가 자기 발신 때문에 구독을 거절당하지 않게 하기 위해서다.

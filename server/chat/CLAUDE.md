...
- **만료된 access token은 `UNAUTHORIZED`로 거절한다.** STOMP는 CONNECT 때 받은 토큰을
  SEND·SUBSCRIBE마다 다시 검증한다. 다른 기기가 사용자별 멤버십 캐시를 채워도 만료된 소켓은
  발신·새 구독을 할 수 없다. 앱은 토큰을 갱신하고 다시 연결해야 한다. 만료 시 기존 구독을
  강제 해제하지는 않는다. 상류의 401도 `UNAUTHORIZED`로 전달하며, 403은 캐시하지 않는 빈
  멤버십으로 처리한다.

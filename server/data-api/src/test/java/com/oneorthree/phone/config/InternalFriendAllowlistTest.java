...
import com.oneorthree.phone.internal.InternalFriendController;
...
/**
 * 친구 내부 경로 7종이 <b>실제</b> {@code application-satellites.yml} 의 business 허용목록을 통과하는지
 * 검증한다 (GROMO-1894, GROMO-1759 의 {@code InternalIslandAllowlistTest} 와 같은 방식).
 *
 * <p>경로는 <b>컨트롤러 애노테이션에서 읽고</b>, 허용목록은 <b>배포되는 yml 파일에서 읽는다</b> — 둘 중
 * 하나만 바뀌면 이 테스트가 깨진다. 겨냥하는 실패는 «세그먼트 수» 다: {@link AntPathMatcher} 의 {@code *}
 * 는 세그먼트 하나라 {@code /friend-requests/*} 는 {@code /friend-requests/{id}/accept} 를 덮지 않는다.
 */
class InternalFriendAllowlistTest {

    private static final String ID = "3f4a6c1e-0000-7000-8000-00000000abcd";
    private final AntPathMatcher matcher = new AntPathMatcher();

    @Test
    @DisplayName("친구 내부 컨트롤러의 모든 매핑이 배포되는 허용목록을 통과한다")
    void everyFriendInternalRouteIsAllowedByTheShippedFile() throws IOException {
    ...
    void allowlistDoesNotLeakIntoNeighbours() throws IOException {
        ...
        // 조회 두 줄은 GET 만 — 같은 경로의 다른 메서드로 새지 않는다.
        assertThat(allows(allow, "DELETE /internal/users/" + ID + "/friends")).isFalse();
        assertThat(allows(allow, "POST /internal/users/" + ID + "/friends")).isFalse();
        assertThat(allows(allow, "GET /internal/users/" + ID + "/friends/" + ID)).isFalse();
        // 요청 행 하나를 직접 만지는 경로는 없다 — 상태 전이는 accept·reject·cancel 이름으로만 연다.
        assertThat(allows(allow, "POST /internal/users/" + ID + "/friend-requests/" + ID)).isFalse();
    ...
    @Test
    @DisplayName("조회와 생성은 메서드로만 갈리므로 한 줄로 합쳐 두지 않았다")
    void listAndCreateAreSeparateEntries() throws IOException {
        List<String> allow = shippedBusinessAllowlist();

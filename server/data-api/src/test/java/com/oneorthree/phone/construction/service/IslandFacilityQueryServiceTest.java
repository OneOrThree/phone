...
class IslandFacilityQueryServiceTest {
    ...
    void offPreservesTheOldStubVerdicts() {
        ...
        assertThat(gates.hasGram(ISLAND)).isTrue();
    ...
    @Test
    @DisplayName("enforce=ON — COMPLETED 가 아닌 시설은 다섯 게이트가 전부 닫힌다")
    void onRejectsUncompleted() {
        ...
        assertThat(gates.hasGram(ISLAND)).isFalse();
        assertThat(gates.hasMailboxOnAnyOf(List.of(ISLAND))).isFalse();
    }

    @Test
    @DisplayName("enforce=ON — COMPLETED 시설은 다섯 게이트가 전부 열린다")
    void onAllowsCompleted() {
        ...
        when(facilities.existsCompleted(ISLAND, "gram")).thenReturn(true);

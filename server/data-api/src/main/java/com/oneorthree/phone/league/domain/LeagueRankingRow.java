 ...
 *
 * <p>{@code todayFocusSeconds} 는 조회 창 마지막 날(랭킹 조회 = KST 오늘)의 당일 집중초 — 라이브
 * 앵커와 같은 SQL 문장(같은 스냅샷)에서 나온다. 별도 조회로 당일분을 읽으면 그 사이 세션이 끝나
 * 집계된 유저의 라이브 합산(당일분 + 경과)이 완료 세션을 이중 계상한다(코드리뷰 반영).
 * 라이브 앵커가 필요 없는 조회(4-인자 생성자)에서는 0 이다.
 *

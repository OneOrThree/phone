...
public class IslandMembershipService {
        ...
        JsonNode data = publicCommands.run(command,
                ...
                () -> {
                    ...
                    return new PublicCommandResult(201,
                            InternalJson.tree(view), InternalJson.tree(List.of(created, members)));
                }).value().data();
        return InternalJson.decode(data, IslandCreatedView.class);
    }
    ...
    public CurrentIslandView switchCurrentIsland(UUID userId, UUID islandId, UUID idempotencyKey) {
        PublicCommandRequest command = new PublicCommandRequest(userId, "PUT:/me/current-island:" + userId,
                idempotencyKey, InternalJson.tree(Map.of("islandId", islandId.toString())));
        JsonNode data = publicCommands.run(command,
                () -> userQueryService.getCallerForUpdate(userId),
                ignored -> userQueryService.getCallerForUpdate(userId),
                () -> {
                    ...
                    if (islandId.equals(context.getCurrentIslandId())) {
                        ...
                        return new PublicCommandResult(200,
                                InternalJson.tree(new CurrentIslandView(islandId)),
        ...
        return InternalJson.decode(data, CurrentIslandView.class);
    }
        ...
        if (membership.isEmpty()) {
            ...
            IslandJoinRequest latest = joinRequestRepository
                    .findLatestByIslandIdAndApplicantId(
                            islandId, viewerUserId, PageRequest.of(0, 1))
                    .stream().findFirst().orElse(null);
            String membershipStatus = latest != null && latest.isPending()
    ...
    public MyIslandsView myIslands(UUID userId) {
        ...
        List<Group> islands = groupMemberRepository.findActiveMembershipsByUserId(userId).stream()
                ...
                .toList();
        List<UUID> ids = islands.stream().map(Group::getId).toList();
        Map<UUID, Integer> counts = memberCountsOf(ids);
        Map<UUID, JoinRequestRef> latest = latestRequests(userId, ids);
        List<IslandSummaryView> items = islands.stream()
                .map(island -> IslandSummaries.of(island, counts.getOrDefault(island.getId(), 0),
     ...
     * {@link IslandMovementGuards#requireObservatoryUnlocked} 의 javadoc 에 적어 뒀다.
     *
    ...
    private void requireObservatory(User user) {
        ...
        movementGuards.requireObservatoryUnlocked(current);
    }
    ...
    private UUID liveCurrentIsland(UUID userId) {
        ...
        boolean live = groupMemberRepository.findActiveMembershipsByUserId(userId).stream()
                .map(GroupMember::getGroup)
                .anyMatch(island -> island.getId().equals(stored) && IslandMovementGuards.isAlive(island));
    ...
    private List<IslandSummaryView> summariesOf(UUID userId, List<UUID> orderedIds) {
        ...
        Map<UUID, JoinRequestRef> latest = latestRequests(userId, orderedIds);
        List<IslandSummaryView> items = new ArrayList<>(orderedIds.size());
        for (UUID id : orderedIds) {
            Group island = byId.get(id);
            if (island != null) {
                JoinRequestRef ref = latest.get(id);
                items.add(IslandSummaries.of(island, counts.getOrDefault(id, 0),
                        IslandSummaries.statusOf(mine.contains(id), ref),
                        ref == null ? null : ref.getRequestId()));
            }
        }
        return items;
    }

    /** «본인 최신 신청» 일괄 해석 — 요약을 만드는 경로가 섬마다 쿼리를 내지 않게 한 번에 묶는다. */
    private Map<UUID, JoinRequestRef> latestRequests(UUID userId, Collection<UUID> islandIds) {
        if (islandIds.isEmpty()) {
            return Map.of();
        }
        return IslandSummaries.latestByIsland(
                joinRequestRepository.findRefsByApplicantIdAndIslandIds(userId, islandIds));
    }

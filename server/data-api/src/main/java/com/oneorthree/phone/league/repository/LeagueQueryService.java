 ...
 * 티어 설정 단건뿐이다.
 *
 * <p><b>호출부가 1곳뿐이라 보일러플레이트가 줄지는 않는다.</b> 그래도 만든 이유는 §3·GROMO-1655 의
 * 목표가 "조회 계층 밖의 id 조회 0건"이기 때문이다 — 한 도메인만 자기 데이터를 직접 읽으면 다음
 * 사람이 "여기는 왜 예외인가"를 물어야 한다. 호출부가 늘면 그때 갈래가 생긴다.
 *

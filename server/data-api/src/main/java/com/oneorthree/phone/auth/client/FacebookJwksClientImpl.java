...
/**
 * Facebook OIDC id_token 을 <b>로컬에서</b> 검증한다 — 제공자 API 를 매 로그인마다 호출하는 대신
 ...
 * {@link InvalidTokenErrorCode#FACEBOOK_TOKEN} 로 통일한다 — 공격자에게 어느 검증에서 걸렸는지 알려 주지 않기 위해서다.
 *
 * <p><b>5단계가 보안상 결정적이다.</b> 서명만 보면 Facebook 이 <b>다른 앱</b>에 발급한 정상 토큰도
 * 우리 키로 통과한다. {@code aud} 가 우리 페이스북 App ID 이고 {@code iss} 가
 * {@code https://www.facebook.com} 인지까지 봐야 그 토큰으로 남의 계정에 로그인하는 경로가 막힌다.
 *
 * <p>JWKS 응답은 캐시하지 않는다 — Facebook 이 키를 회전해도 다음 로그인부터 바로 새 키를 집는다.
 */
@Component
public class FacebookJwksClientImpl implements SocialLoginClient {
    ...
    /**
     * @param jwksUrl {@code facebook.jwks-url} — Facebook 공개키 목록 엔드포인트. 매 검증마다 조회하므로
     *                여기가 막히면 Facebook 로그인 전체가 멎는다
     * @param clientId {@code facebook.client-id} — {@code aud} 클레임과 대조할 우리 페이스북 App ID.

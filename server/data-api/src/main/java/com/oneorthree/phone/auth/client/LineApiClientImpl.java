...
/**
 * LINE 액세스 토큰을 LINE 프로필 API 로 <b>왕복 검증</b>한다 — JWKS 로컬 검증을 쓰는
 * Apple·Google·Facebook 과 달리, 토큰 자체를 해석하지 않고 LINE 에게 "이 토큰의 주인이 누구냐"를 묻는다.
 * 따라서 매 로그인이 외부 API 호출 1회에 의존하며, LINE 이 응답하지 않으면 이 제공자 로그인만 멎는다.
 *
 * <p>토큰은 Authorization: Bearer 헤더로 실어 {@code /v2/profile} 를 부르고 응답의 {@code userId} 를 식별자로 삼는다.
 * 4xx 는 모두 {@link InvalidTokenErrorCode#LINE_TOKEN} 로 접어 만료·위조·권한 부족을 구분하지 않는다.
...
public class LineApiClientImpl implements SocialLoginClient {
    ...
    /**
     * @param baseUrl {@code line.api-base-url} — LINE 프로필 API 의 호스트. 프로파일별로
     *                갈아끼울 수 있게 상수가 아니라 설정값이며, 여기를 잘못 가리키면 검증이
     *                통째로 무력해지므로 신뢰할 수 있는 LINE 도메인이어야 한다
     */

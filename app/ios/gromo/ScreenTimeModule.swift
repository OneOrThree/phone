...
class ScreenTimeModule: NSObject {
    ...
    @objc func startFocusShield(
        ...
        ScreenTimeModule.applySafariWebBlock(to: store, allowSafariWeb: allowSafariWeb)
        resolve(true)
    }

    // 사파리 차단 + 웹 콘텐츠 필터를 토글 값에 맞춰 적용/해제 (GROMO-866).
    // 끔(기본) = 차단: 실드와 달리 번들 ID로 직접 지정 가능한 blockedApplications로 사파리를
    // 숨기고, 웹 콘텐츠 필터로 사파리·타 브라우저·인앱 웹뷰의 웹페이지를 시스템 '제한됨'
    // 화면으로 차단한다. 유저 지정 허용 웹도메인(토큰)은 WebDomain으로 변환 불가라 예외로
    // 못 옮긴다. 해제는 stopFocusShield의 clearAllSettings()도 커버한다.
    @available(iOS 16.0, *)
    private static func applySafariWebBlock(to store: ManagedSettingsStore, allowSafariWeb: Bool) {
        if allowSafariWeb {
    ...
    @objc func setFocusAllowSafariWeb(
        ...
        if #available(iOS 16.0, *) {
            ...
            if store.shield.applicationCategories != nil {
                ScreenTimeModule.applySafariWebBlock(to: store, allowSafariWeb: allowed)
            }
        }
        resolve(nil)
    }

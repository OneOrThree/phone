...
(async () => {
  ...
  for (const [orientation, width, height] of [
    ...
    assert.ok(img);
    await p.mouse.click(img.x + (350 / 1024) * img.width, img.y + (830 / 1536) * img.height);
    await p.waitForFunction(() => window.__gromoReview.route === 'focusSetup');
    assert.equal((await state()).session, null);
    await p.getByPlaceholder('수학 문제 풀기').fill('영어 단어 외우기');
    ...
    await p.getByRole('button', { name: '하트', exact: true }).click();
    ...
    await p.getByRole('button', { name: '구매', exact: true }).click();

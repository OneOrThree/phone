...
class ScreenTimeModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    // 2.0 퀘스트·기록과 같은 KST 날짜로 한 시점의 오늘/어제 값을 반환한다.
    // null/권한 오류를 실제 0분으로 바꾸지 않는다. JS가 측정 시작일과 확정 기록을 보존한다.
    AsyncFunction("getUsageSnapshot") {
      check(isUsageAccessGranted()) { "USAGE_ACCESS_DENIED" }
      val userManager = context.getSystemService(Context.USER_SERVICE) as UserManager
      check(userManager.isUserUnlocked) { "USER_LOCKED" }
      ...
      fun dateKey(calendar: Calendar) = String.format(java.util.Locale.US, "%04d-%02d-%02d",
        calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH))
      // 2.0 Android는 전체 앱 측정이다. 1.x의 앱 피커 선택 키는 가져오지 않는다.
      val minutes = (UsageSessionCalculator.foregroundMillis(manager, null, today.timeInMillis, now) / 60000L).toInt()
      val previous = (UsageSessionCalculator.foregroundMillis(manager, null, yesterday.timeInMillis, today.timeInMillis) / 60000L).toInt()
      check(isUsageAccessGranted()) { "USAGE_ACCESS_DENIED" }
      mapOf("date" to dateKey(today), "minutes" to minutes,
        "previousDate" to dateKey(yesterday), "previousMinutes" to previous)
    }

    AsyncFunction("resetScreenTimeData") {

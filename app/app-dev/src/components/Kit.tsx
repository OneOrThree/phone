...
import { C, useScreenInsets, MotionContext } from '@/components/ui';
    ...
    <Animated.View style={[{ transform: [{ scale: s }] }, style]}>
      <Pressable
        ...
        style={{
          ...
          borderColor: kind === 'destructive' ? '#B84A32' : C.brown,
          backgroundColor: kind ? 'transparent' : C.pink,
          alignItems: 'center',
          justifyContent: 'center',
          opacity: disabled ? 0.45 : 1,
          boxShadow: kind || disabled ? 'none' : `0px ${small ? 3 : 4}px 0px ${C.brown}`,
          ...(kind === 'sec' ? { backgroundColor: C.paper } : {}),
          // glass = 몰입 화면(바다·모닥불) 위 반투명 보조 버튼, destructive = 탈퇴 같은 파괴적 확인
          ...(kind === 'glass' ? { backgroundColor: '#FFFDFAB8' } : {}),
          ...(kind === 'destructive' ? { backgroundColor: '#F4C0C0' } : {}),
        }}
      >
        <Txt
          style={{
    ...
    <View style={[k.group, flat && { boxShadow: 'none' }, style]}>
      {React.Children.toArray(children)
        ...
        .map((c: any, i) => (
          <View
            key={c.key || i}
            style={i ? { borderTopWidth: 1, borderTopColor: '#8B695633' } : undefined}
          >
            {c}
          </View>
        ))}
      ...
      {items.map((x: string) => (
        <Pressable
          ...
          style={{
            ...
            borderRadius: 999,
            borderWidth: 1.5,
            borderColor: x === value ? C.brown : '#D9C6B8',
  ...
  return (
    <View style={{ gap: 6 }}>
      ...
      <TextInput
        ...
        placeholderTextColor={placeholderColor || '#A0908A'}
        multiline={multiline}
  ...
  return (
    <Pressable
      ...
      style={{
        ...
        height: 28,
        borderRadius: 999,
        backgroundColor: value ? '#3FB6A0' : '#D9C6B8',
      }}
    >
      <Animated.View
        style={{
  ...
  return (
    <View
      style={{
        ...
        borderRadius: 999,
        borderWidth: 1.5,
        borderColor: soft ? '#D9C6B8' : C.brown,
    ...
    >
      ...
      <View
        style={{
          position: 'absolute',
          inset: 0,
          backgroundColor: sheet ? '#493B3940' : '#493B3966',
  ...
  return (
    <View style={{ gap: 4 }}>
      <Svg width="100%" height={110} viewBox="0 0 362 110">
        {[20, 58, 96].map((y) => (
          <Line key={y} x1={0} x2={362} y1={y} y2={y} stroke="#8B695622" />
        ))}
        <Path

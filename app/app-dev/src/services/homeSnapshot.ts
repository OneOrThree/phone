...
export async function loadHomeSnapshot({
  ...
  alive();
  // 어댑터는 wire 를 검증하지 않는다 — items/island/id 가 빠진 응답이 TypeError 로 새지 않게
  // 쓰기 직전에 좁게 확인하고 계약 오류로 통일한다.
  if (!Array.isArray(mine.items)) throw contractError('items');
  ...
  alive();
  const [home, options, firstPage] = await Promise.all([
    getHome(date, timezone),
    getConstructionOptions(current),
    getMembers(current),
  ]);
  alive();
  // 두 도메인 응답에는 islandId 가 없다 — home 의 섬이 캡처한 current 와 같은지 여기서 묶는다.
  if (!isRecord(home.island) || typeof home.island.id !== 'string') {
  ...
  const take = (page: MembersPage) => {
    if (!Array.isArray(page.items)) throw contractError('members.items');
  ...
  while (cursor !== null) {
    ...
    alive();
    const page = await getMembers(current, cursor);
    alive();
    take(page);
  ...
  alive();
  const again = await myIslands();
  alive();
  if (
    !Array.isArray(again.items) ||
    again.currentIslandId !== current ||
    !again.items.some((item) => item.id === current)
  ) {
    throw new ApiError(CLIENT_SNAPSHOT_STALE, '섬 상태가 바뀌었어요. 다시 시도해 주세요.', 0);
  }

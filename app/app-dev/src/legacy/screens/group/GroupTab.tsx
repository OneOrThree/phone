...
import { T, inkBox } from '@/legacy/constants/legacyTheme';
...
import type { RootStackParamList } from '@/legacy/types/navigation';

// 그룹 상세에 화면에서 쓰는 추가 필드를 더한 형태
...
// 멤버 그리드 항목 — 초대 카드 placeholder 포함
type MemberListItem = GroupMember | { __invite: true };
...
function formatWindowTime(timeStr: string | null | undefined) {
  if (!timeStr) return '';
...
function SpeechBubble({ bio, isMe, onPress }: SpeechBubbleProps) {
...
function MemberCard({ member, myUserId, bio, onEditBio, onPress }: MemberCardProps) {
...
export default function GroupTab({ group, groupId, refreshing, onRefresh }: GroupTabProps) {
  ...
  const sortedMembers = [...(group.members ?? [])].sort(
    (a, b) => (b.focusTimeMinutes ?? 0) - (a.focusTimeMinutes ?? 0),
  );

  const isFull = (group.members?.length ?? 0) >= (group.maxMembers ?? 0);
  const listData: MemberListItem[] = isFull
    ? sortedMembers
    : [...sortedMembers, { __invite: true }];
  ...
  return (
    <FlatList<MemberListItem>
      data={listData}
      keyExtractor={(item) => ('__invite' in item ? '__invite' : String(item.userId))}
      numColumns={NUM_COLS}
      renderItem={({ item }) =>
        '__invite' in item ? (
          <InviteCard />

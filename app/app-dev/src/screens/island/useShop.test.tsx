...
test('focus route — 집중 화면용 공동 보유곡 정본을 읽어 SHOP_SYNC한다', async () => {
  const dispatch = jest.fn();
  const { result } = await renderHook((p: Parameters<typeof useShop>[0]) => useShop(p), {
    initialProps: params({ route: 'focus', dispatch }),
  });

  await waitFor(() => assert.equal(result.current.shared?.inventoryVersion, 1));
  assert.equal((getIslandInventory as jest.Mock).mock.calls.length, 1);
  const sync = dispatch.mock.calls.map((call) => call[0]).find((action) => action.sharedInventory);
  assert.deepEqual(sync.sharedInventory.audio, ['waves']);
  assert.equal((getShopProducts as jest.Mock).mock.calls.length, 0);
});

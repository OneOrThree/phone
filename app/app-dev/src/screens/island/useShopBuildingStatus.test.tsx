...
const page = (...items: ReturnType<typeof item>[]): ShopProductPage => ({ items, nextCursor: null });
...
it('사용 불가이거나 이미 보유한 상품은 구매 상태로 표시하지 않는다', async () => {
  ...
  const next = updateShopCatalogSnapshot(
    previous,
    [item('shirt', { owned: true }), item('hat', { available: false })] as any,
  );

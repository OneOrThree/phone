...
func buildActivityReport(from data: DeviceActivityResults<DeviceActivityData>) async -> ActivityReport {
    ...
    let sharedDefaults = UserDefaults(suiteName: "group.com.oneorthree.focuscat")
        ...
        for await segment in d.activitySegments {
            ...
            for await categoryActivity in segment.categories {
                ...
                for await app in categoryActivity.applications {
                    ...
                    if app.application.bundleIdentifier == "com.oneorthree.focuscat" {
    ...
    let totalDuration = max(0, hasSelection ? selectedSum : segmentTotal)

    // 메인 앱이 App Group에 쓴 목표 시간을 읽어서 "남은 시간" 계산에 사용
    let goalSeconds = (sharedDefaults?.object(forKey: "gromo:user:goalSeconds") as? NSNumber)?.doubleValue ?? -1

    return ActivityReport(
        totalDuration: totalDuration,
        apps: apps,
        categories: categories,
        isAvailable: hasSegments && hasSelection,

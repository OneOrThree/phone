...
- 모닥불 모션은 낮·밤 배경에 이미 그려진 화덕을 덮도록 원본 캔버스 좌표 `[358, 434, 150, 90]`에서 렌더링하고, 실제 섬 배율을 따라 움직인다. 컨테이너 투명도를 낮추지 않아 정적 불꽃이 비치지 않게 한다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.

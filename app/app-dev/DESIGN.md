...
- 쪽지의 치수와 간격은 `componentTokens.homeQuestIndicator`를 정본으로 삼는다. 높이는 고정하지 않고 콘텐츠에 맞춰 늘어나며, 큰 글자 모드에서는 제목과 조건을 각각 두 줄까지 표시한다. 홈 HUD와 쪽지는 같은 세로 레이아웃에 두어 HUD 높이가 커져도 겹치지 않는다.

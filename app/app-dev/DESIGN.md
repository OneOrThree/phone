...
- 밤에는 정적 돌 테두리와 장작을 배경 원본에 두고 모션 PNG의 불꽃 코어만 클립해 노출한다. 낮에는 배경의 정적 불꽃 전체가 남지 않도록 불꽃 없는 화덕 에셋의 안쪽 장작 코어를 먼저 겹친 뒤, 연기 프레임의 연기 코어만 클립해 올린다. 코어의 원본 캔버스 상자는 `[420, 443, 100, 71]`이며 실제 섬 배율을 따라 움직인다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.
- 주민 0명은 약한 불빛과 느린 잔불 프레임, 주민 1~2명은 기본 모션, 3명 이상은 더 빠르고 긴 불꽃·연기 순환 및 강한 불빛을 사용한다. 아직 주민 수를 불러오지 못했다면 기본 모션을 사용한다.

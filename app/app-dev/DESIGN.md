...
- 밤에는 정적 돌 테두리와 장작을 배경 원본에 두고 모션 PNG의 불꽃 코어만 클립해 노출한다. 낮에는 배경에서 불꽃만 제거해 만든 투명 오버레이 `village-world/motion/fire/day-fire-off-overlay.png`(`[440, 435, 60, 80]`)를 먼저 합성하고, 연기 프레임의 연기 코어를 표시한다. 모션 코어 상자는 `[420, 443, 100, 71]`이며 실제 섬 배율을 따라 움직인다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.

...
- 홈의 상점 상태는 개인·섬 상품 카탈로그에서 사용 가능하고 아직 보유하지 않은 상품을 기준으로 한다. 첫 조회는 기기별 기준점으로 저장하고 이후 새로 추가된 상품이 있으면 실제 상점에 들어갈 때까지 새 상품 상태를 유지한다. 그 외 구매 가능 상품은 구매 가능 상태로 표시한다. 카탈로그 조회에 실패하면 상태 표시는 숨긴다.
- 홈 건물 위의 상점 상태 이름표는 `componentTokens.shopStatusLabel`의 최소 높이·가로 여백·반경·선 굵기·글자 크기를 사용한다. 모션 레이어는 장식으로 접근성 트리에서 숨기며 상품 상태는 건물 버튼에서 한 번만 읽힌다.
...
- 낮에는 장작 픽셀을 제거하고 연기만 투명 추출한 `motion/fire-smoke-core` 4개 에셋을 360ms 간격으로, 저녁에는 실제 순환에 포함된 3개 불꽃 프레임(1~3)을 155ms 간격으로 표시한다. 빈 섬의 밤은 프레임 1~2만 느리게 순환한다. 동작 줄이기 설정이나 앱 비활성 상태에서는 낮 프레임 0/밤 프레임 1에 멈춘다.
- 밤에는 정적 불꽃과 겹치지 않도록 배경의 불꽃 실루엣에 맞춘 불투명 코어를 모션 아래에 먼저 그리고, 모션 PNG의 불꽃 코어만 클립해 노출한다. 낮에는 배경에서 불꽃만 제거해 만든 투명 오버레이 `village-world/motion/fire/day-fire-off-overlay.png`(`[440, 435, 60, 80]`)를 먼저 합성하고, 연기 에셋의 연기 실루엣만 표시한다. 모션 코어 상자는 `[420, 443, 100, 71]`이며 실제 섬 배율을 따라 움직인다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.
- 주민 0명은 같은 가림 코어 위에 0.68배 축소·52% 투명도의 작은 불씨 프레임을 느리게 표시하고, 주민 1~2명은 기본 모션, 3명 이상은 더 빠르고 긴 불꽃·연기 순환 및 강한 불빛을 사용한다. 아직 주민 수를 불러오지 못했다면 기본 모션을 사용한다.
- `MotionContext`의 동작 줄이기 설정과 개별 `reduceMotion` prop 중 하나라도 켜져 있으면 프레임 타이머를 정리하고 정지 프레임을 유지한다.

...
- 낮에는 화덕 위에 4개 연기 프레임을 360ms 간격으로, 저녁에는 4개 불꽃 프레임을 155ms 간격으로 순환한다. 동작 줄이기 설정, 앱 비활성 상태에서는 각각 첫 연기/기본 불꽃 프레임에 멈춘다.
- 밤에는 정적 돌 테두리와 장작을 배경 원본에 두고 모션 PNG의 불꽃 코어만 클립해 노출한다. 주민이 없을 때는 더 넓은 불투명 잔불 실루엣을 모션 코어보다 먼저 그려 배경의 큰 정적 불꽃이 새지 않게 덮은 뒤, 약화한 작은 불꽃 프레임을 겹친다. 낮에는 배경에서 불꽃만 제거해 만든 투명 오버레이 `village-world/motion/fire/day-fire-off-overlay.png`(`[440, 435, 60, 80]`)를 먼저 합성하고, 연기 프레임의 연기 코어를 표시한다. 모션 코어 상자는 `[420, 443, 100, 71]`이며 실제 섬 배율을 따라 움직인다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.
- 주민 0명은 불꽃 없는 장작 코어 위에 0.68배 축소·52% 투명도의 작은 불씨 프레임을 느리게 표시하고, 주민 1~2명은 기본 모션, 3명 이상은 더 빠르고 긴 불꽃·연기 순환 및 강한 불빛을 사용한다. 아직 주민 수를 불러오지 못했다면 기본 모션을 사용한다.

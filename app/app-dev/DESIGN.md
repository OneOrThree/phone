...
- 게시판 완공 뒤 홈에는 `오늘 집중` 시간 바로 아래에 작은 쪽지 묶음을 둔다. 한 개면 실제 제목과 조건, 여러 개면 앞 퀘스트와 전체 개수, 받을 보상이 있으면 `보상 받기`와 개수를 우선 표시한다. 진행률 막대는 넣지 않으며, 누르면 게시판의 퀘스트 목록을 연다. 게시판 미완공·방문 중·퀘스트와 받을 보상이 모두 없는 상태에서는 숨긴다.

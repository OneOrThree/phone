...
- 유효 집중 60초마다 물고기 1마리가 섬 잔액에 바로 들어간다. 하루 상한은 주민별·섬 전체 각각 480마리이고 하루 경계는 UTC 날짜다. 상한에 닿아도 집중 기록은 쌓이지만 물고기는 더 낚지 않으며 못 받은 몫은 이월하지 않는다. 섬 한도 도달 시 남은 개인 한도와 무관하게 무보상 상태로 안내한다.
...
- API 연결 상태: 제품 목표는 주민 누구나 구매다. 현재 서버의 `SHARED_PURCHASE` 기본값은 `OWNER_ONLY`이고 변경 API가 없어 일반 주민 구매가 거절되므로, 서버 계약이 제품 정책과 맞기 전에는 주민에게 구매 가능·완료 상태를 노출하지 않고 “구매 권한 준비 중”으로 안내한다. 이 임시 차단을 제품 권한으로 확정하지 않는다.
...
- 공지 목록, 상세 읽기, 작성·수정을 구분한다. 파일 첨부·업로드는 지원하지 않는다.

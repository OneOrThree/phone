...
- 정적 돌 테두리와 장작은 낮·밤 배경 원본을 유지하고, 모션 PNG는 SVG 클립 경로로 중앙 불꽃/연기 코어만 노출한다. 코어의 원본 캔버스 상자는 `[420, 443, 100, 71]`이며 실제 섬 배율을 따라 움직인다. 이로써 프레임 타일의 다른 모양 돌·투명 가장자리가 정적 화덕과 겹치지 않는다. `WorldMap`의 서버 홈은 정본 `memberCount`, 로컬/방문 섬은 해당 섬의 주민 수를 전달한다.

...
// 그룹 설정 = 관리 허브 (root stack 'GroupSettings') — 3차 A-1. **방장 전용**.
//
// 진입 → getGroupDetail(groupId) → 실패 [에러+재시도] / 방장 아님 [권한 안내] / 편집 폼
//
// 형태: 헤더(항상 원형 백버튼) + 편집 폼(이름·소개·정원·공개설정 + 저장) + 관리 진입 섹션
//       (방장 넘기기·멤버 관리·공지 권한 — 각각 다른 라우트로 push).
//
// ⚠️ 저장은 **바뀐 필드만** PATCH한다(updateGroup은 부분 수정 — 보낸 필드만 반영). 바뀐 게
//    없으면 저장을 잠근다. 성공하면 기준값(base)을 방금 보낸 값으로 굳혀 '변경 없음'으로 되돌린다
//    (재조회 대신 로컬 갱신 — 서버가 방금 쓴 값을 그대로 갖고 있어 다시 받을 이유가 없다).
// ⚠️ 진입은 방장 전용이지만(그룹방 ⋯ 메뉴) 여기서도 방어적으로 한 번 더 막는다 — 멤버십은
//    다른 기기에서 바뀔 수 있어(위임·강퇴) 상세의 내 role이 OWNER가 아니면 폼을 열지 않는다.
...
export default function GroupSettingsScreen() {
  ...
  // (NoticeScreen·GroupRoomScreen과 같은 패턴). cleanup에서 올려 언마운트 뒤 setState를 막는다.
  ...
  // 스텝퍼에서 미리 막고, 그래도 새 나가면 저장 실패를 code로 잡아 안내한다(이중 방어).
  ...
  async function save() {
    ...
    try {
      ...
      // 정원을 현재 인원 미만으로 줄인 경우 — 공통 문구로 떨어뜨리면 '잠시 후 다시 시도'가 되는데
      // 인원이 그대로면 영원히 같은 실패라 사유를 그대로 알려준다(§3-2 code 분기).
  ...
  // 헤더 — 원형 백버튼 + 좌측 정렬 제목(그룹 만들기·공지 화면과 같은 규격, §5-1).
  // 로딩·에러·권한 분기에도 항상 세운다 — 이 화면엔 네이티브 헤더도 탭바도 없다.
  const header = (
    <View style={s.header}>
      <TouchableOpacity
        style={s.backBtn}
  ...
  if (loading && detail === null) {
    ...
    body = (
      <View style={s.center}>
        ...
        <Text style={s.emptyDesc}>그룹 설정은 방장이 관리해요.</Text>
      </View>
    );
  } else {
    // ── 편집 폼 + 관리 섹션 ──
    body = (
      ...
      >
        ...
        {/* ── 관리 진입 섹션 ── */}
        <Text style={s.sectionTitle}>관리</Text>
...
const s = StyleSheet.create({
  ...
  // 화면 전체를 차지하는 에러/권한 헤드라인은 T.text.title(26/800) — 그룹 3화면 공통 위계.
  ...
  // 관리 진입 섹션
  sectionTitle: { ...T.text.label, color: T.ink, marginTop: T.space.xxl, marginBottom: T.space.sm },
  navGroup: { gap: T.space.sm },
  navRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: T.space.md,

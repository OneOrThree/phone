...
export default function BetSheet({
  ...
  function handleSubmitError(e: unknown, sentTomorrow: boolean) {
    switch (groupErrorCode(e)) {
      ...
      // 같은 날 재개설이 실제로 성공한다). 개설 날짜는 오늘·내일뿐이라 이 409의 뜻은
      // "그 날짜에 활성 내기 존재" 하나 — '이미 열려 있어요'가 항상 사실이다.
      // ⚠️ 내일 날짜로 보낸 요청(마감 후 개설·재시도)의 충돌은 **내일** 내기 이야기다 —
      //    '오늘' 문구를 그대로 내면 거짓이 된다(PR #473 리뷰). 두 브랜치 모두
      //    존재 사실 + 참가 안내만 말한다(정책 설명 금지 — 계약 §2).

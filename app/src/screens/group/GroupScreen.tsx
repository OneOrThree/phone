...
export default function GroupScreen() {
  ...
  // 같은 GroupScreen 인스턴스가 유지돼도 다른 탭에서 탭바의 그룹 버튼을 누른 재진입은 `tab`이다.
  // 자식 스택(GroupRoom 등)에서 뒤로 온 focus에는 tabPress가 없으므로 `return`과 구분할 수 있다.
  useEffect(() => {
  ...
  const closeInvite = useCallback((requested?: PendingInvite | null) => {
    ...
    inviteOpenedWhileFocusedRef.current = false;
    clearPendingInvite();
    setInvite(null);
  }, []);

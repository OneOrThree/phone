...
export function useStagedRanking<T extends Keyed>(items: T[]): T[] {
  ...
  //
  //    하한을 라이브 합계로 올리는 쪽은 택하지 않았다 — rankSwap의 감소 가드가 "하나라도 줄면
  //    전체 생략"이라, 집중 중인 사람이 있는 한 거의 모든 갱신에서 단계화가 통째로 꺼진다.
  //    ("그 행만 기록 단계화에서 빼면 혼자 최종값으로 튄다"던 옛 걱정은 GROMO-1475가 닫았다 —
  //     단계값은 이제 프레임 전체를 보고 배정돼 어떤 행도 혼자 튀지 않는다. 남은 문제는 이 훅이
  //     **화면에 실제로 보이던 라이브 합계를 모른다**는 것뿐이고, 그건 여기서 건너뛰어 막는다.)

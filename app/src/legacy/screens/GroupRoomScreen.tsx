...
import type { TabScreenProps } from '@/legacy/types/navigation';
import type { Group } from '@/types/api';

// 그룹 상세 응답 — 공용 Group을 화면에서 쓰는 추가 필드로 확장
...
export default function GroupDetailScreen({ navigation, route }: TabScreenProps<'GroupDetail'>) {
          ...
          {showSettings ? (
            <TouchableOpacity
              onPress={() => {
                ...
                navigation.setParams({ showSettings: false } as never);
              }}
          ...
          {!showSettings && (
            <TouchableOpacity
              onPress={() => {
                ...
                navigation.setParams({ showSettings: true } as never);
              }}
              hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }}
      ...
      <View style={s.content}>
        {loading ? (
          ...
          <SettingsScreen
            ...
            onChatEnabledChange={(val) => navigation.setParams({ chatEnabled: val } as never)}
            onGroupUpdated={(patch) =>
              setGroup((prev) => ({ ...(prev ?? {}), ...patch }) as GroupDetail)
            }

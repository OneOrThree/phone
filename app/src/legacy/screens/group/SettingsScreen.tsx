...
interface SettingsScreenProps {
  ...
  isOwner: boolean;
  onLeaveSuccess?: () => void;
  onChatEnabledChange?: (val: boolean) => void;
  onGroupUpdated?: (patch: Partial<GroupDetail>) => void;
  onRefresh?: () => void;
}
...
function formatExpiry(instant: string | null | undefined) {
  if (!instant) return '';
...
function ToggleRow({ label, sub, value, onValueChange }: ToggleRowProps) {
  return (
...
function NavRow({ label, onPress, danger, indent, value }: NavRowProps) {
  return (
...
function SelectorRow({ label, value, options, onChange }: SelectorRowProps) {
  return (
    <View style={s.row}>
...
export default function SettingsScreen({
  ...
  async function handleToggleChatEnabled(val: boolean) {
    setChatEnabled(val);
  ...
  async function handleInvitePermission(val: string) {
                ...
                if (delegateMode === 'leave') {
                  ...
                  Alert.alert('방장 위임', '정말 방장 권한을 위임할까요?', [
                    { text: '취소', style: 'cancel' },
                    {
                      ...
                      style: 'destructive',
                      onPress: () => {
                        if (delegateTarget != null) doDelegate(delegateTarget);
                      },
                    },
                  ]);
                }

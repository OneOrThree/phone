...
//
// ── 시트를 새로 추가할 때: **"누가 열었는가"로 갈린다** ──────────────────────────
// 여기가 그 판단을 적어 두는 자리다. 전면 오버레이(RN Modal)를 새로 만들면 둘 중 하나를 고른다.
//
//   A. **사용자가 방금 손으로 열었다** → `useOverlayBlocker(id, open)`.
//      등록만 하고 승인은 기다리지 않는다. 전면 모달이 떠 있으면 그 버튼을 누를 수 없으므로
//      "이미 뜬 오버레이 위에 이게 열린다"가 성립하지 않는다.
//      예: 그룹 찾기 시트 · 내기/챌린지 만들기 시트 · 챌린지 카드가 여는 시트 4종.
//
//   B. **사용자 터치 없이 열린다** → `useOverlaySlot(...) === 'granted'` 일 때만 마운트한다.
//      외부 딥링크나 비동기 응답이 여는 것들이다. 이미 보유자가 있는 순간에도 도착할 수 있고,
//      조정자는 보유자를 뺏지 않으므로 그냥 렌더하면 **RN Modal 두 개가 겹친다.**
//      그러면 뒤에 깔린 결과 모달이 **가려진 채 seen/ack** 되어 사용자는 한 번도 못 봤는데
//      서버는 봤다고 기록한다 — 이 조정자를 만든 이유가 통째로 무너지는 자리다.

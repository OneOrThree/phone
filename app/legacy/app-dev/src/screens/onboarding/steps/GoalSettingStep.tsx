...
function goalLabel(minutes: number): string {
  return minutes === 0 ? t('onboarding.format.hours', { count: 0 }) : formatDuration(minutes);
}
...
export default function GoalSettingStep({ data, update, onNext }: StepProps) {
  ...
  return (
    <StepScaffold
      ...
      center
      scrollable
      title={t('onboarding.goalSetting.title')}
    ...
    >
      <View style={s.cards}>
        <View style={s.card}>
          <TouchableOpacity
            ...
            onPress={() => toggleFocus()}
          >
            <Text style={s.label}>{t('onboarding.goalSetting.focusLabel')}</Text>
        ...
        <View style={s.card}>
          ...
          >
            <Text style={s.label}>{t('onboarding.goalSetting.screenLabel')}</Text>
            <View style={s.valueWrap}>
              <Text style={s.value}>
                {t('onboarding.goalSetting.screenValue', { value: goalLabel(screenMin) })}
              </Text>
              <Ionicons
      ...
      {!focusSet || !screenSet ? (
        <Text style={s.hint}>{t('onboarding.goalSetting.hint')}</Text>
      ) : null}
    </StepScaffold>
  );
}

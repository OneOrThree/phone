...
/**
 * V44(참가 행 {@code (user_id, session_id)} 인덱스, GROMO-1415 조회 성능)의 실 SQL 검증.
 *
 * <p>보는 것: ① V43 까지는 user_id <b>선두</b> 인덱스가 없다(V39 유니크는 선두가 session_id 라
 * 참가자 스코프 조회 {@code /me/*} 가 못 쓴다), ② V44 가 그 인덱스를 만든다, ③ 컬럼 순서가
 * (user_id, session_id) 다 — 순서가 뒤집히면 존재해도 같은 이유로 안 쓰인다.
 */
class GroupBetParticipantV44MigrationTest {
    ...
    @Test
    @DisplayName("V43 까지는 user_id 선두 인덱스가 없다 — V44 가 (user_id, session_id) 로 만든다")
    void userSessionIndexAppearsAtV44() {
        migrate("43");
    ...
    /**
     * 요청 버전 이하의 <b>실재하는</b> 최고 버전으로 타깃을 해석한다 — 선행 배치(B4 의 V42)가
     * 아직 없는 워크트리에서도 동작한다({@code GroupBetV41MigrationTest} 와 같은 장치).
     */

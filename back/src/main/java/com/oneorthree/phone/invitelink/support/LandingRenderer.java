...
public class LandingRenderer {
    ...
    /** 초대자 닉네임 표시 상한(코드포인트). og:title 한 줄에서 초대자가 그룹명을 밀어내지 않을 만큼만 남긴다. */
    ...
    private static String escapeInviterName(String inviterName) {
        ...
        if (trimmed.codePointCount(0, trimmed.length()) <= INVITER_NAME_MAX) {
            return HtmlUtils.htmlEscape(trimmed);
        }
        // 자르기는 이스케이프 <b>전에</b> 한다 — 뒤에 자르면 &quot; 같은 엔티티가 중간에서 잘려 깨진다.
        // 코드포인트 기준이라 이모지(서로게이트 쌍) 닉네임도 반쪽으로 쪼개지지 않는다.
        String cut = trimmed.substring(0, trimmed.offsetByCodePoints(0, INVITER_NAME_MAX));

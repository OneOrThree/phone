-- ════════════════════════════════════════════════════════════════════
-- V24 — 그룹 챌린지 확장: 내기 상태 확장 + 창 목표분 + 창 사용분 보고 + 활성 중복 DB 강제
-- ════════════════════════════════════════════════════════════════════
-- 원래 V20 으로 머지됐으나(#414), 53초 앞서 머지된 V21(초대 링크, #412)이 먼저 배포되며
-- dev DB 가 19 → 21 로 건너뛰어 V20 이 영영 미적용 상태가 됐다. 이후 모든 배포가 Flyway
-- validate("resolved migration not applied to database: 20")에서 죽어 dev 가 502 였다.
-- forward-only 로 수습 — 적용된 적 없는 파일이라 번호만 V24 로 옮긴다(내용 무변경).
-- V21~V23 과 건드리는 객체가 겹치지 않아(group_challenge_* 전용) 뒤에 적용해도 최종 스키마 동일.

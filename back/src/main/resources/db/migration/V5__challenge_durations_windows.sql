...
-- ── 3) 챌린지가 없는 기존 그룹의 미션 설정 → 대표 챌린지 신설(보존 이관) ─────────
...
WHERE g.mission_type IS NOT NULL AND g.mission_category IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM group_challenges c WHERE c.group_id = g.id);

-- 신설(또는 상세 없는) DURATION/TIME_WINDOW 챌린지에 그룹 인라인 값 백필
INSERT INTO group_challenge_durations (challenge_id, duration_minutes)
SELECT c.id, g.duration_minutes
FROM group_challenges c
         JOIN groups g ON g.id = c.group_id
WHERE c.type = 'DURATION' AND g.duration_minutes IS NOT NULL
...
FROM group_challenges c
         JOIN groups g ON g.id = c.group_id
WHERE c.type = 'TIME_WINDOW' AND g.window_start IS NOT NULL AND g.window_end IS NOT NULL
